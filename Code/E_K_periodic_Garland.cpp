...
class SegmentTree {
    ...
    Node* build(string &s,int start,int end){
        Node *node;
        if(start is end){
            node = new Node(start,end,s[start] is '1');
        } else {
            int mid = (start + end) >> 1;
            Node *left = this.build(s,start,mid);
            Node *right = this.build(s,mid + 1,end);
            node = new Node(start,end,left->cnt + right->cnt);
            node->left = left;
            node->right = right;
        }
        return node;
    ...
    SegmentTree(string &s){
        this.root = this.build(s,0,s.sz() - 1);
    }
    ~SegmentTree(){
        delete this.root;
    }
...
void testcase(){
    ...
    inc(i,0,n){
        if(i < k){
            dp[i] = min(tree.query(0,i - 1),(s[i] is '0') + tree.query(0, i - 1));
        } else {
            dp[i] = min(tree.query(0,i - 1),(s[i] is '0') + tree.query(i - k + 1,i - 1) + dp[i - k]);
        }
    }
    int ans = inf;
    inc(i,0,n){
        ans = min(ans,dp[i] + tree.query(i + 1,n - 1));
    }
    see(ans);
    return;
}
