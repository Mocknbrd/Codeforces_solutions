...
void testcase(){
    ...
    vt(vpii)pos(n + 1);
    inc(i,0,n){
        pos[first[i]].pb(mp(i,0));
    }
    inc(i,0,n){
        pos[second[i]].pb(mp(i,1));
    }
    vi f1,f2,p;
    inc(i,1,n + 1){
        if(pos[i].empty() is false){
            if(pos[i].front().sc is pos[i].back().sc){
                if(pos[i].front().sc is 0){
                    f1.pb(i);
                } else {
                    f2.pb(i);
                }
    ...
    vi ans1,ans2;
    int size = min(k,2 * min((int)f1.sz(),(int)f2.sz()));
    int start = 0;
    while(start < f1.sz() and ans1.sz() + 2 <= size){
        int value = f1[start++];
        ans1.pb(pos[value].front().fi);
        ans1.pb(pos[value].back().fi);
    }
    start = 0;
    while(start < f2.sz() and ans2.sz() + 2 <= size){
        int value = f2[start++];
        ans2.pb(pos[value].front().fi);
        ans2.pb(pos[value].back().fi);
    }
    start = 0;
    while(start < p.sz() and ans1.sz() < k){
        int value = p[start++];
        ans1.pb(pos[value].front().fi);
        ans2.pb(pos[value].back().fi);
    }
