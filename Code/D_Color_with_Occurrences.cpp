...
void testcase(){
    ...
    inc(ind,0,n){
        string cnd = arr[ind];
        inc(i,0,t.sz()){
            if(i + cnd.sz() <= t.sz() and t.substr(i,cnd.sz()) is cnd){
                range.pb(vi(3,0));
                range.back()[0] = i;
                range.back()[1] = i + cnd.sz() - 1;
                range.back()[2] = ind + 1;
            }
        }
    }
    ...
    while(start < range.sz() and range[start].front() is 0){
        start++;
    }
    if(start is 0){
        see(-1);
    } else {
        left = 0,right = range[start - 1][1];
        ans.pb(make_pair(range[start - 1].back(),range[start - 1].front() + 1));
        while(start < range.sz() and right isnt t.sz() - 1){
            int end = start,cnd = start;
            while(end < range.sz() and range[end][0] <= right + 1){
                if(range[end][1] > range[cnd][1]){
                    cnd = end;
                }
                end++;
            }
            if(end is start){
                ...
                right = range[cnd][1];
                ans.pb(make_pair(range[cnd].back(),range[cnd].front() + 1));
                start = end;
            }
        }
        if(right isnt t.sz() - 1){
            see(-1);
        } else {
            see(ans.sz());
            each(entry,ans){
                see(entry.fr << " " << entry.sc);
            }
        }
    }
    return;
