...
void testcase(){
    ...
    inc(first,0,10){
        inc(second,0,10){
            ...
            cnds.pb(first);
            cnds.pb(second);
            int curr = -1,cnd = 0,parity = 0;
            while(true){
                int start = 0,end = pos[cnds[parity]].sz() - 1,next = -1;
                while(start <= end){
                    int mid = (start + end) >> 1;
                    if(pos[cnds[parity]][mid] > curr){
                        next = pos[cnds[parity]][mid];
                        end = mid - 1;
                    } else {
                        start = mid + 1;
                    }
