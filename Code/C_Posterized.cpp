...
void testcase(){
    ...
    vi ans = input;
    inc(index,0,n){
        int currColor = input[index];
        if(colors[currColor] is -1){
            int target = currColor - 1;
            while(target >= 0 and colors[target] is -1){
                target--;
            }
            int start = currColor - 1;
            if(target is -1 or currColor - colors[target] + 1 > k){
                start = max(target + 1,currColor - k + 1);
            } else {
                start = colors[target];
            }
            inc(j,start,currColor + 1){
                colors[j] = start;
            }
        }
        ans[index] = colors[currColor];
    }
    writeArray(ans);
    return;
}
