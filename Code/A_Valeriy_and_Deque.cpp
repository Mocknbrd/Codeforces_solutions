...
void testcase(){
    ...
    while(start < n){
        ...
        while(end < n and arr[end] < arr[start]){
            nonCycle[turn++] = mp(arr[start],arr[end]);
            dq.pb(arr[end++]);
        }
        if(end < n){
            nonCycle[turn++] = mp(arr[start],arr[end]);
    ...
    turn = 0;
    inc(i,0,n - 1){
        int first = dq.front(); dq.rf();
        int second = dq.front(); dq.rf();
        cycle[turn++] = mp(first,second);
        dq.pf(max(first,second));
        dq.pb(min(first,second));
    }
    while(q--){
        cin >> turn;
        int key = mod(turn - 1,n - 1);
        if(turn - 1 < n - 1){
            see(nonCycle[turn - 1].fi << " " << nonCycle[turn - 1].sc);
        } else {
            see(cycle[key].fi << " " << cycle[key].sc);
        }
    }
    return;
}
