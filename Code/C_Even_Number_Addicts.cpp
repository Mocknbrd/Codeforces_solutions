...
const ll linf = 2e18;
ll dp[101][101][2][2][2];
bool solve(int even,int odd,int alice,int bob,int curr);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    inc(i,0,n){
        int value;
        cin >> value;
        even += mod(value,2) is 0;
        odd += mod(value,2) is 1;
    }
    if(solve(even,odd,0,0,0) is true){
        Aa;
    } else {
        Bb;
    }
...
bool solve(int even,int odd,int alice,int bob,int curr){
    if(even is 0 and odd is 0){
        return alice is 0;
    } elif(dp[even][odd][alice][bob][curr] isnt -1){
        return dp[even][odd][alice][bob][curr];
    } else {
        if(curr is 0){
            bool ans1 = false,ans2 = false;
            if(even isnt 0){
                ans1 = solve(even - 1,odd,alice,bob,curr ^ 1);
            } 
            if(odd isnt 0){
                ans2 = solve(even,odd - 1,alice ^ 1,bob,curr ^ 1);
            }
            return dp[even][odd][alice][bob][curr] = (ans1 or ans2);
        } else {
            ...
            if(odd isnt 0){
                ans2 = solve(even,odd - 1,alice,bob ^ 1,curr ^ 1);
            }
            return dp[even][odd][alice][bob][curr] = (ans1 and ans2);
        }
    }
}
