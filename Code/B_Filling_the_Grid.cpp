...
class DSU {
    ...
    void merge(int larger,int smaller){
        this.parents[smaller] = larger;
        this.ranks[larger] += this.ranks[smaller];
    }

    int find(int x){
        while(x isnt this.parents[x]){
            x = this.parents[x];
        }
        return x;
    }

    void rollback(int parent,int child){
        this.parents[child] = child;
        this.ranks[parent] -= this.ranks[child];
    }
    ...
    DSU(int n){
        this.parents = vi(n,0);
        this.ranks = vi(n,1);
        iota(all(this.parents),0);
    }

    void rollback(int x){
        int parx = this.find(x);
        if(parx isnt x){
            this.rollback(parx,x);
        }
    }
    ...
    int findSize(int x){
        return this.ranks[this.parents[x]];
    }
};
void testcase();
bool verify(vvi &arr,vi &rows,vi &cols);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    if(verify(arr,rows,cols) is false){
        see(0);
    } else {
        vvb isCnd(h,vb(w,true));
        inc(i,0,h){
            isCnd[i][0] = false;
        }
        inc(j,0,w){
        ...
        inc(i,1,h){
            DSU dsu(arr[i].sz());
            inc(j,1,arr[i].sz()){
                if(arr[i][j] is 1 and arr[i][j - 1] is 1){
                    dsu.findUnion(j,j - 1);
                }
                if(arr[i][j] is 1){
                    isCnd[i][j] = false;
            ...
            inc(j,1,arr[i].sz()){
                if(arr[i][j] is 0){
                    int p = dsu.findSize(0);
                    if(arr[i][j - 1] is 1){
                        dsu.findUnion(j,j - 1);
                    }
                    if(dsu.findSize(0) isnt p){
                        isCnd[i][j] = false;
                    }
                    dsu.rollback(j);
                }
        ...
        inc(j,1,arr[0].sz()){
            DSU dsu(arr.sz());
            inc(i,1,arr.sz()){
                if(arr[i][j] is 1 and arr[i - 1][j] is 1){
                    dsu.findUnion(i,i - 1);
                }
            }
            inc(i,1,arr.sz()){
...
bool verify(vvi &arr,vi &rows,vi &cols){
    vi r(rows.sz(),0),c(cols.sz(),0);
    inc(i,0,arr.sz()){
        int start = 0;
        while(start < arr[0].sz() and arr[i][start] is 1){
            r[i]++;
            start++;
        }
    }
    inc(j,0,arr[0].sz()){
        int start = 0;
        while(start < arr.sz() and arr[start][j] is 1){
            c[j]++;
            start++;
        }
    }
    return r is rows and c is cols;
}
