...
void testcase(){
    ...
    vll arr(n);
    readArray(arr);
    ll start = 0,end = linf,ans = linf;
    while(start <= end){
        ll mid = (start + end) >> 1ll;
        vll curr = arr,waiting(n,0);
        curr.back() = max(curr.back() - mid,0ll);
        dec(i,n - 2,0){
            if(curr[i + 1] >= curr[i]){
                break;
            }
            ll wait = 0;
            if(arr[i] <= arr[i + 1]){
                wait = waiting[i + 1] + arr[i + 1] - arr[i] + 1;
            } else {
                wait = max(0ll,waiting[i + 1] - (arr[i] - arr[i + 1]) + 1);
            }
            waiting[i] = wait;
            ll reduction = max(0ll,mid - wait);
            curr[i] = max(curr[i] - reduction,0ll);
        }
        // see("**** " << mid << " ****");
        // writeArray(curr);
        // writeArray(waiting);
        if(*max_element(all(curr)) is 0){
            ans = mid;
            end = mid - 1;
        } else {
            start = mid + 1;
