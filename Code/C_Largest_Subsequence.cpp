...
void testcase(){
    ...
    vt(pair(char,int))stack;
    inc(i,0,n){
        while(stack.empty() is false and s[i] > stack.back().fi){
            stack.rb();
        }
        stack.pb(mp(s[i],i));
    }
    ...
    each(value,stack){
        indices.pb(value.sc);
        characters.pb(value.fi);
    }
    int end = 0;
    while(end < characters.sz() and characters[end] is characters[0]){
        end++;
    }
    sorted(characters);
    sorted(indices);
    inc(i,0,characters.sz()){
        s[indices[i]] = characters[i];
    }
    if(isSorted(s)){
        see(characters.sz() - end);
    } else {
        see(-1);
    }
    return;
