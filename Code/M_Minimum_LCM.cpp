...
void testcase(){
    ...
    while(factor * factor <= n){
        if(mod(n,factor) is 0){
            factors.pb(factor);
            if(n / factor isnt factors.back()){
                factors.pb(n / factor);
            }
        }
        factor++;
    }
    ll curr = lcm(1ll,n - 1),a = 1,b = n - 1;
    each(value,factors){
        ll cnd1 = min(value,n - value);
        ll cnd2 = max(n - value,value);
        if(mod(cnd2,cnd1) is 0 and lcm(cnd1,cnd2) < curr){
            curr = lcm(cnd1,cnd2);
            a = cnd1;
            b = cnd2;
        }
