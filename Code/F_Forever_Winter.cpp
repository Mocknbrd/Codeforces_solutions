...
const ll linf = 2e18;
void testcase();
bool check(vi &degree,int x,int y);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    int total = cnt(degree,1);
    int factor = 2;
    while(factor * factor <= total){
        if(mod(total,factor) is 0){
            if(check(degree,factor,total / factor) is true){
                see(factor << " " << total / factor);
                return;
            }
            if(check(degree,total / factor,factor) is true){
                see(total / factor << " " << factor);
                return;
            }
        }
        factor++;
...
bool check(vi &degree,int x,int y){
    ...
    inc(vertex,1,degree.sz()){
        first += (degree[vertex] is x);
        second += (degree[vertex] is y + 1);
        third += (degree[vertex] is 1);
    }
    if(first is second){
        return first is x + 1 and third is x * y;
    } else {
        return first is 1 and second is x and third is x * y;
    }
}
