...
const int inf = 2e9;
const ll linf = 2e18;
ll dfs(string &curr,vvll &dp,int ind,bool under);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ll k;
    cin >> k;
    ll start = 1,end = 1e15,ans = k;
    while(start <= end){
        ll curr = (start + end) >> 1ll;
        string n = to_string(curr);
        vvll dp(n.sz(),vll(2,-1));
        ll pos = dfs(n,dp,0,false) - 1;
        if(pos >= k){
            ans = curr;
            end = curr - 1;
        } else {
...
ll dfs(string &curr,vvll &dp,int ind,bool under){
    if(ind is curr.sz()){
        return 1;
    } elif(dp[ind][under] isnt -1){
        return dp[ind][under];
    } else {
        int digit = curr[ind] - '0';
        int end = (under is true ? 10 : digit + 1);
        ll ans = 0;
        inc(nxt,0,end){
            bool status = (under or nxt < digit);
            if(nxt isnt 4){
                ans += dfs(curr,dp,ind + 1,status);
            }
        }
        return dp[ind][under] = ans;
    }
}
