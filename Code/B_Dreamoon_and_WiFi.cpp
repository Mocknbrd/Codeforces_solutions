...
const ll linf = 2e18;
void testcase();
void backtrack(string &s2,int &ans,int index,int curr,int target);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    string s1,s2;
    cin >> s1 >> s2;
    int target = count(all(s1),'+') - count(all(s1),'-');
    int ans = 0;
    backtrack(s2,ans,0,0,target);
    ld den = (ld)power(2,(int)count(all(s2),'?'),inf);
    see(setprecision(30) << (ld)ans / den);
}
void backtrack(string &s2,int &ans,int index,int curr,int target){
    if(index is s2.sz()){
        ans += (curr is target);
        ...
        if(s2[index] is '+'){
            backtrack(s2,ans,index + 1,curr + 1,target);
        } elif(s2[index] is '-'){
            backtrack(s2,ans,index + 1,curr - 1,target);
        } else {
            backtrack(s2,ans,index + 1,curr + 1,target);
