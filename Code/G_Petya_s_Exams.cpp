...
void testcase(){
    int n,m;
    cin >> n >> m;
    min_heap(pair(pii,pii))pq;
    vi ans(n,-1);
    inc(i,0,m){
        int start,end,days;
        cin >> start >> end >> days;
        pq.push(mp(mp(end - 1,start - 1),mp(i + 1,days)));
        ans[end - 1] = m + 1;
    }
    while(pq.empty() is false){
        pair(pii,pii)curr = pq.top(); pq.pop();
        int end = curr.fi.fi,start = curr.fi.sc,index = curr.sc.fi,days = curr.sc.sc;
        inc(i,start,end){
            if(days is 0){
                break;
            } elif(ans[i] is -1){
