...
void testcase(){
    ...
    cin >> a >> b >> l;
    set(ll)values;
    ll xb = ceil<ll>(log2<ll>(l),log2<ll>(a));
    ll yb = ceil<ll>(log2<ll>(l),log2<ll>(b));
    inc_type(ll,x,0,xb + 1){
        inc_type(ll,y,0,yb + 1){
            ll first = power(a,x);
            ll second = power(b,y);
            if(mod(l,first * second) is 0){
                values.ins(l / (first * second));
            }
        }
    }
    see(values.sz());
