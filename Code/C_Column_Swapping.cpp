...
void testcase(){
    ...
    if(m == 1){
        ...
        vi diffpos = diff(matrix[0]);
        if(diffpos.sz() > 2){
            see(-1);
        } else {
            int left = diffpos.sz() ? diffpos.front() : -1,right = diffpos.sz() ? diffpos.back() : -1;
            inc(r,1,n){
                vi curr = diff(matrix[r]);
                if(curr.sz() > 2){
                    see(-1);
                    return;
                } else {
                    if(curr.sz() == 2){
                        if(left == -1 or (left == curr[0] and right == curr[1])){
                            left = curr[0];
                            right = curr[1];
                        } else if(curr[0] == -1){
                            continue;
            ...
            inc(i,0,n){
                swap(matrix[i][left - 1],matrix[i][right - 1]);
...
vi diff(vi &row){
    ...
    vi sortedRow = row;
    sorted(sortedRow);
    if(sortedRow.front() == sortedRow.back()){
        return {-1,-1};
    } else {
        inc(i,0,row.sz()){
            if(sortedRow[i] != row[i]){
                ans.pb(i + 1);
            }
        }
        return ans;
