...
void testcase(){
    ...
    while(q--){
        ...
        int start = l,end = n - 1;
        while(end - start > 2){
            int mid1 = start + (end - start + 1) / 3;
            int mid2 = end - (end - start + 1) / 3;
            ll sum1 = prefix[mid1 + 1] - prefix[l];
            ll sum2 = prefix[mid2 + 1] - prefix[l];
            ll first = sum1 * u - sum1 * (sum1 - 1) / 2;
            ll second = sum2 * u - sum2 * (sum2 - 1) / 2;
            if(second > first){
                start = mid1 + 1;
            } else {
                end = mid2 - 1;
        ...
        int ans = l + 1;
        inc(i,start,end + 1){
            ll sum = prefix[i + 1] - prefix[l];
            ll cnd = sum * u - sum * (sum - 1) / 2;
            if(cnd > maxi){
                ans = i + 1;
                maxi = cnd;
            }
