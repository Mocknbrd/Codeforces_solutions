...
const ll linf = 2e18;
void testcase();
bool solve(vt(string)&arr,vvi &dp,int r,int c);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
bool solve(vt(string)&arr,vvi &dp,int r,int c){
    if(r is 1 and c is arr[0].sz() - 1){
        return dp[r][c] = true;
    } elif(dp[r][c] isnt -1){
        return dp[r][c];
    } else {
        if(r + 1 < 2 and arr[r + 1][c] isnt '<'){
            if(solve(arr,dp,r + 1,c + 1) is true){
                return dp[r][c] = true;
            }
        } elif(r + 1 is 1 and c is arr[0].sz() - 1){
            return dp[r][c] = true;
        }
        if(r - 1 >= 0 and arr[r - 1][c] isnt '<'){
            if(solve(arr,dp,r - 1,c + 1) is true){
                return dp[r][c] = true;
            }
        }
        if(c + 1 < arr[0].sz() and arr[r][c + 1] isnt '<'){
            if(solve(arr,dp,r,c + 2) is true){
                return dp[r][c] = true;
            }
        } elif(r is 1 and c + 1 is arr[0].sz()){
            return dp[r][c] = true;
        }
        return dp[r][c] = false;
    }
