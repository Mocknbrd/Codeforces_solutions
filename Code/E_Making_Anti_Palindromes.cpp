...
void testcase(){
    ...
    if(mod(n,2) is 1){
        ...
        vi characterCnts(26,0),palindrome(26,0);
        int start = 0,end = n - 1,same = 0;
        while(start < end){
            same += s[start] is s[end];
            characterCnts[char_index(s[start])]++;
            characterCnts[char_index(s[end])]++;
            palindrome[char_index(s[start])] += s[start] is s[end];
            start++;
            end--;
        }
        int maxi = *max_element(all(palindrome));
        if(*max_element(all(characterCnts)) > n / 2){
            see(-1);
        } else {
            see(max(maxi,ceil(same,2)));
        }
    }
    return;
}
