...
void testcase(){
    ...
    vt(string)arr(n);
    readArray(arr);
    vvb exitable(n,vb(m,true));
    vi notExitable(m,0);
    inc(i,1,n){
        inc(j,1,m){
            if((arr[i - 1][j] is 'X' or exitable[i - 1][j] is false) and (arr[i][j - 1] is 'X' or exitable[i][j - 1] is false)){
                exitable[i][j] = false;
                notExitable[j]++;
            }
        }
    ...
    while(q--){
        ...
        cin >> s >> e;
        s--,e--;
        if(notExitable[e] - notExitable[s] isnt 0){
            NN;
        } else {
            YY;
        }
