...
const ll linf = 2e18;
void testcase();
char findXor(char first,char second);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    string s,t;
    cin >> n >> s >> t;
    if(s[0] isnt t[0] or s.bk() isnt t.bk()){
        see(-1);
    } else {
        string sbar,tbar;
        inc_la(i,0,n,1){
            sbar.pb(findXor(s[i],s[i + 1]));
            tbar.pb(findXor(t[i],t[i + 1]));
        }
        vll first,second;
        inc(i,0,sbar.sz()){
            if(sbar[i] is '1'){
        ...
        if(first.sz() isnt second.sz()){
            see(-1);
        } else {
            ll ans = 0;
            while(first.empty() is false){
                ans += abs(first.bk() - second.bk());
                first.rb();
                second.rb();
            }
            see(ans);
