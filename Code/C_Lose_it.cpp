...
const int inf = 2e9;
const ll linf = 2e18;
bool mark(vi &arr,vvi &pos,int posind,int index);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    if(vals.sz() < 6){
        ...
        while(pos[0].empty() is false){
            if(mark(arr,pos,1,pos[0].bk()) is false){
                pos[0].rb();
            }
        }
        int ans = 0;
...
bool mark(vi &arr,vvi &pos,int posind,int index){
    if(posind is pos.sz()){
        ...
        while(pos[posind].empty() is false and pos[posind].bk() < index){
            pos[posind].rb();
        }
        if(pos[posind].empty() is true){
            return false;
        } else {
            bool ans = mark(arr,pos,posind + 1,pos[posind].bk());
            if(ans is true){
                pos[posind - 1].rb();
                arr[index] = -inf;
            } 
