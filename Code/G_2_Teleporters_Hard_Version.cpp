...
void testcase(){
    ll n,c;
    cin >> n >> c;
    min_heap(pll)left,right;
    inc_type(ll,i,0,n){
        ll value;
        cin >> value;
        debug(i << " " << value << " " << value + i + 1 << " " << value + n - i);
        left.push(mp(value + i + 1,i));
        right.push(mp(value + n - i,i));
    }
    vb seen(n,false);
    ll sum = left.top().fi;
    ll ans = (sum <= c);
    seen[left.top().sc] = true;
    left.pop();
    while(sum <= c){
        while(left.empty() is false and seen[left.top().sc] is true){
            left.pop();
        }
        while(right.empty() is false and seen[right.top().sc] is true){
            right.pop();
        }
        if(left.empty() is true and right.empty() is true){
            break;
        } else {
            if(right.empty() is true or left.top().fi <= right.top().fi){
                sum += left.top().fi;
                seen[left.top().sc] = true;
                ans += (sum <= c);
            } else {
                sum += right.top().fi;
                seen[right.top().sc] = true;
                ans += (sum <= c);
            }
        }
    }
