...
void testcase(){
    int n;
    cin >> n;
    vi suffixmn(n + 2,inf),suffixmx(n + 2,-inf),prefixmn = suffixmn,prefixmx = suffixmx;
    prefixmx[1] = 1,prefixmn[1] = 0,suffixmx[1] = 1,suffixmn[1] = 0;
    int curr = 2;
    inc(i,0,n){
        char type;
        cin >> type;
        if(type is '+'){
            int parent,weight;
            cin >> parent >> weight;
            suffixmx[curr] = max(0,suffixmx[parent] + weight);
            suffixmn[curr] = min(0,suffixmn[parent] + weight);
            prefixmx[curr] = max(prefixmx[parent],suffixmx[curr]);
            prefixmn[curr] = min(prefixmn[parent],suffixmn[curr]);
            curr++;
        } else {
            int _,end,weight;
            cin >> _ >> end >> weight;
            if(inBetween(prefixmn[end],weight,prefixmx[end]) is true){
                YY;
            } else {
                NN;
            }
