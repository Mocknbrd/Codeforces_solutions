...
void testcase(){
    ...
    inc(index,0,20){
        ...
        if(a is b){
            break;
        } elif(diff[index].empty() is false){
            char replace = 'z';
            each(position,diff[index]){
                if(a[position] isnt b[position]){
                    replace = min(replace,b[position]);
                }
            }
            if(replace <= curr){
                see(-1);
                ...
                ans += (replace isnt 'z');
                while(diff[index].empty() is false){
                    int pos = diff[index].back(); diff[index].rb();
                    if(a[pos] isnt b[pos]){
                        a[pos] = replace;
                        diff[replace - 'a'].pb(pos);
                    }
                }
            }
        }
