...
void testcase(){
    ...
    while(q--){
        ...
        if(x * x - 4ll * y < 0){
            ...
            ll ans = 0;
            set<pii>cnds;
            if(root * root is disc){
                if((x + root) % 2 is 0){
                    ll first = (x + root) / 2ll;
                    ll second = (y % first isnt 0) ? inf : y / first;
                    cnds.insert({min(first,second),max(first,second)});
                }
                if(x >= root and (x - root) % 2 is 0){
                    ll first = (x - root) / 2ll;
                    ll second = (y % first isnt 0) ? linf : y / first;
                    cnds.insert({min(first,second),max(first,second)});
                }
            ...
            each(cnd,cnds){
                if(cnd.f isnt cnd.s){
                    ans +=  counts[cnd.f] * counts[cnd.s];
                } else {
                    ans += counts[cnd.f] * (counts[cnd.f] - 1ll) / 2ll;
                }
            }
            cout << ans << " ";
        }
