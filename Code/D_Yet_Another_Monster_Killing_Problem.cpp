...
void testcase(){
    ...
    SegmentTree pTree(arr),dTree(s);
    int index = 0,ans = 0;
    while(index < arr.sz()){
        int start = index,end = arr.sz() - 1,next = -1;
        while(start <= end){
            int mid = (start + end) >> 1;
            int maxMonster = pTree.query(index,mid);
            int begin = lower_bound(all(p),maxMonster) - p.begin();
            if(begin >= p.sz()){
                see(-1);
                return;
            } else {
                int maxDur = dTree.query(begin,s.sz() - 1);
                if(maxDur >= mid - index + 1){
                    next = mid;
                    start = mid + 1;
                } else {
                    end = mid - 1;
