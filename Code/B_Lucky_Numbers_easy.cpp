...
const ll linf = 2e18;
void testcase();
void backtrack(string &n,string &curr,string &ans,int four,int seven);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    string ans(15,'7');
    string curr = "";
    int target = ceil((int)n.sz(),2);
    backtrack(n,curr,ans,target,target);
    curr = "";
    backtrack(n,curr,ans,target + 1,target + 1);
    see(ans);
    return;
}
void backtrack(string &n,string &curr,string &ans,int four,int seven){
    if(four is 0 and seven is 0){
        if(stoll(curr) >= stoll(n)){
            ans = (stoll(curr) < stoll(ans) ? curr : ans);
        }
    } else {
        if(four isnt 0){
            curr.pb('4');
            backtrack(n,curr,ans,four - 1,seven);
            curr.rb();
        }
        if(seven isnt 0){
            curr.pb('7');
            backtrack(n,curr,ans,four,seven - 1);
            curr.rb();
        }
    }
}
