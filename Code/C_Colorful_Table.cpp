...
class SegmentTree {
    ...
    Node* __build(vi &arr,int start,int end){
        if(start is end){
            ...
            Node *node = new Node(start,end,max(left->maxi,right->maxi),min(left->mini,right->mini));
            node->left = left;
            node->right = right;
            return node;
        }
    }
    int __findMin(Node *node,int start,int end){
        if(!node or start > node->end or end < node->start){
            return inf;
        } elif(node->start >= start and node->end <= end){
            return node->mini;
        } else {
            return min(self.__findMin(node->left,start,end),self.__findMin(node->right,start,end));
        }
    }
    int __findMax(Node *node,int start,int end){
        if(!node or start > node->end or end < node->start){
            return -inf;
        } elif(node->start >= start and node->end <= end){
            return node->maxi;
        } else {
            return max(self.__findMax(node->left,start,end),self.__findMax(node->right,start,end));
        }
    }
    public:
    SegmentTree(vi &arr){
    ...
    int findMin(int start,int end){
        return self.__findMin(self.root,start,end);
    }
    int findMax(int start,int end){
        return self.__findMax(self.root,start,end);
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    SegmentTree tree(indices);
    inc(color,1,k + 1){
        if(cols.count(color) is false){
            cout << 0 << " ";
        } else {
            int start = 0,end = n - 1,pos = n - 1;
            while(start <= end){
                int mid = (start + end) >> 1;
                if(colors[mid]  is color){
                    pos = mid;
                    end = mid - 1;
            ...
            int l = tree.findMin(pos,n - 1);
            int r = tree.findMax(pos,n - 1);
            cout << 2 * (r - l + 1) << " ";
        }
    }
    br();
