...
class DSU {
    ...
    int find(int vertex){
        if(vertex is self.parent[vertex]){
            return vertex;
        } else {
            return self.parent[vertex] = self.find(self.parent[vertex]);
        }
    }
...
void testcase(){
    ...
    map(int,pii)components;
    inc(vertex,1,n + 1){
        int parent = dsu.find(vertex);
        if(components.count(parent) is false){
            components[parent] = make_pair(vertex,vertex);
        } else {
            components[parent].sc = vertex;
        }
    }
    vpii comps;
    each(entry,components){
        comps.pb(entry.sc);
    }
    sorted(comps);
    int ans = 0,end = 0;
