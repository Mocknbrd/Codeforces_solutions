...
void testcase(){
    ...
    inc(i,0,k){
        ...
        arr[i].fi = min(u,v);
        arr[i].sc = max(u,v);
        vertices.erase(arr[i].fi);
        vertices.erase(arr[i].sc);
    }
    inc(vertex,1,2 * n + 1){
        if(vertices.count(vertex) is true){
            int res = vertex,total = 0;
            inc(partner,vertex + 1,2 * n + 1){
                if(vertices.count(partner) is true){
                    pii point = mp(vertex,partner);
                    int cnt = 0;
                    each(cnd,arr){
                        if(point.fi > cnd.fi and point.fi < cnd.sc and point.sc > cnd.sc){
                            cnt++;
                        } elif((point.fi < cnd.sc or point.fi > cnd.sc) and point.sc > cnd.sc){
                            cnt++;
                        }
                    }
                    if(cnt > total){
    ...
    each(curr,arr){
        each(cnd,arr){
            if(curr isnt cnd){
                if(curr.fi > cnd.fi and curr.fi < cnd.sc and curr.sc > cnd.sc){
                    ans++;
                } elif((curr.fi < cnd.sc or curr.fi > cnd.sc) and curr.sc > cnd.sc){
                    ans++;
                }
            }
        }
