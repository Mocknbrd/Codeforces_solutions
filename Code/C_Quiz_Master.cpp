...
const ll linf = 2e18;
void testcase();
void updateMap(map(int,int)&counts,int number,int upper);
void removeFromMap(map(int,int)&counts,int number);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    readArray(arr);
    sorted(arr);
    int ans = inf,start = 0,end = 0;
    map(int,int)counts;
    while(end < n){
        while(end < n and counts.sz() < m){
            updateMap(counts,arr[end],m);
            end++;
        }
        while(start < end and counts.sz() is m){
            removeFromMap(counts,arr[start]);
            start++;
        }
        if(start is 0){
            see(-1);
        ...
        ans = min(ans,arr[end - 1] - arr[start - 1]);
    }
    see(ans);
    return;
}
void updateMap(map(int,int)&counts,int number,int upper){
    ...
    while(prime * prime <= number){
        if(mod(number,prime) is 0 and prime <= upper){
            counts[prime]++;
            if(number / prime <= upper and prime isnt number / prime){
                counts[number / prime]++;
            }
        }
        prime++;
...
void removeFromMap(map(int,int)&counts,int number){
    int prime = 1;
    while(prime * prime <= number){
        if(mod(number,prime) is 0){
            if(counts.count(prime) and --counts[prime] is 0){
                counts.erase(prime);
            }
            if(counts.count(number / prime) and prime isnt number / prime and --counts[number / prime] is 0){
                counts.erase(number / prime);
            }
        }
        prime++;
    }
