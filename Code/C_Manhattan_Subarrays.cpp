...
void testcase(){
    ...
    int ans = 0;
    inc(length,1,5){
        inc_la(start,0,n,length - 1){
            bool cnd = false;
            inc(i,start,start + length){
                inc(j,i + 1,start + length){
                    inc(k,j + 1,start + length){
                        cnd = cnd or ((arr[i] >= arr[j] and arr[j] >= arr[k]) or (arr[i] <= arr[j] and arr[j] <= arr[k]));
                    }
                }
            }
            ans += (cnd is false);
