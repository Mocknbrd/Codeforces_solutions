...
const ll linf = 2e18;
void testcase();
ll solve(vvll &arr,vvll &dp,int r,int c);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    do {
        ...
        ans = max(ans,solve(tmp,dp,0,0));
    } while(next_permutation(all(indices)));
    see(ans);
    return;
}
ll solve(vvll &arr,vvll &dp,int r,int c){
    if(r >= arr.sz() or c >= arr[0].sz()){
        return 0;
    } elif(dp[r][c] isnt -1){
        return dp[r][c];
    } else {
        ll ans1 = solve(arr,dp,r,c + 1);
        ll ans2 = arr[r][c] + solve(arr,dp,r + 1,c + 1);
