...
void testcase(){
    ...
    each(a,grpx){
        ...
        prefix.pb(0);
        inc(i,0,a.sz()){
            prefix.pb(prefix.back() + a[i]);
        }
        inc(i,0,a.sz()){
            ll leftLen = i,rightLen = a.sz() - i;
            ll left = prefix[i],right = prefix.back() - prefix[i];
            ans += right - left + (leftLen - rightLen) * a[i];
        }
    }
    each(a,grpy){
        sorted(a);
