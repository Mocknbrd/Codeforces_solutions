...
void testcase(){
    ...
    each(character,s){
        horizontal += (character is 'N') - (character is 'S');
        vertical += (character is 'E') - (character is 'W');
    }
    if(mod(horizontal,2) is 1 or mod(vertical,2) is 1 or (horizontal is 0 and vertical is 0 and s.sz() is 2)){
        NN;
    } elif(horizontal is 0 and vertical is 0){
        string ans = s;
        ans[0] = 'H';
        inc(i,1,n){
            if((s[i] is 'N' and s[0] is 'S') or (s[i] is 'S' and s[0] is 'N') or (s[i] is 'E' and s[0] is 'W') or (s[i] is 'W' and s[0] is 'E')){
                ans[i] = 'H';
                break;
            }
        }
        each(character,ans){
            character = (character isnt 'H' ? 'R' : 'H');
        }
        see(ans);
    } else {
        string ans = s;
        int horNum = abs(horizontal) >> 1;
        int verNum = abs(vertical) >> 1;
        char target = (horizontal < 0 ? 'S' : 'N');
        inc(i,0,n){
            if(s[i] is target and horNum > 0){
                ans[i] = 'R';
                horNum--;
            }
        }
        target = (vertical < 0 ? 'W' : 'E');
        inc(i,0,n){
            if(s[i] is target and verNum > 0){
                ans[i] = 'R';
                verNum--;
            }
        }
        each(character,ans){
            character = (character isnt 'R' ? 'H' : 'R');
        }
        see(ans);
    }
    return;
