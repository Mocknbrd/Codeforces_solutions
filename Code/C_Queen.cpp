...
const ll linf = 2e18;
void testcase();
bool dfs(vvi &tree,vi &respect,vi &ans,int vertex);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    inc(vertex,1,n + 1){
        int parent;
        cin >> parent >> respect[vertex];
        root = (parent is -1 ? vertex : root);
        if(parent isnt -1){
            tree[parent].pb(vertex);
        }
    }
    vi ans;
    dfs(tree,respect,ans,root);
    sorted(ans);
    if(ans.empty() is true){
        see(-1);
    } else {
        writeArray(ans);
    }
...
bool dfs(vvi &tree,vi &respect,vi &ans,int vertex){
    bool flag = (respect[vertex] is 1);
    each(child,tree[vertex]){
        dfs(tree,respect,ans,child);
        if(respect[child] is 0){
            flag = false;
        }
    }
    if(flag is true){
