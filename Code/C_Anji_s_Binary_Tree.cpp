...
const int inf = 2e9;
const ll linf = 2e18;
void dfs(vpii &tree,string &s,vi &ans,int vertex,int changes);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    vi ans(n,inf);
    dfs(tree,s,ans,0,0);
    see(*min_element(all(ans)));
    return;
}
void dfs(vpii &tree,string &s,vi &ans,int vertex,int changes){
    if(tree[vertex].fi is -1 and tree[vertex].sc is -1){
        ans[vertex] = changes;
    } else {
        if(tree[vertex].fi isnt -1){
            dfs(tree,s,ans,tree[vertex].fi,changes + (s[vertex] isnt 'L'));
        }
        if(tree[vertex].sc isnt -1){
            dfs(tree,s,ans,tree[vertex].sc,changes + (s[vertex] isnt 'R'));
        }
    }
}
