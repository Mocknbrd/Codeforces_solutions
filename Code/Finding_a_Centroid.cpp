...
const ll linf = 8e18;
void testcase();
int findSubTreeSize(vvi &tree,vi &subtree,int vertex,int parent);
int findCentroid(vvi &tree,vi &subtree,int vertex,int parent);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    vi subtree(tree.sz(),0);
    findSubTreeSize(tree,subtree,1,0);
    see(findCentroid(tree,subtree,1,0));
    return;
}
int findSubTreeSize(vvi &tree,vi &subtree,int vertex,int parent){
    subtree[vertex] = 1;
    each(child,tree[vertex]){
        if(child isnt parent){
            subtree[vertex] += findSubTreeSize(tree,subtree,child,vertex);
        }
    }
...
int findCentroid(vvi &tree,vi &subtree,int vertex,int parent){
    each(child,tree[vertex]){
        if(subtree[child] * 2 > tree.sz() - 1 and child isnt parent){
            return findCentroid(tree,subtree,child,vertex);
        }
    }
    return vertex;
}
