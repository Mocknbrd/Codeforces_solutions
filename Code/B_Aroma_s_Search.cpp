...
void testcase(){
    ...
    while(points.back().fr < ceil(linf,ax) and points.back().sc < ceil(linf,ay)){
        ll xn = ax * points.back().fr + bx;
        ll yn = ay * points.back().sc + by;
        points.pb(make_pair(xn,yn));
    }
    vll prefix;
    prefix.pb(0);
    inc(i,1,points.sz()){
        prefix.pb(prefix.back() + manhattanDist(points[i],points[i - 1]));
    }
    int ans = 0;
    pll origin = make_pair(xs,ys);
    inc(i,0,points.sz()){
        if(manhattanDist(points[i],origin) <= t){
            ll landing = manhattanDist(points[i],origin);
            int start = i,end = points.sz() - 1;
            while(start <= end){
                int mid = (start + end) >> 1;
                if(prefix[mid] - prefix[i] <= t - landing){
                    ans = max(ans,mid - i + 1);
                    start = mid + 1;
                } else {
                    end = mid - 1;
    ...
    dec(i,points.sz() - 1,0){
        if(manhattanDist(origin,points[i]) <= t){
            ll landing = manhattanDist(points[i],origin);
            int start = 0,end = i;
            while(start <= end){
                int mid = (start + end) >> 1;
                if(prefix[i] - prefix[mid] <= t - landing){
                    ans = max(ans,i - mid + 1);
                    end = mid - 1;
                } else {
                    start = mid + 1;
