...
void testcase(){
    ...
    vi ones(n + m - 1,0),zeros(n + m - 1,0);
    inc(i,0,n){
        inc(j,0,m){
            ones[distance[i][j]] += arr[i][j] is 1;
            zeros[distance[i][j]] += arr[i][j] is 0;
        }
    }
    int start = 0,end = ones.sz() - 1,ans = 0;
    while(start < end){
        int one = zeros[start] + zeros[end];
        int zero = ones[start++] + ones[end--];
        ans += min(one,zero);
    }
    see(ans);
    return;
