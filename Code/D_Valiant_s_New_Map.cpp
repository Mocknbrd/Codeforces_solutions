...
class Node {
    ...
    int maxi;
    Node *left,*right;
    Node(pii topLeft,pii bottomRight,int maxi){
...
class TwoDimensionalSegmentTree {
    private:  
    Node *root;
    Node* build(vvi &arr,pii topLeft,pii bottomRight){
        Node *node;
        if(topLeft.fi is bottomRight.fi and topLeft.sc is bottomRight.sc){
            node = new Node(topLeft,bottomRight,arr[topLeft.fi][topLeft.sc]);
        } elif(topLeft.sc is bottomRight.sc){
            int mid = (topLeft.fi + bottomRight.fi) >> 1;
            Node *left = this.build(arr,topLeft,mp(mid,bottomRight.sc));
            Node *right = this.build(arr,mp(mid + 1,topLeft.sc),bottomRight);
            node = new Node(topLeft,bottomRight,min(left->maxi,right->maxi));
            node->left = left;
            node->right = right;
        } else {
            int mid = (topLeft.sc + bottomRight.sc) >> 1;
            Node *left = this.build(arr,topLeft,mp(bottomRight.fi,mid));
            Node *right = this.build(arr,mp(topLeft.fi,mid + 1),bottomRight);
            node = new Node(topLeft,bottomRight,min(left->maxi,right->maxi));
            node->left = left;
            node->right = right;
        }
    ...
    int query(Node *node,pii topLeft,pii bottomRight){
        if(node is None or (node->bottomRight.sc < topLeft.sc) or (node->bottomRight.fi < topLeft.fi) or (node->topLeft.fi > bottomRight.fi) or (node->topLeft.sc > bottomRight.sc)){
            return inf;
        } elif(inBetween(topLeft.fi,node->topLeft.fi,bottomRight.fi) and inBetween(topLeft.fi,node->bottomRight.fi,bottomRight.fi) and inBetween(topLeft.sc,node->topLeft.sc,bottomRight.sc) and inBetween(topLeft.sc,node->bottomRight.sc,bottomRight.sc)){
            return node->maxi;
        } else {
            int left = this.query(node->left,topLeft,bottomRight);
            int right = this.query(node->right,topLeft,bottomRight);
            return min(left,right);
        }
    }

    public:  
    TwoDimensionalSegmentTree(vvi &arr){
        this.root = this.build(arr,mp(0,0),mp(arr.sz() - 1,arr[0].sz() - 1));
    }
    int query(pii topLeft,pii bottomRight){
        return this.query(this.root,topLeft,bottomRight);
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    vvi arr(n,vi(m));
    readMatrix(arr);
    TwoDimensionalSegmentTree tree(arr);
    int ans = 1;
    inc(i,0,n){
        inc(j,0,m){
            int start = 0,end = min(n,m),curr = 0;
            while(start <= end){
                int mid = (start + end) >> 1;
                if(i + mid > n or j + mid > m){
                    end = mid - 1;
                } else {
                    if(tree.query(mp(i,j),mp(i + mid - 1,j + mid - 1)) >= mid){
                        curr = mid;
                        start = mid + 1;
                    } else {
                        end = mid - 1;
