...
class FenwickTree {
    ...
    void update(int value){
        while(value < this.tree.sz()){
            this.tree[value]++;
            value += this.lsb(value);
        }
    }
    int smaller(int value){
        int curr = value - 1,ans = 0;
        while(curr){
            ans += this.tree[curr];
            curr -= this.lsb(curr);
        }
...
void testcase(){
    int n;
    cin >> n;
    vt(pair(pii,int))greater,smaller;
    inc(i,0,n){
        int first,second;
        cin >> first >> second;
        if(first > second){
            greater.pb(mp(mp(first,second),i));
        } else {
            smaller.pb(mp(mp(first,second),i));
        }
    }
    vi ans1,ans2;
    {
        if(greater.empty() is false){
            FenwickTree tree(n << 1);
            each(element,greater){
                tree.update(element.fi.fi);
            }
            vpii data;
            inc(i,0,greater.sz()){
                data.pb(mp(tree.bigger(greater[i].fi.sc),i));
            }
            rsort(data);
            int prev = -inf;
            each(element,data){
                int index = element.sc;
                if(greater[index].fi.fi > prev){
                    ans1.pb(greater[index].sc + 1);
                    prev = greater[index].fi.sc;
                }
            }
        }
    }
    {
        if(smaller.empty() is false){
            FenwickTree tree(n << 1);
            each(element,smaller){
                tree.update(element.fi.fi);
            }
            vpii data;
            inc(i,0,smaller.sz()){
                data.pb(mp(tree.smaller(smaller[i].fi.sc),i));
            }
            rsort(data);
            int prev = inf;
            each(element,data){
                int index = element.sc;
                if(smaller[index].fi.fi < prev){
                    ans2.pb(smaller[index].sc + 1);
                    prev = smaller[index].fi.sc;
                }
            }
        }
    }
