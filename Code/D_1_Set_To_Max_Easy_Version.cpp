...
class Node {
    public:  
    int start,end,maxi,mini;
    Node *left,*right;
    Node(int start,int end,int maxi,int mini){
        this.start = start;
        this.end = end;
        this.maxi = maxi;
        this.mini = mini;
...
class SegmentTree {
    protected:  
    Node *root;
    virtual int query(Node *node,int start,int end){
        return 0;
    }

    private:  
    Node* build(vi &arr,int start,int end){
        Node *node;
        if(start is end){
            node = new Node(start,end,arr[start],arr[end]);
        } else {
            int mid = (start + end) >> 1;
            Node *left = this.build(arr,start,mid);
            Node *right = this.build(arr,mid + 1,end);
            node = new Node(start,end,max(left->maxi,right->maxi),min(left->mini,right->mini));
            node->left = left;
            node->right = right;
        }
        return node;
    ...
    public:  
    SegmentTree(vi &arr){
        this.root = this.build(arr,0,arr.sz() - 1);
    }
    virtual int query(int start,int end){
        return 0;
    }
};
class MaxSegmentTree: public SegmentTree {
    private: 
    int query(Node *node,int start,int end) override {
        if(!node or start > node->end or end < node->start){
            return -inf;
        } elif(node->start >= start and node->end <= end){
            return node->maxi;
        } else {
            int left = this.query(node->left,start,end);
            int right = this.query(node->right,start,end);
            return max(left,right);
        }
    }

    public:  
    MaxSegmentTree(vi &arr): SegmentTree(arr){}
    int query(int start,int end) override {
        return this.query(this.root,min(start,end),max(start,end));
    }
};
class MinSegmentTree: public SegmentTree {
    ...
    public:  
    MinSegmentTree(vi &arr): SegmentTree(arr){}
    int query(int start,int end) override {
        return this.query(this.root,min(start,end),max(start,end));
    }
};
int justLower(vi &arr,int value){
    int start = 0,end = arr.sz() - 1,ans = -inf;
    while(start <= end){
        int mid = (start + end) >> 1;
        if(arr[mid] < value){
            ans = arr[mid];
...
int justHigher(vi &arr,int value){
    int start = 0,end = arr.sz() - 1,ans = inf;
    while(start <= end){
        int mid = (start + end) >> 1;
        if(arr[mid] > value){
...
void testcase(){
    ...
    MinSegmentTree minTree(b);
    inc(i,0,n){
        if(pos.count(b[i]) is false){
            NN;
            return;
        } else {
            int left = justLower(pos[b[i]],i);
            int right = justHigher(pos[b[i]],i);
            if(left is -inf and right is inf){
                if(a[i] isnt b[i]){
                    NN;
                    return;
                } 
            } elif(left is -inf or right is inf){
                if(left is -inf){
                    if((minTree.query(right,i) isnt b[i] or maxTree.query(right,i) isnt b[i]) and a[i] isnt b[i]){
                        NN;
                        return;
                    }
                } else {
                    if((maxTree.query(left,i) isnt b[i] or minTree.query(left,i) isnt b[i]) and a[i] isnt b[i]){
                        NN;
                        return;
                    }
                }
            } else {
                int small = (abs(i - left) < abs(i - right) ? left : right);
                int large = (small is left ? right : left);
                if(minTree.query(small,i) is b[i] and maxTree.query(small,i) is b[i]){
                    continue;
                } 
                if(maxTree.query(large,i) is b[i] and minTree.query(large,i) is b[i]){
                    continue;
                }
                if(a[i] is b[i]){
                    continue;
