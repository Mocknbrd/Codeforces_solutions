...
void testcase(){
    ...
    readArray(a);
    readArray(b);
    ll total = 0,constant = 0;
    inc(i,0,n){
        total += a[i] + b[i];
        constant += power(a[i],2ll) + power(b[i],2ll);
    }
    constant *= (n - 2);
    vvb dp(n,vb(total + 1,false));
    dp[0][a[0]] = dp[0][b[0]] = true;
    inc(index,1,n){
        inc_type(ll,sum,0,total + 1){
            if(sum >= a[index]){
                dp[index][sum] = (dp[index][sum] or dp[index - 1][sum - a[index]]);
            }
            if(sum >= b[index]){
                dp[index][sum] = (dp[index][sum] or dp[index - 1][sum - b[index]]);
            }
        }
    }
    ll ans = linf;
    inc_type(ll,sum,0,total + 1){
        if(dp.back()[sum] is true){
            ll second = total - sum;
            ans = min(ans,constant + power(sum,2ll) + power(second,2ll));
        }
    }
    see(ans);
    return;
