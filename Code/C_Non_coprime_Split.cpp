...
void testcase(){
    int left,right;
    cin >> left >> right;
    if(mod(right,2) is 0 and right isnt 2){
        see(right / 2 << " " << right / 2);
    } elif(right - left >= 1 and mod(right - 1,2) is 0 and right - 1 isnt 2){
        see((right - 1) / 2 << " " << (right - 1) / 2);
    } else {
        if(right - left >= 1){
            see(-1);
        } else {
            vi primes = factors(right);
            if(primes.empty() is true){
                see(-1);
            } else {
                int ans1 = primes.front();
                int ans2 = right - ans1;
                see(ans1 << " " << ans2);
            }
        }
