...
void testcase(){
    ...
    if(arr.back() is 1){
        ...
        YY;
        vi ans(n,0);
        vpii zeroOne,oneZero;
        inc_la(i,0,n,1){
            if(arr[i] is 0 and arr[i + 1] is 1){
                zeroOne.pb(mp(i,i + 1));
            } elif(arr[i] is 1 and arr[i + 1] is 0){
                oneZero.pb(mp(i,i + 1));
            }
        }
        int parity = 0,end = n - 1;
        while(end >= 0 and (zeroOne.empty() is false or oneZero.empty() is false)){
            if(parity is 0){
                ans[end--] = oneZero.back().sc;
                oneZero.rb();
            } else {
                ans[end--] = zeroOne.back().sc;
                zeroOne.rb();
            }
            parity ^= 1;
        }
