...
const ll linf = 2e18;
void testcase();
bool dfs(vvi &graph,vb &seen,int vertex);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    int ans = m;
    inc(vertex,0,n){
        if(graph[vertex].empty() is false and graph[vertex][0] is vertex){
            ans--;
        } elif(seen[vertex] is false) {
            int curr = vertex;
            bool isCycle = true;
            do {
                seen[curr] = true;
                if(graph[curr].empty() is true){
                    isCycle = true;
                    break;
                }
                curr = graph[curr][0];
            } while(seen[curr] is false);
            ans += isCycle;
        }
    }
    see(ans);
