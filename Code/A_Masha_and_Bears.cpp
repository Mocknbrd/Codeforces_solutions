...
void testcase(){
    int father,mother,child,sasha;
    cin >> father >> mother >> child >> sasha;
    inc(i,father,2 * father + 1){
        inc(j,mother,min(i,2 * mother + 1)){
            inc(k,child,min(j,2 * child + 1)){
                if(sasha <= k and 2 * sasha < j and 2 * sasha >= k){
                    see(i);
                    see(j);
                    see(k);
                    return;
