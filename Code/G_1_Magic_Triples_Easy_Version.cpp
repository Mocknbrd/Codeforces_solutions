...
void testcase(){
    ...
    ll ans = 0,maxi = (*counts.rbegin()).fi;
    each(element,counts){
        ans += element.sc * (element.sc - 1) * (element.sc - 2);
        inc_type(ll,ratio,2,sqrtl(ceil(maxi,element.fi)) + 1){
            if(counts.count(element.fi * ratio) is true and counts.count(element.fi * ratio * ratio) is true){
                ans += element.sc * counts[element.fi * ratio] * counts[element.fi * ratio * ratio];
            }
        }
    }
    see(ans);
