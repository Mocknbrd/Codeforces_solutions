...
void testcase(){
    ...
    inc(i,0,n){
        if(values.sz() is 1){
            int res = mod(arr[i] + *values.bg(),n);
            values.erase(values.bg());
            cout << res << " ";
        } else {
            int left = *values.bg();
            int ans1 = mod(arr[i] + left,n),ans2 = inf,right = left;
            if(values.lower_bound(mod(-arr[i],n)) isnt values.ed()){
                right = *values.lower_bound(mod(-arr[i],n));
                ans2 = mod(arr[i] + right,n);
            }
            cout << min(ans1,ans2) << " ";
            if(ans1 < ans2){
                values.erase(values.find(left));
            } else {
                values.erase(values.find(right));
            }
        }
    }
    return;
