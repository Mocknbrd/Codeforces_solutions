...
class MaxQueue {
    ...
    void push(ll value){
        if(self.q.empty() is true){
            q.push(mp(value,value));
        } else {
            ll mxValue = max(self.q.fr().sc,self.q.bk().sc);
            q.push(mp(value,max(value,mxValue)));
        }
    }
    void pop(){
    ...
    ll findMax(){
        if(self.q.empty()){
            ...
            return max(self.q.fr().sc,self.q.bk().sc);
        }
    }
};
void testcase();
ll dist(ll s,ll t,ll n){
    return mod(t - s,n);
}
int main(){
    ios;
...
void testcase(){
    int n,m;
    cin >> n >> m;
    vll out(n,0),minDist(n,linf);
    inc(i,0,m){
        int start,end;
        cin >> start >> end;
        start--,end--;
        out[start]++;
        minDist[start] = min(minDist[start],dist(start,end,n));
    }
    MaxQueue mq;
    inc(i,0,n){
        ll buffer = -n;
        if(out[i] > 0){
            buffer = dist(0,i,n) + n * (out[i] - 1) + minDist[i];
        }
        mq.push(buffer);
    }
    vll ans(n,0);
