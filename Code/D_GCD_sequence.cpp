...
void testcase(){
    ...
    vi b;
    inc_la(i,0,n,1){
        b.pb(gcd(arr[i + 1],arr[i]));
    }
    vb suffix(b.sz(),false),prefix(b.sz(),false);
    suffix.back() = true,prefix.front() = true;
    dec(i,b.sz() - 2,0){
        suffix[i] = (suffix[i + 1] and (b[i] <= b[i + 1]));
    }
    inc(i,1,b.sz()){
        prefix[i] = (prefix[i - 1] and (b[i] >= b[i - 1]));
    }
    if(prefix[b.sz() - 2] is true or suffix[1] is true){
        YY;
    } else {
        inc_la(i,1,arr.sz(),1){
            int curr = gcd(arr[i - 1],arr[i + 1]);
            if((i - 2 < 0 or prefix[i - 2] is true) and (i + 1 >= suffix.sz() or suffix[i + 1] is true)){
                bool left = (i - 2 < 0 or b[i - 2] <= curr);
                bool right = (i + 1 is suffix.sz() or b[i + 1] >= curr);
                if(left and right){
                    YY;
                    return;
                }
