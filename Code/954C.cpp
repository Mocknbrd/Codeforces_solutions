...
bool isAdjacent(pii pos1,pii pos2);
int main(){
    ios;
    Input(int,n);
    vi path(n);
    inputArray(path);
    set<int>diff;
    inc_look_ahead(int,i,0,path.size(),1){
        diff.insert(abs(path[i]-path[i+1]));
    }
    diff.erase(1);
    if(diff.empty() or (diff.size()==1 and !diff.count(0))){
        int col=(diff.empty())?1:*diff.begin();
        bool isAns=true;
        inc_look_ahead(int,i,0,path.size(),1){
            if(abs(path[i]-path[i+1])==col){
                continue;
            } else {
                int r1=(path[i]-1)/col;
                int r2=(path[i+1]-1)/col;
                if(r1!=r2){
                    isAns=false;
                    break;
                }
