...
int main(){
    ...
    while(t--){
        ...
        inc(int,turn,1,n*m+1){
            set<pii>coord;
            int last=upper_bound(all(serious),turn)-serious.begin();
            inc(int,i,0,last){
                int student=serious[i];
                coord.insert({(turn-student)/m+1,(turn-student)%m+1});
            }
            set<int>rows,cols;
            each(coords,coord){
                rows.insert(coords.first);
                cols.insert(coords.second);
            }
