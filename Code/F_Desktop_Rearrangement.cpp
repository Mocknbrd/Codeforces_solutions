...
class SegmentTree {
    private:  
    Node *root;
    Node* __build(string &s,int start,int end){
        if(start is end){
            Node *node = new Node(start,end,s[start] is '.');
            return node;
        } else {
            int mid = (start + end) >> 1;
            Node *left = self.__build(s,start,mid);
            Node *right = self.__build(s,mid + 1,end);
    ...
    SegmentTree(string &s){
        self.root = self.__build(s,0,s.sz() - 1);
    }
    int query(int start,int end){
        return self.__query(self.root,start,end);
    }
    void update(int pos){
        self.__update(self.root,pos);
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    while(q--){
        ...
        int pos = c * n + r;
        tree.update(pos);
        int end = input.sz() - tree.query(0,input.sz() - 1) - 1;
        see(tree.query(0,end));
    }
    return;
}
