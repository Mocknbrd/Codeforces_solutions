...
const int inf = 2e9;
const ll linf = 2e18;
int findNextLand(string &river,int curr,int m);
int findNextWater(string &river,vi &suffix,int curr,int m);
void testcase();
int main(){
    ios;
    MANY;
...
void testcase(){
    ...
    cin >> n >> m >> k >> river;
    river = "L" + river + "L";
    vi suffix(river.sz(),0);
    dec(i,river.sz() - 1,0){
        if(river[i] is 'W'){
            suffix[i] = min(suffix[i + 1] + 1,inf);
        } elif(river[i] is 'C'){
            suffix[i] = inf;
        }
    }
    int start = 0;
    while(start <= n){
        int land = findNextLand(river,start,m);
        if(land is -1){
            int water = findNextWater(river,suffix,start,m);
            if(water is -1){
                NN;
                return;
            } else {
...
int findNextLand(string &river,int curr,int m){
    int ans = -1;
    inc(step,1,m + 1){
        if(curr + step < river.sz()){
            if(river[curr + step] is 'L'){
                ans = curr + step;
            }
        }
    }
...
int findNextWater(string &river,vi &suffix,int curr,int m){
    ...
    inc(step,1,m + 1){
        if(curr + step < river.sz()){
            if(river[curr + step] is 'W'){
                if(suffix[curr + step] < cost){
                    cost = suffix[curr + step];
                    ans = curr + step; 
                }
            }
        }
