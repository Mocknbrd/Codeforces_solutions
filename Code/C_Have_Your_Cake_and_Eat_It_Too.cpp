...
const ll linf = 2e18;
void testcase();
bool isIntersect(pii &first,pii &second){
    return (first.fi >= second.fi and first.sc <= second.sc) or (second.fi >= first.fi and second.sc <= first.sc);
};
int main(){
    ios;
    MANY;
...
void testcase(){
    int n;
    cin >> n;
    vvll arr(3,vll(n,0)),prefix(3,vll(1,0));
    readMatrix(arr);
    inc(i,0,3){
        inc(j,0,arr[i].sz()){
            prefix[i].pb(prefix[i].back() + arr[i][j]);
        }
    }
    vt(vpii)ranges(3);
    ll target = ceil(prefix[0].back(),3ll);
    inc(i,0,3){
        vpii cnd;
        inc(j,0,n){
            int start = j,end = n - 1,ind = -1;
            while(start <= end){
                int mid = (start + end) >> 1;
                if(prefix[i][mid + 1] - prefix[i][j] >= target){
                    ind = mid;
                    end = mid - 1;
                } else {
                    start = mid + 1;
        ...
        while(start < cnd.sz()){
            pii curr = cnd[start];
            int end = start;
            while(end < cnd.sz() and isIntersect(curr,cnd[end]) is true){
                curr = mp(max(curr.fi,cnd[end].fi),min(curr.sc,cnd[end].sc));
                end++;
            }
            optim.pb(curr);
            start = end;
    ...
    do {
        ...
        cnds.pb(0);
        inc(i,1,cpy.sz()){
            int prev = cpy[i - 1][cnds.back()].sc;
            int start = 0,end = cpy[i].sz() - 1,ind = -1;
            while(start <= end){
                int mid = (start + end) >> 1;
                if(cpy[i][mid].fi > prev){
                    ind = mid;
                    end = mid - 1;
                } else {
                    start = mid + 1;
        ...
        if(cnds.sz() is 3){
            map(int,pii)ans;
            inc(i,0,3){
                ans[inds[i]] = cpy[i][cnds[i]];
            }
            each(element,ans){
                cout << element.sc.fi + 1 << " " << element.sc.sc + 1 << " ";
            }
            br();
            return;
        }
