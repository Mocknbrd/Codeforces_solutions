...
int main(){
    ...
    while(t--){
        ...
        vi arr(n);
        inputArray(arr);
        vi twosWithoutOps(n,0),twosWithOps(n,0);
        int twos=0,ans=0;
        PQ<int>pq;
        inc(i,0,n){
            twosWithoutOps[i]=findTwos(arr[i]);
            twosWithOps[i]=findTwos(arr[i])+findTwos(i+1);
            twos+=twosWithoutOps[i];
            pq.push(twosWithOps[i]-twosWithoutOps[i]);
        }
        while(twos<n and pq.sz()){
            twos+=pq.top(); pq.pop();
            ans++;
        }
        if(twos<n){
