...
const int inf = 2e9;
const ll linf = 2e18;
class OrderedStatisticTree {
    private:  
    vi tree;
    int lsb(int value){
        return value & (-value);
    }
    int _query(int value){
        int ans = 0;
        while(value){
            ans += this.tree[value];
    ...
    public:  
    OrderedStatisticTree(int n){
        this.tree = vi(n + 1);
    }
    void update(int value,int change){
        while(value < this.tree.sz()){
            this.tree[value] += change;
            value += this.lsb(value);
        }
    }
    int find(int n){
        int start = 1,end = this.tree.sz() - 1,ans = end;
        while(start <= end){
            int mid = (start + end) >> 1;
            if(this._query(mid) >= n){
                ans = mid;
                end = mid - 1;
            } else {
                start = mid + 1;
...
void testcase(){
    ...
    int q;
    cin >> q;
    vt(pair(pii,int))queries(q);
    inc(i,0,q){
        cin >> queries[i].fi.fi >> queries[i].fi.sc;
        queries[i].sc = i;
    }
    sorted(queries);
    vi ans(q);
    OrderedStatisticTree tree(n);
    int length = 0;
    each(query,queries){
        while(length < query.fi.fi){
            tree.update(b[length++].sc,1);
        }
        ans[query.sc] = arr[tree.find(query.fi.sc)];
    }
    each(value,ans){
        see(value);
    }
