...
class DSU {
    ...
    int find(int vertex){
        if(vertex is this.parents[vertex]){
            return vertex;
        } else {
            return this.parents[vertex] = this.find(this.parents[vertex]);
        }
    }
...
void dijkstra(vt(vpll)&graph,vt(pair(pll,int))&edges,int vertex,int maxi);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    dijkstra(graph,edges,0,k);
    return;
}
void dijkstra(vt(vpll)&graph,vt(pair(pll,int))&edges,int vertex,int maxi){
    vll distance(graph.sz(),linf);
    min_heap(pair(ll,pll))pq;
    each(neighbour,graph[vertex]){
        pq.push(mp(neighbour.sc,mp(vertex,neighbour.fi)));
    }
    distance[vertex] = 0;
    while(pq.empty() is false){
        pair(ll,pll)curr = pq.top(); pq.pop();
        ll weight = curr.fi,from = curr.sc.fi,to = curr.sc.sc;
        if(distance[from] + weight < distance[to]){
            distance[to] = distance[from] + weight;
            each(neighbour,graph[to]){
                if(neighbour.fi isnt from){
                    pq.push(mp(neighbour.sc,mp(to,neighbour.fi)));
                }
            }
        }
    }
    vll tracker(graph.sz(),inf);
    tracker[vertex] = 0;
    set(int)vertices;
    vertices.ins(vertex);
    each(neighbour,graph[vertex]){
    ...
    while(vertices.sz() < maxi + 1 and pq.empty() is false){
        pair(ll,pll)curr = pq.top(); pq.pop();
        ll weight = curr.fi,from = curr.sc.fi,to = curr.sc.sc;
        if(tracker[from] + weight < tracker[to]){
            tracker[to] = tracker[from] + weight;
            each(neighbour,graph[to]){
                if(neighbour.sc isnt from){
                    pq.push(mp(neighbour.sc,mp(to,neighbour.fi)));
                }
            }
        }
        if(tracker[to] is distance[to]){
            vertices.ins(to);
        }
    }
    min_heap(pll)edgepq;
    inc(i,0,edges.sz()){
        if(vertices.count(edges[i].fi.fi) is true and vertices.count(edges[i].fi.sc) is true){
            edgepq.push(mp(edges[i].sc,i + 1));
        }
    }
    DSU dsu(graph.sz());
    vi ans;
    while(edgepq.empty() is false){
        int index = edgepq.top().sc; edgepq.pop();
        if(dsu.findUnion(edges[index - 1].fi.fi,edges[index - 1].fi.sc) is false){
            ans.pb(index);
        }
    }
    see(ans.sz());
