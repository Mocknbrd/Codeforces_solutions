...
void testcase(){
    ...
    vpii edgeList = getEdges(tree);
    vi dp(n + 1,0);
    vi turns(tree.sz(),-inf);
    turns[1] = -inf;
    dp[1] = 1;
    each(edge,edgeList){
        int src = edge.fi,target = edge.sc,ind = edges[edge];
        turns[target] = ind;
        dp[target] = dp[src] + (turns[src] > turns[target]);
    }
    see(*max_element(all(dp)));
    return;
}
vpii getEdges(vvi &tree){
    vpii ans;
    vb seen(tree.sz(),false);
    seen[1] = true;
    queue<int>q;
    q.push(1);
    while(q.empty() is false){
        ...
        seen[curr] = true;
        each(child,tree[curr]){
            if(seen[child] is false){
                seen[child] = true;
                q.push(child);
                ans.pb(mp(curr,child));
            }
        }
    }
    return ans;
