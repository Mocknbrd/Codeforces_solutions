...
void testcase(){
    ...
    vi arr(n);
    readArray(arr);
    coordinateCompressInplace(arr);
    vi counts(*max_element(all(arr)) + 1,0);
    each(value,arr){
        counts[value]++;
    }
    if(counts.sz() >= 3){
        int target = n >> 1,gold = counts.back(),silver = 0;
        int end = counts.sz() - 2;
        while(end >= 0 and silver <= gold){
            silver += counts[end--];
        }
        if(end >= 0 and gold + silver <= target){
            int bronze = 0;
            while(end >= 0 and gold + silver + bronze + counts[end] <= target){
                bronze += counts[end--];
            }
            if(gold < bronze){
                see(gold << " " << silver << " " << bronze);
                return;
            }
        }
    }
