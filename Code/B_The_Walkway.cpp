...
void testcase(){
    ...
    ll num = 0;
    int start = 1;
    while(start < pos.sz() - 1){
        ll between = (pos[start] - pos[start - 1] - 1) / d;
        ll nearest = pos[start - 1] + between * d;
        int left = start + 1,right = pos.sz() - 1,nxt = left;
        while(left <= right){
            int mid = (left + right) >> 1;
            if(pos[start] + d >= pos[mid]){
                nxt = mid;
                left = mid + 1;
            } else {
                right = mid - 1;
        ...
        // debug(pos[start] << " " << nearest << " " << pos[nxt]);
        num += nxt - start - (pos[nxt] is pos[start] + d);
        start = nxt;
    }
    see(ans - 1 << " " << num);
    return;
