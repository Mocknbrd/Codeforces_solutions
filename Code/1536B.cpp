...
int main(){
    ...
    cin>>t;
    set<string>s;
    inc_type(char,i,'a','z'+1){
        string s1;
        s1.pb(i);
        s.ins(s1);
        inc_type(char,j,'a','z'+1){
            string s2;
            s2+=s1;
            s2.pb(j);
            s.ins(s2);
            inc_type(char,k,'a','z'+1){
                string s3;
                s3+=s2;
                s3.pb(k);
                s.ins(s3);
    ...
    clln=vector<string>(all(s));
    custSort(clln,[](string &a,string &b){
        if(a.sz()!=b.sz()){
            return a.sz()<b.sz();
        } else {
            return a<b;
...
void testCase(set<int>left){
    ...
    inc(buffer,0,3){
        inc_la(i,0,n,buffer){
            string cnd=str.slice(i,i+buffer);
            left.erase(pos[cnd]);
        }
    }
    int index=*left.begin();
    puts(mp[index]);
    return;
}