...
const ll linf = 2e18;
void testcase();
vt(string)split(string s,char id);
pair(string,string)format(pair(string,string)&curr);
bool isValid(pair(string,string)&curr,int hours,int mins);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    string time;
    cin >> hours >> mins >> time;
    vt(string)splittedTime = split(time,':');
    pair(string,string)start = mp(splittedTime[0],splittedTime[1]);
    pair(string,string)curr = start;
    do {
        if(isValid(curr,hours,mins) is true){
            see(curr.fi << ":" << curr.sc);
            return;
        }
        int currHour = stoi(curr.fi),currMin = stoi(curr.sc);
        currMin++;
        currHour += currMin / mins;
        currMin = mod(currMin,mins);
        currHour = mod(currHour,hours);
        curr = mp(to_string(currHour),to_string(currMin));
        curr = format(curr);
    } while(curr != start);
    return;
}
bool isValid(pair(string,string)&curr,int hours,int mins){
    ...
    map['5'] = '2';
    map['8'] = '8';
    string currHour = curr.fi,currMin = curr.sc;
    each(digit,currHour){
        if(map.count(digit) is false){
            return false;
        }
    }
    each(digit,currMin){
    ...
    string newHour = map[currMin[1]] + map[currMin[0]];
    string newMin = map[currHour[1]] + map[currHour[0]];
    return stoi(newHour) < hours and stoi(newMin) < mins;
}
vt(string)split(string s,char id){
    vt(string)ans;
    int start = 0;
    while(start < s.sz()){
        int end = start;
        while(end < s.sz() and s[end] isnt id){
            end++;
        }
        if(start isnt end){
...
pair(string,string)format(pair(string,string)&curr){
    string hour = (curr.fi.sz() is 1 ? "0" + curr.fi : curr.fi);
    string min = (curr.sc.sz() is 1 ? "0" + curr.sc : curr.sc);
    return mp(hour,min);
}
