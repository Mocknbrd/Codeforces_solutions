...
const ll linf = 2e18;
void testcase();
int findAns(string &src,string &target);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
int findAns(string &src,string &target){
    if(count(all(src),'1') isnt count(all(target),'1')){
        return inf;
    } else {
        int ones = 0,zeros = 0;
        inc(i,0,src.sz()){
            ones += (src[i] is '1' and target[i] is '0');
            zeros += (src[i] is '0' and target[i] is '1');
        }
        if(ones isnt zeros){
            return inf;
        } else {
