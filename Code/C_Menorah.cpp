...
void testcase(){
    ...
    vi state(4,0);
    inc(i,0,n){
        state[0] += a[i] is '0' and b[i] is '0';
        state[1] += a[i] is '0' and b[i] is '1';
        state[2] += a[i] is '1' and b[i] is '0';
        state[3] += a[i] is '1' and b[i] is '1';
    }
    set(vi)seen;
    queue<pair(int,vi)>q;
    ...
    while(q.empty() is false){
        pair(int,vi)curr = q.front();
        q.pop();
        if(curr.sc.front() + curr.sc.back() is n){
            see(curr.fr);
            return;
        } 
        if(state[2] > 0){
            ...
            next[2] = state[0] + 1;
            next[3] = state[2];
            if(seen.count(next) is false){
                seen.ins(next);
                q.push(make_pair(curr.fr + 1,next));
            }
        }
        if(state[3] > 0){
            vi next(4,0);
