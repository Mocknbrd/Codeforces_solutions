...
void testcase(){
    ...
    vi mini(t.sz(),0),maxi(t.sz(),t.sz() - 1);
    int prev = -1;
    inc(i,0,t.sz()){
        char curr = t[i];
        int index = upper_bound(all(pos[curr - 'a']),prev) - pos[curr - 'a'].begin();
        mini[i] = pos[curr - 'a'][index];
        prev = mini[i];
    }
    prev = s.sz();
    dec(i,t.sz() - 1,0){
        char curr = t[i];
        int start = 0,end = pos[curr - 'a'].sz() - 1,res = pos[curr - 'a'].back();
        while(start <= end){
            int mid = (start + end) >> 1;
            if(pos[curr - 'a'][mid] < prev){
                res = pos[curr - 'a'][mid];
                start = mid + 1;
            } else {
                end = mid - 1;
    ...
    int ans = max(maxi.front(),cast(s.sz(),int) - mini.back() - 1);
    inc(i,1,t.sz()){
        ans = max(ans,max(maxi[i] - mini[i - 1] - 1,maxi[i - 1] - mini[i] - 1));
    }
    see(ans);
    return;
}
