...
const int inf = 2e9;
const ll linf = 2e18;
void findDistance(vvi &tree,vi &distance,int vertex,int parent,int curr);
void dfs(vvi &tree,vi &current,vi &goal,vi &distance,vi &ans,int vertex,int parent,int even,int odd);
void testcase();
int main(){
    ios;
    SINGLE;
...
void testcase(){
    ...
    vi current(n + 1,0),goal(n + 1,0);
    readArray(current,1);
    readArray(goal,1);
    vi distance(n + 1,0);
    findDistance(tree,distance,1,-1,0);
    vi ans;
    dfs(tree,current,goal,distance,ans,1,-1,0,0);
    see(ans.sz());
    sorted(ans);
    each(value,ans){
        see(value);
...
void findDistance(vvi &tree,vi &distance,int vertex,int parent,int curr){
    ...
    each(child,tree[vertex]){
        if(child isnt parent){
            findDistance(tree,distance,child,vertex,curr + 1);
        }
    }
}
void dfs(vvi &tree,vi &current,vi &goal,vi &distance,vi &ans,int vertex,int parent,int even,int odd){
    int parity = mod(distance[vertex],2);
    if(parity is 1 and (current[vertex] ^ odd) isnt goal[vertex]){
        odd ^= 1;
        ans.pb(vertex);
    }
    if(parity is 0 and (current[vertex] ^ even) isnt goal[vertex]){
        even ^= 1;
        ans.pb(vertex);
    }
    each(child,tree[vertex]){
        if(child isnt parent){
            dfs(tree,current,goal,distance,ans,child,vertex,even,odd);
        }
    }
}
