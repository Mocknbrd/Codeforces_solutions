...
void testCase(){
    ll l,r;
    cin>>l>>r;
    ll sqrtl=sqrt(l),sqrtr=sqrt(r);
    ll ans=max((sqrtr-sqrtl-1)*3,llzero);
    inc(i,0,3){
        ll left=sqrtl*(sqrtl+i);
        ll right=sqrtr*(sqrtr+i);
        ans+=(left>=l and left<=r)+(sqrtr!=sqrtl and right>=l and right<=r);
    }
    puts(ans);
    return;
}
ll sqrt(ll n){
    ll start=0,end=2000000123,ans=1;
    while(start<=end){
        ll mid=(start+end)>>1;
        if(mid*mid<=n){
            ans=mid;
