...
void testcase(){
    ...
    min_heap(pll)cnds;
    each(value,seen){
        if(seen.count(value - 1) is false){
            cnds.push(mp(1,value - 1));
        }
        if(seen.count(value + 1) is false){
            cnds.push(mp(1,value + 1));
        }
    ...
    vi res;
    while(res.sz() < m){
        pll cnd = cnds.top(); cnds.pop();
        if(seen.count(cnd.sc) is false){
            ans += cnd.fi++;
            seen.ins(cnd.sc);
            res.pb(cnd.sc);
        }
        if(seen.count(cnd.sc + 1) is false){
            cnds.push(mp(cnd.fi,cnd.sc + 1));
        }
        if(seen.count(cnd.sc - 1) is false){
            cnds.push(mp(cnd.fi,cnd.sc - 1));
        }
