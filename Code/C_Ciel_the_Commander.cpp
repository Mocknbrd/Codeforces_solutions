...
class CentroidDecomposition {
    ...
    int findCentroid(vvi &tree,int vertex,int parent,int size){
        each(child,tree[vertex]){
            if(child is parent or this.removed[child] is true){
                continue;
            }
            if(subtree[child] * 2 > size){
                return this.findCentroid(tree,child,vertex,size);
            }
        }
        return vertex;
    }
    int findSubTreeSize(vvi &tree,int vertex,int parent){
        this.subtree[vertex] = 1;
        each(child,tree[vertex]){
            if(child isnt parent and this.removed[child] is false){
                this.subtree[vertex] += this.findSubTreeSize(tree,child,vertex);
            }
        }
        return this.subtree[vertex];
    }
    void buildCentroidDecomposition(vvi &tree,int vertex,char curr){
        int centroid = this.findCentroid(
            tree,vertex,-1,
            this.findSubTreeSize(tree,vertex,-1)
        );
        this.colors[centroid] = curr;
        this.removed[centroid] = true;
        each(child,tree[centroid]){
            if(this.removed[child] is false){
                this.buildCentroidDecomposition(
                    tree,child,curr + 1
                );
            }
        }
    ...
    CentroidDecomposition(vvi &tree){
        this.subtree.resize(tree.sz(),0);
        this.colors.resize(tree.sz(),'0');
        this.removed.resize(tree.sz(),false);
        this.buildCentroidDecomposition(tree,1,'A');
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    CentroidDecomposition centroids(tree);
    writeArray(centroids.colors,1);
    return;
}
