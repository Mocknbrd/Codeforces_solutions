...
void testcase(){
    ...
    while(q--){
        ...
        if(d is n){
            ...
            ll start = 0,end = stops.sz() - 1,ind = 0;
            while(start <= end){
            ...
            start = time[ind],end = time[ind + 1];
            ll ans = time[ind];
            while(start <= end){
                ll cnd = (start + end) >> 1ll;
                if((stops[ind + 1] - stops[ind]) * (cnd - time[ind]) <= (d - stops[ind]) * (time[ind + 1] - time[ind])){
                    ans = cnd;
                    start = cnd + 1;
                } else {
                    end = cnd - 1;
