...
const ll linf = 2e18;
void testcase();
bool canSplit(vvi &dp,vi &arr,int curr,int target);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    if(mod(sum,2) is 1){
        ...
        vvi dp(n,vi(sum / 2 + 1,-1));
        if(canSplit(dp,arr,0,sum / 2) is false){
            see(0);
        } else {
            inc(i,0,n){
                if(mod(arr[i] / g,2) is 1){
                    see(1);
                    see(i + 1);
                    return;
                }
...
bool canSplit(vvi &dp,vi &arr,int curr,int target){
    if(target is 0){
        ...
        return false;
    } elif(dp[curr][target] isnt -1){
        return dp[curr][target];
    } else {
        bool ans1 = false,ans2 = canSplit(dp,arr,curr + 1,target);
        if(target >= arr[curr]){
            ans1 = canSplit(dp,arr,curr + 1,target - arr[curr]);
        }
        return dp[curr][target] = ans1 or ans2;
    }
}
