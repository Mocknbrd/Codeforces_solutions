...
int main(){
    ...
    decrement(i,s.size()-1,0){
        if(s[i]==']'){
            closingBracket=i;
            break;
        }
    }
    if(openingBracket==-1 or closingBracket==-1 or openingBracket>closingBracket){
        cout<<-1<<endl;
    } else {
        length+=2;
        int firstColon=-1,secondColon=-1;
        increment(i,openingBracket+1,closingBracket){
            if(s[i]==':'){
                firstColon=i;
                break;
            }
        }
        decrement(i,closingBracket-1,openingBracket+1){
            if(s[i]==':'){
                secondColon=i;
                break;
            }
        }
        if(firstColon==-1 or secondColon==-1 or firstColon>=secondColon){
            cout<<-1<<endl;
        } else {
            length+=2;
            increment(i,firstColon+1,secondColon){
                length+=(s[i]=='|');
            }
            cout<<length<<endl;
        }
