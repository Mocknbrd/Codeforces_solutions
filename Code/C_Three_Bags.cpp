...
void testcase(){
    ...
    vpll maxi,mini;
    inc(i,0,3){
        maxi.pb(mp(*--arr[i].end(),i));
        mini.pb(mp(*arr[i].begin(),i));
    }
    rsort(mini);
    sorted(maxi);
    if(maxi.back().sc is mini.back().sc){
        maxi.rb();
    }
    ll maxValue = maxi.back().fi;
    ll maxIndex = maxi.back().sc;
    arr[maxi.back().sc].erase(maxValue); 
    ll miniValue = mini.back().fi;
    ll miniIndex = mini.back().sc;
    arr[mini.back().sc].erase(miniValue);
    maxi.rb(); mini.rb();
    ll secondMin = mini.back().fi;
    arr[mini.back().sc].erase(secondMin);
    mini.rb();
    ll sum = 0;
    while(arr[miniIndex].empty() is false){
        secondMin -= *arr[miniIndex].begin();
        arr[miniIndex].erase(arr[miniIndex].begin());
    }
    if(secondMin <= 0){
        maxValue -= secondMin;
    } else {
