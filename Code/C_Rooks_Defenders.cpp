...
class SegmentTree{
    ...
    void _add(Node*node,int pos){
        if(!node or pos > node->end or pos < node->start){
            return;
        } else if(pos >= node->start and pos <= node->end){
            node->cnt++;
        }
        this->_add(node->left,pos);
        this->_add(node->right,pos);
    }

    void _remove(Node*node,int pos){
        if(!node or pos < node->start or pos > node->end){
            return;
        } else if(pos >= node->start and pos <= node->end){
            node->cnt--;
        }
        this->_remove(node->left,pos);
        this->_remove(node->right,pos);
    }

    int _rangeQuery(Node*node,int start,int end){
        if(!node or start > node->end or end < node->start){
            return 0;
        } else if(node->start >= start and node->end <= end){
            return node->cnt;
        } else {
            int left = this->_rangeQuery(node->left,start,end);
            int right = this->_rangeQuery(node->right,start,end);
            return left + right;
        }
    }
    ...
    SegmentTree(int start,int end){
        this->root = this->_build(start,end);
    }

    void add(int pos){
        this->_add(this->root,pos);
    }

    void remove(int pos){
        this->_remove(this->root,pos);
    }

    int rangeQuery(int start,int end){
        return this->_rangeQuery(this->root,start,end);
    }
...
void testcase(){
    int n,q;
    cin >> n >> q;
    map<int,int>rowCnts,colCnts;
    SegmentTree rows(1,n),cols(1,n);
    while(q--){
        int t;
        cin >> t;
        switch(t){
            ...
            default: {
                int x1,y1,x2,y2;
                cin >> x1 >> y1 >> x2 >> y2;
                if(rows.rangeQuery(x1,x2) == x2 - x1 + 1 or cols.rangeQuery(y1,y2) == y2 - y1 + 1){
                    yy;
                } else {
                    nn;
                }
