...
void testcase(){
    ...
    cin >> n >> s;
    inc(distance,1,n + 1){
        if(mod(n,distance) is 0){
            string sub = s.substr(0,distance),prefix = "";
            while(prefix.sz() isnt s.sz()){
                prefix += sub;
            }
            int cnt = 0;
            inc(i,0,n){
                cnt += prefix[i] isnt s[i];
                if(cnt is 2){
                    break;
                }
            }
            ...
            sub = s.substr(s.sz() - distance);
            string suffix = "";
            while(suffix.sz() isnt s.sz()){
                suffix += sub;
            }
            cnt = 0;
            inc(i,0,n){
                cnt += suffix[i] isnt s[i];
                if(cnt is 2){
                    break;
                }
            }
