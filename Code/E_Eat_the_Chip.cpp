...
void testcase(){
    ...
    int diff = abs(xa - xb);
    int bobMoves = diff / 2;
    int aliceMoves = diff - bobMoves;
    pii alice = mp(max(1,ya - aliceMoves),min(w,ya + aliceMoves));
    pii bob = mp(max(1,yb - bobMoves),min(w,yb + bobMoves));
    if(xa >= xb){
        see("Draw");
    } elif(mod(diff,2) is 0){
        if(alice.fi >= bob.fi and alice.sc <= bob.sc){
            Bb;
        } else {
            see("Draw");
        }
    } else {
        if(bob.fi >= alice.fi and bob.sc <= alice.sc){
            Aa;
        } else {
            see("Draw");
        }
