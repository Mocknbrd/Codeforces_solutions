...
void testcase(){
    ...
    inc(maxi,0,n){
        ...
        set(int)cnd;
        inc(i,0,segments.sz()){
            if(maxi < segments[i].fi or maxi > segments[i].sc){
                inc(j,segments[i].fi,segments[i].sc + 1){
                    tmp[j]--;
                }
                cnd.ins(i + 1);
            }
        }
        int value = *max_element(all(tmp)) - *min_element(all(tmp));
        if(value > ans){
            ans = value;
            chosen = cnd;
        }
