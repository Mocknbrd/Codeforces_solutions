...
void testCase(){
    ...
    while(sink[0].sz()){
        int index=0;
        inc(i,0,n){
            if(*sink[i].begin()<*sink[index].begin()){
                index=i;
            }
        }
        ans[index].pb(*sink[index].begin());
        sink[index].erase(sink[index].begin());
        inc(i,0,n){
            if(i!=index){
                ans[i].pb(*sink[i].rbegin());
                sink[i].erase(sink[i].find(*sink[i].rbegin()));
            }
        }
    }
    inc(i,0,ans.sz()){
