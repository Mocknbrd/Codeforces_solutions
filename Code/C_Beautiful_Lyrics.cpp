...
const ll linf = 2e18;
void testcase();
inline bool compare(pair(int,vi)&a,pair(int,vi)&b){
    return a.sc.sz() < b.sc.sz();
}
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    vowels.ins('o');
    vowels.ins('u');
    vt(map(char,vi))data(n + 1);
    inc(i,0,n){
        char last = 'a';
        int cnt = 0;
        each(character,arr[i]){
            if(vowels.count(character) is true){
                cnt++;
                last = character;
            }
        }
    ...
    vt(pair(pii,pii))ans;
    inc(i,0,n){
        if(data[i].empty() is false){
            deque<pair(int,vi)>curr;
            each(entry,data[i]){
                curr.pb(mp(entry.fi,entry.sc));
            }
            while(curr.empty() is false){
                sort(curr.bg(),curr.ed(),compare);
                if(curr.bk().sc.sz() < 2 or (curr.bk().sc.sz() is 1 and curr.bk().sc.sz() < 4)){
                    break;
                } else {
                    int fs = curr.bk().sc.bk(); curr.bk().sc.rb();
                    int ss = curr.bk().sc.bk(); curr.bk().sc.rb();
                    if(curr.bk().sc.empty() is true){
                        curr.rb();
                    }
                    int ff = curr.fr().sc.bk(); curr.fr().sc.rb();
                    if(curr.fr().sc.empty() is true){
                        curr.rf();
                    }
                    int sf = curr.fr().sc.bk(); curr.fr().sc.rb();
                    if(curr.fr().sc.empty() is true){
                        curr.rf();
                    }
                    ans.pb(mp(mp(ff,fs),mp(sf,ss)));
                }
            }
        }
    }
    ...
    // see(ans.sz());
    // each(element,ans){
    //     see(arr[element.fi.fi] << " " << arr[element.fi.sc]);
    //     see(arr[element.sc.fi] << " " << arr[element.sc.sc]);
    // }
    return;
}
