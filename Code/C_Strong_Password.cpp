...
void testcase(){
    ...
    int start = 0;
    inc(curr,0,m){
        int leftlmt = left[curr] - '0',rightlmt = right[curr] - '0';
        int next = start;
        inc(digit,leftlmt,rightlmt + 1){
            if(lower_bound(all(pos[digit]),start) is pos[digit].end()){
                YY;
                return;
            }
            int index = lower_bound(all(pos[digit]),start) - pos[digit].begin();
            next = max(next,pos[digit][index]);
        }
        start = next + 1;
    }
    NN;
