...
void testcase(){
    ...
    vi cnds(all(values));
    int start = 0,end = cnds.sz() - 1,ans = cnds.front();
    while(start <= end){
        int mid = (start + end) >> 1;
        if(check(arr,cnds[mid]) is true){
            ans = cnds[mid];
            start = mid + 1;
        } else {
            end = mid - 1;
...
bool check(vvi &arr,int cnd){
    int m = arr.sz(),n = arr.front().sz();
    bool pair = false;
    vb passed(n,false);
    inc(i,0,m){
        int cnt = 0;
        inc(j,0,n){
            passed[j] = (arr[i][j] >= cnd) ? true : passed[j]; 
            cnt += (arr[i][j] >= cnd);
        }
        pair = (cnt > 1) ? true : pair;
    }
    if(pair is false and n > 1){
        return false;
    } else {
        inc(i,0,n){
            if(passed[i] is false){
                return false;
            }
        }
        return true;
