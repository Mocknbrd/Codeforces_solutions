...
template<typename tmp> tmp ceil(tmp num,tmp den){
    return (num / den) + (abs(num) % abs(den) != 0);
}

template<typename tmp> tmp mod(tmp number,tmp base){
    if(number < 0){
        ll mul = ceil(abs(number),base);
        number += mul * base;
    }
    return number % base;
}
...
template<typename tmp> tmp power(tmp base,tmp exponent){
    ...
    if(exponent is 0){
        ...
        dp[0] = base;
        inc(i,1,dp.sz()){
            dp[i] = mod(dp[i - 1] * dp[i - 1],md);
        }
        tmp ans = 1;
        while(exponent){
            int pos = log2(exponent & (-exponent));
            exponent ^= (1 << pos);
            ans = mod(ans * dp[pos],md);
        }
        return ans;
    }
}
...
void testcase(){
    ...
    vll arr(n);
    readArray(arr);
    ll sum = 0,curr = 0,total = 0;
    each(value,arr){
        curr = max(curr + value,value);
        sum = max(sum,curr);
        total = mod(total + value,md);
    }
    sum = mod(sum,md);
    ll ans = mod(total + mod((power(2ll,k) - 1) * sum,md),md);
    see(ans);
    return;
}
