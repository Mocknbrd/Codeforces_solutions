...
class SegmentTree {
    ...
    void getValue(Node *node,int pos,int &value){
        if(node is None or node->start > pos or node->end < pos){
            return;
        } else {
            if(node->lazy isnt (1 << 30) - 1){
                node->store = (node->store & node->lazy);
                if(node->left isnt None){
                    node->left->lazy = (node->left->lazy & node->lazy);
                }
                if(node->right isnt None){
                    node->right->lazy = (node->right->lazy & node->lazy);
                }
                node->lazy = (1 << 30) - 1;
            }
            if(pos >= node->start and pos <= node->end){
                value = (value & node->store);
            }
            this.getValue(node->left,pos,value);
            this.getValue(node->right,pos,value);
        }
    }
    void update(Node *node,int left,int right,int value){
        if(node is None or node->start > right or node->end < left){
            return;
    ...
    int getValue(int pos){
        int ans = (1 << 30) - 1;
        this.getValue(this.root,pos,ans);
        return ans;
    }
    void update(int left,int right,int value){
        this.update(this.root,left,right,value);
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    inc(pos,0,31){
        ...
        if(ones isnt 0){
            ans = mod(ans + power(2ll,pos + n - 1ll,md),md);
        }
    }
    see(ans);
    return;
