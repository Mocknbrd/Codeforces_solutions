...
void testcase(){
    ...
    readArray(arr);
    vi left,right;
    left.pb(arr.fr()),right.pb(arr.bk());
    int start = 1,end = n - 2;
    while(start < n and arr[start] > left.bk()){
        left.pb(arr[start++]);
    }
    while(end >= 0 and arr[end] > right.bk()){
        right.pb(arr[end--]);
    }
    int first = 0,second = 0;
    string ans;
    while(first < left.sz() and second < right.sz() and left[first] isnt right[second]){
    ...
    if(left.sz() - first >= right.sz() - second){
        while(first < left.sz()){
            ans += 'L';
            first++;
        } 
    } else {
        while(second < right.sz()){
            ans += 'R';
            second++;
        }
    }
