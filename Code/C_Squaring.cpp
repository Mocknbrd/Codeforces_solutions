...
void testcase(){
    ...
    readArray(arr);
    rev(arr);
    while(arr.empty() is false and arr.back() is 1){
        arr.rb();
    }
    rev(arr);
    each(value,arr){
    ...
    if(is_sorted(all(arr)) is true){
        see(0);
    } else {
        vll counts(arr.sz(),0);
        ll ans = 0;
        inc(i,1,counts.sz()){
            counts[i] = max(0ll,(ll)(ceil(counts[i - 1] - (ld)1e-9 + log2l(log2l(arr[i - 1])) - log2l(log2l(arr[i])))));
            ans += counts[i];
        }
        see(ans);
    }
