...
bool checkPalindrome(int hour,int min);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    int ans = 0;
    pii time = getTime(s);
    int hours = time.fi,minutes = time.sc,absTime = hours * 60 + minutes;
    set(int)times;
    inc(_,0,1440){
        absTime = mod(absTime + x,1440);
        times.ins(absTime);
    }
    inc(totalMin,0,1440){
        int currMin = mod(totalMin,60);
        int currHour = totalMin / 60;
        ans += (checkPalindrome(currHour,currMin) is true and times.count(totalMin));
    }
    see(ans);
    return;
}
pii getTime(string s){
    ...
    return mp(stoi(s.substr(0,start)),stoi(s.substr(start + 1)));
}
bool checkPalindrome(int hour,int min){
    ...
    m = (m.sz() is 1 ? "0" + m : m);
    string time = h + ":" + m;
    int start = 0,end = time.sz() - 1;
    while(start <= end){
        if(time[start++] isnt time[end--]){
            return false;
        }
    }
    return true;
