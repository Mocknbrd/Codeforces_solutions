...
const int inf = 2e9;
const ll linf = 2e18;
void dfs(vvll &graph,vb &seen,vi &path,int vertex);
ll findCnd(string &s);
void testcase();
int main(){
    ios;
...
void dfs(vvll &graph,vb &seen,vi &path,int vertex){
    ...
    each(child,graph[vertex]){
        if(seen[child] is false){
            dfs(graph,seen,path,child);
        }
    }
}
ll findCnd(string &s){
    int n = s.sz();
    if(count(all(s),s[0]) is n){
        return 1;
    }
    int ans = n;
    int factor = 2;
    while(factor * factor <= n){
        if(mod(n,factor) is 0){
            string pattern = s.slice(0,factor - 1),res = "";
            while(res.sz() isnt s.sz()){
                res += pattern;
            }
            if(res is s){
                return factor;
            } elif(factor isnt n / factor) {
                pattern = s.slice(0,n / factor - 1),res = "";
                while(res.sz() isnt s.sz()){
                    res += pattern;
                }
                if(res is s){
                    ans = min(ans,n / factor);
                }
            }
        }
        factor++;
