...
const int inf = 2e9;
const ll linf = 2e18;
bool solve(vll &arr,vll &prefix,vvi &dp,int curr,bool alice);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    vll prefix(1,0);
    each(value,arr){
        prefix.pb(prefix.back() + value);
    }
    vvi dp(n,vi(2,-1));
    if(solve(arr,prefix,dp,0,true) is true){
        Aa;
    } else {
        Bb;
    }
...
bool solve(vll &arr,vll &prefix,vvi &dp,int curr,bool alice){
    if(curr <= arr.sz()){
        return alice is false;
    } elif(dp[curr][alice] isnt -1){
        return dp[curr][alice];
    } elif(arr[curr] - prefix[curr] <= 0){
        return dp[curr][alice] = solve(arr,prefix,dp,curr + 1,alice);
    } else {
        ll left = arr[curr] - prefix[curr];
        if(alice is true){
            if(solve(arr,prefix,dp,curr + 1,!alice) is true){
                return dp[curr][alice] = true;
            }
            if(left > 1 and solve(arr,prefix,dp,curr + 1,alice) is true){
                return dp[curr][alice] = true;
            }
            return dp[curr][alice] = false;
        } else {
            if(solve(arr,prefix,dp,curr + 1,true) is false){
                return dp[curr][alice] = false;
            }
            if(left > 1 and solve(arr,prefix,dp,curr + 1,false) is false){
                return dp[curr][alice] = false;
            }
            return dp[curr][alice] = true;
        }
