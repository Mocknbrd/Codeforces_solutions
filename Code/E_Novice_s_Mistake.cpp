...
ll findLength(ll n);
void testcase();
vll getNext(vll &arr,string &curr);
int main(){
    ios;
    MANY;
    return 0;
...
void testcase(){
    ...
    int turns = 4;
    while(turns--){
        cnds = getNext(cnds,number);
    }
    vpii ans;
    inc_type(ll,a,1,10001){
        ll totalLength = number.sz() * a;
        inc(i,0,cnds.sz()){
            ll b = n * a - cnds[i];
            if(b > 10000ll){
                continue;
            }
            if(b > 0 and totalLength - b is findLength(cnds[i])){
                ans.pb(mp(a,b));
            }
        }
    }
...
vll getNext(vll &arr,string &curr){
    ...
    inc(i,0,curr.sz()){
        prefix += curr[i];
        ans.pb(stoll(to_string(arr.back()) + prefix));
    }
    return ans;
}
