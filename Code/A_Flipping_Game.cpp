...
class Node {
    public:  
    int start,end,ones,zeros;
    Node *left,*right;
    Node(int start,int end,int ones,int zeros){
        this.start = start;
        this.end = end;
        this.ones = ones;
        this.zeros = zeros;
...
class SegmentTree {
    ...
    Node* _build(vi &arr,int start,int end){
        Node *node;
        if(start is end){
            node = new Node(start,end,arr[start] is 1,arr[start] is 0);
        } else {
            int mid = (start + end) >> 1;
            Node *left = this._build(arr,start,mid);
            Node *right = this._build(arr,mid + 1,end);
            node = new Node(start,end,left->ones + right->ones,left->zeros + right->zeros);
            node->left = left;
            node->right = right;
        }
        return node;
    }
    int _query(Node *node,int start,int end,int value){
        if(!node or start > node->end or end < node->start){
            return 0;
        } elif(node->start >= start and node->end <= end){
            return (value is 1) * node->ones + (value is 0) * node->zeros;
        } else {
            return this._query(node->left,start,end,value) + this._query(node->right,start,end,value);
        }
    }
    ...
    int findOnes(int start,int end){
        return this._query(this.root,start,end,1);
    }
    int findZeros(int start,int end){
        return this._query(this.root,start,end,0);
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    inc(left,0,n){
        inc(right,left,n){
            ans = max(ans,tree.findOnes(0,left - 1) + tree.findZeros(left,right) + tree.findOnes(right + 1,n - 1));
        }
    }
    see(ans);
    return;
