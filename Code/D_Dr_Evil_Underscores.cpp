...
int findAns(vi arr,int pos){
    if(pos is -1){
        ...
        each(value,arr){
            switch((value >> pos) & 1){
                case 0: zero.pb(value); break;
                case 1: one.pb(value); break;
            }
        }
        if(one.empty() is true or zero.empty() is true){
            return findAns(arr,pos - 1);
        } else {
            return (1 << pos) + min(findAns(one,pos - 1),findAns(zero,pos - 1));
        }
    }
}
