...
const ll linf = 2e18;
void testcase();
void dfs1(vt(vpii)&tree,set(int)&cnds,int end,int curr,int vertex,int parent);
bool dfs2(vt(vpii)&tree,set(int)&cnds,int start,int curr,int vertex,int parent);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    set(int)starting;
    dfs1(tree,starting,b,0,a,-1);
    if(dfs2(tree,starting,b,0,b,-1) is true){
        YY;
    } else {
        NN;
    }
...
void dfs1(vt(vpii)&tree,set(int)&cnds,int end,int curr,int vertex,int parent){
    if(vertex isnt end){
        ...
        each(child,tree[vertex]){
            if(child.fi isnt parent){
                dfs1(tree,cnds,end,curr ^ child.sc,child.fi,vertex);
            }
        }
    }
}
bool dfs2(vt(vpii)&tree,set(int)&cnds,int start,int curr,int vertex,int parent){
    if(vertex isnt start and cnds.count(curr) is true){
        return true;
    } else {
        each(child,tree[vertex]){
            if(child.fi isnt parent){
                if(dfs2(tree,cnds,start,curr ^ child.sc,child.fi,vertex) is true){
                    return true;
                }
            }
        }
