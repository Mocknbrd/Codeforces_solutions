...
const int md = 1e9 + 7;
void testcase();
int solve(vi &arr,vvi &dp,int index,int mask,int &k);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    readArray(arr);
    vvi dp(n,vi(128,-1));
    see(solve(arr,dp,0,(1 << 7) - 1,k));
    return;
}
int solve(vi &arr,vvi &dp,int index,int mask,int &k){
    if(index >= arr.sz()){
        return __builtin_popcount(mask) is k;
    } elif(dp[index][mask] isnt -1){
        return dp[index][mask];
    } else {
        return dp[index][mask] = mod(solve(arr,dp,index + 1,mask,k) + solve(arr,dp,index + 1,(mask & arr[index]),k),md);
    }
}
