...
void testcase(){
    ...
    vt(string)arr(n);
    readArray(arr);
    vvi horizontal(n,vi(1,0)),vertical(m,vi(1,0));
    inc(i,0,n){
        inc(j,0,m){
            horizontal[i].pb(horizontal[i].back() + (arr[i][j] is '.'));
        }
    }
    inc(j,0,m){
        inc(i,0,n){
            vertical[j].pb(vertical[j].back() + (arr[i][j] is '.'));
        }
    }
    int ans = n * m;
    inc(i,0,n){
        inc(j,0,m){
            int left = horizontal[i][j] - horizontal[i][0],right = horizontal[i].back() - horizontal[i][j + 1],up = vertical[j][i] - vertical[j][0],down = vertical[j].back() - vertical[j][i + 1];
            ans = min(ans,(arr[i][j] is '.') + left + right + up + down);
        }
    }
    see(ans);
    return;
