...
const ll linf = 2e18;
void testcase();
bool solve(vll &arr,int n,int m);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    vll arr(k);
    readArray(arr);
    if(solve(arr,n,m) or solve(arr,m,n)){
        yy;
    } else {
        nn;
    }
...
bool solve(vll &arr,int n,int m){
    ll ans = 0;
    bool three = mod(m,2) is 0;
    each(value,arr){
        ans += (value / n >= 2) * (value / n);
        three = (three or value / n >= 3);
    }
    return ans >= m and three is true;
}
