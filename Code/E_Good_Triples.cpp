...
const ll linf = 2e18;
void testcase();
ll solve(ll digit,ll num,vvll &dp);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
ll solve(ll digit,ll num,vvll &dp){
    if(num is 1){
        return 1;
    } elif(dp[digit][num] isnt -1){
        return dp[digit][num];
    } else {
        ll ans = 0;
        inc_type(ll,next,0,digit + 1){
            ans += solve(digit - next,num - 1,dp);
        }
        return dp[digit][num] = ans;
    }
}
