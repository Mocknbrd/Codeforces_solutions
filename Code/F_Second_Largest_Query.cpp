...
class SegmentTree {
    ...
    Node* build(vi &arr,int start,int end){
        ...
        if(start is end){
            ...
            node = new Node(start,end,max(left->maxi,right->maxi));
            node->left = left;
            node->right = right;
        }
        return node;
    }
    int largest(Node *node,int start,int end){
        if(!node or start > node->end or end < node->start){
            return -inf;
        } elif(node->start >= start and node->end <= end){
            return node->maxi;
        } else {
            int left = this.largest(node->left,start,end);
            int right = this.largest(node->right,start,end);
            return max(left,right);
        }
    }
    int query(Node *node,int start,int end,int value){
        if(!node or start > node->end or end < node->start){
            return -inf;
        } elif(node->start >= start and node->end <= end and node->maxi < value){
            return node->maxi;
        } else {
            int left = this.query(node->left,start,end,value);
            int right = this.query(node->right,start,end,value);
            return max(left,right);
        }
    }
    void update(Node *node,int pos,int value){
        if(!node or pos > node->end or pos < node->start){
            return;
        } else {
            if(node->start is node->end){
                node->maxi = value;
            } else {
                this.update(node->left,pos,value);
                this.update(node->right,pos,value);
                int left = (node->left ? node->left->maxi : -inf);
                int right = (node->right ? node->right->maxi : -inf);
                node->maxi = max(left,right);
            }
        }
    }
    ...
    int query(int start,int end){
        int largest = this.largest(this.root,start,end);
        return this.query(this.root,start,end,largest);
    }
    void update(int pos,int value){
        this.update(this.root,pos,value);
    }
};
void testcase();
int main(){
