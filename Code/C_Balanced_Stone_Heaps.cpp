...
void testcase(){
    ...
    vll arr(n);
    readArray(arr);
    ll start = *min_element(all(arr)),end = arr.back(),ans = *min_element(all(arr));
    while(start <= end){
        ll curr = (start + end) >> 1ll;
        vll dp(n + 1,0);
        dp[n - 1] = (arr.back() - curr) / 3ll;
        dec(i,n - 2,2){
            ll left = 0,right = arr[i] / 3,d = 0;
            while(left <= right){
                ll mid = (left + right) >> 1ll;
                if(arr[i] - 3 * mid + dp[i + 1] + 2 * dp[i + 2] >= curr){
                    d = mid;
                    left = mid + 1;
                } else {
                    right = mid - 1;
