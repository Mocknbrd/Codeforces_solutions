...
int main(){
    ...
    while(t--){
        ...
        int maxZeros=0;
        for(int rs=0,re=1;re<n;re++,rs++){
            for(int cs=0,ce=1;ce<m;ce++,cs++){
                int zeros=(matrix[rs][cs]=='0')+(matrix[rs][ce]=='0')+(matrix[re][cs]=='0')+(matrix[re][ce]=='0');
                maxZeros=max(maxZeros,zeros);
            }
        }
        cout<<ones-(maxZeros<=1)-(maxZeros<=0)<<endl;
    }
    return 0;
}