...
void testcase(){
    ...
    int left = -1,right = -1;
    inc(i,0,n){
        left = (left is -1 and arr[i] isnt -1 ? i : left);
        right = (arr[i] isnt -1 ? i : right);
    }
    if(left is -1){
        arr[0] = 1;
        inc(i,1,n){
            arr[i] = (arr[i - 1] is 1 ? 2 : 1);
        }
        writeArray(arr);
    } else {
        while(left > 0){
        ...
        int start = 0;
        while(start < n){
            while(start < n and arr[start] isnt -1){
                start++;
            }
            if(start < n){
                int end = start;
                while(end < n and arr[end] is -1){
                    end++;
                }
                ranges.pb(mp(start,end - 1));
                start = end;
            }
        }
        each(range,ranges){
        ...
        inc_la(i,0,n,1){
            if(arr[i] isnt arr[i + 1] / 2 and arr[i + 1] isnt arr[i] / 2){
                see(-1);
                return;
            }
        }
...
void solve(vi &arr,pii range){
    int start = arr[range.fi - 1],end = arr[range.sc + 1];
    vi starting,ending;
    starting.pb(start);
    ending.pb(end);
    ...
    vi path;
    ending.rb();
    while(starting.empty() is false){
        path.pb(starting.back());
        starting.rb();
    }
    while(ending.empty() is false){
        path.pb(ending.back());
        ending.rb();
    }
    if(range.sc + 1 - (range.fi - 1) + 1 < path.sz()){
        arr[range.fi] = inf;
    } else {
        int curr = range.fi - 1;
        each(value,path){
        ...
        int actual = arr[curr - 1];
        while(curr <= range.sc){
            if(arr[curr - 1] is actual){
                arr[curr] = 2 * actual;
            } else {
                arr[curr] = actual;
            }
