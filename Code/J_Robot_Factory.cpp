...
const ll linf = 2e18;
void testcase();
void dfs(vvi &matrix,vvb &seen,vpii &dir,int &curr,int row,int col);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    vi ans;
    vpii dir(4);
    dir[0] = mp(0,-1),dir[1] = mp(1,0),dir[2] = mp(0,1),dir[3] = mp(-1,0);
    inc(row,0,n){
        inc(col,0,m){
            if(seen[row][col] is false){
                int curr = 0;
                dfs(config,seen,dir,curr,row,col);
                ans.pb(curr);
            }
        }
    }
...
void dfs(vvi &matrix,vvb &seen,vpii &dir,int &curr,int row,int col){
    ...
    seen[row][col] = true;
    inc(bit,0,dir.sz()){
        if(((matrix[row][col] >> bit) & 1) is 0){
            int nrow = row + dir[bit].fi,ncol = col + dir[bit].sc;
            if(seen[nrow][ncol] is false){
                dfs(matrix,seen,dir,curr,nrow,ncol);
            }
        }
    }
}
