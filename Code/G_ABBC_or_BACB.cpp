...
void testcase(){
    ...
    inc(i,0,s.sz()){
        if(s[i] is 'B'){
            auto lower = --lower_bound(all(pos),i);
            auto upper = upper_bound(all(pos),i);
            dp[i][0] = (*lower isnt -1 ? dp[*lower][0] : 0) + i - *lower - 1;
            dp[i][1] = (*lower isnt -1 ? max(dp[*lower][1],dp[*lower][0]): 0) + *upper - i - 1;
