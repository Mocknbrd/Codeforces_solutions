...
void testcase(){
    ...
    inc(i,0,n){
        pq.push(mp(mp(a[i],a[i]),i));
    }
    inc_la(i,0,n,1){
        if(a[i] >= a[i + 1]){
            pq.push(mp(mp(a[i],a[i + 1]),i));
        }
    }
    vi leftover;
    map(int,vi)insert;
    while(pq.empty() is false){
        pair(pii,int)curr = pq.top(); pq.pop();
        int index = curr.sc,lower = curr.fi.sc,upper = curr.fi.fi;
        while(values.empty() is false and values.top() < lower){
            leftover.pb(values.top());
            values.pop();
        }
        while(values.empty() is false and values.top() <= upper){
            insert[index].pb(values.top());
            values.pop();
        }
    }
