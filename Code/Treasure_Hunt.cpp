...
void testcase(){
    ...
    int firstmx = findMax(first),secondmx = findMax(second),thirdmx = findMax(third);
    int ans1 = min(length,firstmx + n),ans2 = min(length,secondmx + n),ans3 = min(length,thirdmx + n);
    ans1 -= (n is 1 and firstmx is length);
    ans2 -= (n is 1 and secondmx is length);
    ans3 -= (n is 1 and thirdmx is length);
    if(ans1 > max(ans2,ans3)){
        see("Kuro");
    } elif(ans2 > max(ans1,ans3)){
        see("Shiro");
    } elif(ans3 > max(ans1,ans2)){
        see("Katie");
    } else {
        see("Draw");
    }
