...
void testcase(){
    ...
    vvi prefix(n + 1,vi(2,inf)),suffix(n + 1,vi(2,inf));
    prefix.back()[0] = 1;
    prefix.back()[1] = 1;
    dec(i,n - 1,0){
        if(i is n - 1){
            ...
            prefix[i][1] = 1;
            if(type[i] is 1){
                prefix[i][0] = 1 + prefix[i + 1][type[i + 1]];
            } else {
                prefix[i][0] = 1 + prefix[i + 1][type[i + 1] ^ 1];
            }
        }
    }
    suffix[0][0] = 1;
    ...
    inc(i,1,n + 1){
        if(i is 1){
            ...
            suffix[i][0] = 1;
            if(type[i - 1] is 0){
                suffix[i][1] = 1 + suffix[i - 1][type[i - 2]];
            } else {
                suffix[i][1] = 1 + suffix[i - 1][type[i - 2] ^ 1];
            }
        }
    }
    vi ans(n + 1,0);
    ans[0] = prefix[0][type[0]];
    ans.back() = suffix.back()[type[n - 1]];
    inc(i,1,n){
        ans[i] = prefix[i][type[i]] + suffix[i][type[i - 1]] - 1;
    }
    writeArray(ans);
    return;
}
