...
const ll linf = 2e18;
void testcase();
bool solve(vvi &tree,vvi &dp,int vertex,int parent,bool ron);
int main(){
    ios;
    int t = 1;
    // cin >> t;
...
void testcase(){
    ...
    int start;
    cin >> start;
    if(solve(tree,dp,start - 1,-1,true)){
        see("Ron");
    } else {
        see("Hermione");
    }
...
bool solve(vvi &tree,vvi &dp,int vertex,int parent,bool ron){
    if(dp[vertex][ron] isnt -1){
        return dp[vertex][ron];
    } else {
        if(ron is true){
            each(child,tree[vertex]){
                if(child isnt parent){
                    if(solve(tree,dp,child,vertex,false) is true){
                        return dp[vertex][ron] = true;
                    }
                }
            }
            return dp[vertex][ron] = false;
        } else {
            each(child,tree[vertex]){
                if(child isnt parent){
                    if(solve(tree,dp,child,vertex,true) is false){
                        return dp[vertex][ron] = false;
                    }
                }
            }
