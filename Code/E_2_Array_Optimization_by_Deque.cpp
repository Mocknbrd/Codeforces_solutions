...
class SegmentTree {
    ...
    int __update(Node *node,int value){
        if(!node or value < node->start or value > node->end){
            return 0;
        } elif(node->start is node->end){
            node->sum += (node->start is value);
            return node->sum;
        } else {
            node->sum = self.__update(node->left,value) + self.__update(node->right,value);
            return node->sum;
        }
    }
    int __findSmaller(Node *node,int value){
        if(!node or value < node->start){
            return 0;
        } elif(node->end < value){
            return node->sum;
        } else {
            return self.__findSmaller(node->left,value) + self.__findSmaller(node->right,value);
        }
    }
    int __findLarger(Node *node,int value){
        if(!node or value > node->end){
            return 0;
        } elif(node->start > value){
            return node->sum;
        } else {
            return self.__findLarger(node->left,value) + self.__findLarger(node->right,value);
        }
    }
    public:
    SegmentTree(int n){
    ...
    int findSmaller(int value){
        return self.__findSmaller(self.root,value);
    }
    int findLarger(int value){
        return self.__findLarger(self.root,value);
    }
...
void testcase(){
    ...
    int ans = 0;
    inc(i,0,n){
        int front = tree.findSmaller(arr[i]);
        int back = tree.findLarger(arr[i]);
        if(front < back){
            dq.pf(arr[i]);
        } else {
            dq.pb(arr[i]);
