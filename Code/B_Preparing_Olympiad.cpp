...
const ll linf = 2e18;
void testcase();
bool check(vi &c,int mask,int l,int r,int x);
int main(){
    ios;
    SINGLE;
    return 0;
...
bool check(vi &c,int mask,int l,int r,int x){
    int mini = inf,maxi = -inf,total = 0;
    while(mask){
        int pos = log2(mask & (-mask));
        total += c[pos];
        mini = min(mini,c[pos]),maxi = max(maxi,c[pos]);
        mask ^= (1 << pos);
    }
    return inBetween(l,total,r) and maxi - mini >= x;
}
