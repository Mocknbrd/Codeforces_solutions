...
const int inf = 2e9;
const ll linf = 2e18;
ll constructNumber(vll &arr);
ll findThirdNumber(vll &first,vll &second);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    map(ll,int)pos;
    inc(i,0,n){
        pos[constructNumber(arr[i])] = i;
    }
    map(int,set(pii))mapping;
    inc(i,0,n){
        inc(j,i + 1,n){
            ll thirdNumber = findThirdNumber(arr[i],arr[j]);
            if(pos.count(thirdNumber) is true){
                int third = pos[thirdNumber];
                mapping[i].ins(mp(min(j,third),max(j,third)));
                mapping[j].ins(mp(min(i,third),max(i,third)));
                mapping[third].ins(mp(i,j));
            }
        }
    }
    ll ans = 0;
...
ll findThirdNumber(vll &first,vll &second){
    vll res;
    inc(i,0,first.sz()){
        res.pb(mod(3ll - first[i] - second[i],3ll));
    }
    return constructNumber(res);
}
