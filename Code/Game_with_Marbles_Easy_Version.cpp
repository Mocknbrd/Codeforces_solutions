...
const ll linf = 2e18;
void testcase();
ll canAliceWin(vll &alice,vll &bob,int turn);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    readArray(alice);
    readArray(bob);
    see(canAliceWin(alice,bob,0));
    return;
}
ll canAliceWin(vll &alice,vll &bob,int turn){
    bool further = false;
    inc(i,0,alice.sz()){
        if(alice[i] isnt 0 and bob[i] isnt 0){
            further = true;
            break;
        }
    }
    if(further is false){
        ...
        inc(i,0,alice.sz()){
            if(alice[i] isnt 0 and bob[i] isnt 0){
                ...
                ll value = bob[i];
                bob[i] = 0;
                ans = max(ans,canAliceWin(alice,bob,turn ^ 1));
                alice[i]++;
                bob[i] = value;
            }
        }
        ...
        inc(i,0,alice.sz()){
            if(alice[i] isnt 0 and bob[i] isnt 0){
                ...
                ll value = alice[i];
                alice[i] = 0;
                ans = min(ans,canAliceWin(alice,bob,turn ^ 1));
                bob[i]++;
                alice[i] = value;
            }
        }
