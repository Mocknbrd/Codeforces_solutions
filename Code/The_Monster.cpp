...
void testcase(){
    string s;
    cin >> s;
    vvb first(s.sz(),vb(s.sz(),false)),second = first;
    inc(left,0,s.sz()){
        int curr = 0;
        bool ok = true;
        inc(right,left,s.sz()){
            curr += (s[right] isnt ')') - (s[right] is ')');
            ok = ok and curr >= 0;
            first[left][right] = ok;
        }
    }
    inc(right,0,s.sz()){
        ...
        bool ok = true;
        dec(left,right,0){
            curr += (s[left] isnt '(') - (s[left] is '(');
            ok = ok and curr >= 0;
            second[left][right] = ok;
        }
    }
    int ans = 0;
    inc(left,0,s.sz()){
        inc(right,left + 1,s.sz()){
            ans += (mod(right - left + 1,2) is 0) * (first[left][right] and second[left][right]);
        }
    }
    see(ans);
    return;
