...
class SegmentTree {
    ...
    void update(Node *node,int pos,int value){
        if(node is None or pos > node->end or pos < node->start){
            return;
        } else {
            if(node->start is node->end){
                ...
                int left = (node->left isnt None ? node->left->sum : 0);
                int right = (node->right isnt None ? node->right->sum : 0);
                node->sum = left + right;
            }
            return;
        }
...
void testcase(){
    ...
    while(q--){
        ...
        if(type is 1){
            int sum;
            cin >> sum;
            int total = tree.query(0,n);
            if(mod(total - sum,2) is 0){
                YY;
            } elif(ones.empty() is true){
                NN;
            } else {
                int first = *ones.bg(),last = *ones.ed();
                int cnd = max(tree.query(0,last - 1),tree.query(first + 1,n));
                if(cnd >= sum){
                    YY;
                } else {
                    NN;
