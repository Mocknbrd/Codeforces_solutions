...
const ll linf = 2e18;
void testcase();
int bfs(vt(vpii)&graph,vb &seen,int vertex);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    inc(i,0,m){
        ...
        string type;
        cin >> u >> v >> type;
        graph[v].pb(mp(u,type is "imposter"));
        graph[u].pb(mp(v,type is "imposter"));
    }
    vb seen(n + 1,false);
    int ans = 0;
    inc(vertex,1,graph.sz()){
        if(seen[vertex] is false){
            int add = bfs(graph,seen,vertex);
            if(add is -1){
                see(-1);
                return;
            }
...
int bfs(vt(vpii)&graph,vb &seen,int vertex){
    ...
    while(q.empty() is false){
        ...
        seen[curr.fi] = true;
        colors[curr.fi] = curr.sc;
        each(element,graph[curr.fi]){
            int neighbour = element.fi,weight = element.sc;
            if(seen[neighbour] is false){
                seen[neighbour] = true;
                colors[neighbour] = (curr.sc ^ weight);
                q.push(mp(neighbour,colors[neighbour]));
            } else {
                if(colors[neighbour] isnt (curr.sc ^ weight)){
                    return -1;
                }
            }
        }
    ...
    int ones = 0,zeros = 0;
    each(element,colors){
        ones += element.sc is 1;
        zeros += element.sc is 0;
    }
