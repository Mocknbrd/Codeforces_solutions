...
void testcase(){
    ...
    inc(i,0,k){
        ...
        rev(arr[i]);
        pq.push(mp(arr[i].back(),i));
    }
    ll ans = x;
    while(pq.empty() is false and x >= 0){
        pair(pll,int)curr = pq.top(); pq.pop();
        x += curr.fi.sc;
        ans = max(ans,x);
        arr[curr.sc].rb();
        if(arr[curr.sc].empty() is false){
            pq.push(mp(arr[curr.sc].back(),curr.sc));
        }
    }
    see(ans);
    return;
