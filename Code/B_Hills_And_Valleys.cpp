...
const ll linf = 2e18;
void testcase();
bool isValley(vi &arr,int ind);
bool isHill(vi &arr,int ind);
bool isCnd(vi &arr,int ind);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    inc_la(i,1,n,1){
        int tmp = arr[i];
        arr[i] = arr[i - 1];
        ans = min(ans,total - cnd[i - 1] - cnd[i] - cnd[i + 1] + isCnd(arr,i) + isCnd(arr,i + 1) + isCnd(arr,i - 1));
        arr[i] = arr[i + 1];
        ans = min(ans,total - cnd[i] - cnd[i + 1] - cnd[i - 1] + isCnd(arr,i) + isCnd(arr,i + 1) + isCnd(arr,i - 1));
        arr[i] = tmp;
    }
    see(ans);
    return;
...
bool isValley(vi &arr,int ind){
    return ind - 1 >= 0 and ind + 1 < arr.sz() and arr[ind] < min(arr[ind + 1],arr[ind - 1]);
}
bool isHill(vi &arr,int ind){
    return ind - 1 >= 0 and ind + 1 < arr.sz() and arr[ind] > max(arr[ind + 1],arr[ind - 1]);
}
bool isCnd(vi &arr,int ind){
    return isValley(arr,ind) or isHill(arr,ind);
}
