...
class FenwickTree {
    ...
    public:  
    FenwickTree(int n){
        this.tree = vi(n + 1,-inf);
    }
    int find(int index){
        index++;
        int ans = -inf;
        while(index){
            ans = max(ans,this.tree[index]);
    ...
    void update(int index,int value){
        index++;
        while(index < this.tree.sz()){
            this.tree[index] = max(this.tree[index],value);
            index += this.lsb(index);
        }
    }
};
...
void testcase(){
    ...
    custSort(arr,compare);
    FenwickTree tree(n);
    inc(i,0,n){
        tree.update(i,arr[i].fi);
    }
    int ans = inf;
    inc(i,0,n){
        suffix.erase(suffix.find(arr[i].fi));
        int mx = tree.find(i - 1);
        ans = min(ans,abs(mx - arr[i].sc));
        auto it = suffix.lower_bound(arr[i].sc);
        if(it isnt suffix.end() and *it >= mx){
            ans = min(ans,abs(*it - arr[i].sc));
        }
        if(it isnt suffix.begin() and *prev(it) >= mx){
            ans = min(ans,abs(*prev(it) - arr[i].sc));
        }
    }
    see(ans);
    return;
...
bool compare(pii &a,pii &b){
    return a.sc > b.sc or (a.sc is b.sc and a.fi < b.fi);
}
