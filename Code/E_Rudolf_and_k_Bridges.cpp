...
class SegmentTree {
    ...
    void update(Node *node,int pos,ll value){
        if(node is None or pos > node->end or pos < node->start){
            return;
        } elif(node->start is node->end){
            if(node->start is pos){
                node->mini = value;
            }
        } else {
            this.update(node->left,pos,value);
            this.update(node->right,pos,value);
            ll left = (node->left isnt None ? node->left->mini : linf);
            ll right = (node->right isnt None ? node->right->mini : linf);
            node->mini = min(left,right);
        }
    }
    ll query(Node *node,int start,int end){
        if(node is None or start > node->end or end < node->start){
            ...
            ll left = this.query(node->left,start,end);
            ll right = this.query(node->right,start,end);
...
void testcase(){
    ...
    inc(i,0,n){
        ...
        tree.update(0,1);
        inc(j,1,m){
            dp[j] = 1ll + river[i][j] + tree.query(j - d - 1,j - 1);
            tree.update(j,dp[j]);
        }
        costs.pb(dp.back());
    }
    ...
    ll curr = ans;
    slid_win(left,right,k,costs.sz()){
        curr += costs[right] - costs[left - 1];
        ans = min(ans,curr);
    }
    see(ans);
    return;
