...
void testcase(){
    ...
    dec(ops,n - 1,1){
        vi modulo(ops,-1);
        inc(vertex,1,n + 1){
            int rem = mod(arr[vertex],ops);
            if(modulo[rem] is -1 or dsu.isSameComponent(modulo[rem],vertex) is true){
                modulo[rem] = vertex;
            } else {
                ans.pb(mp(modulo[rem],vertex));
                dsu.findUnion(modulo[rem],vertex);
                break;
            }
        }
    }
