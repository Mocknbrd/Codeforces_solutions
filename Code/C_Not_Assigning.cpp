...
const ll linf = 2e18;
void testcase();
void dfs(vvi &tree,map(pii,int)&mapping,int vertex,int parent);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    int more = 0,root = 1;
    inc(vertex,1,tree.sz()){
        more += (tree[vertex].sz() > 2);
        if(tree[vertex].sz() is 2){
            root = vertex;
        }
    }
    if(more isnt 0){
        ...
        map(pii,int)mapping;
        mapping[mp(root,tree[root][0])] = 11;
        mapping[mp(tree[root][0],root)] = 11;
        dfs(tree,mapping,tree[root][0],root);
        if(tree[root].sz() > 1){
            mapping[mp(root,tree[root].back())] = 2;
            mapping[mp(tree[root].back(),root)] = 2;
            dfs(tree,mapping,tree[root].back(),root);
        }
        each(edge,edges){
            cout << mapping[edge] << " ";
        }
...
void dfs(vvi &tree,map(pii,int)&mapping,int vertex,int parent){
    each(child,tree[vertex]){
        if(child isnt parent){
            int next = 2;
            int prev = mapping[mp(vertex,parent)];
            int weight = 2;
            if(mod(prev,2) is 0){
                weight = 5;
            }
            mapping[mp(vertex,child)] = weight;
            mapping[mp(child,vertex)] = weight;
            dfs(tree,mapping,child,vertex);
        }
    }
}
