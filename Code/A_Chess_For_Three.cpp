...
void testcase(){
    vi arr(3);
    readArray(arr);
    vt(vvi)dp(arr[0] + 1,vvi(arr[1] + 1,vi(arr[2] + 1,-1)));
    int ans = solve(arr,dp);
    if(ans < 0){
        see(-1);
    } else {
        see(ans);
...
int solve(vi &arr,vt(vvi) &dp){
    if(*min_element(all(arr)) < 0){
        return -inf;
    } elif(arr[0] is 0 and arr[1] is 0 and arr[2] is 0){
        return 0;
    } elif(dp[arr[0]][arr[1]][arr[2]] isnt -1){
        ...
        inc(i,0,arr.sz()){
            inc(j,i + 1,arr.sz()){
                arr[i] -= 2;
                int cnd1 = solve(arr,dp);
                arr[i] += 2;
                arr[j] -= 2;
                int cnd2 = solve(arr,dp);
                arr[j] += 2;
                arr[i] -= 1,arr[j] -= 1;
                int cnd3 = 1 + solve(arr,dp);
                arr[i]++,arr[j]++;
                ans = max(max(ans,cnd1),max(cnd2,cnd3));
            }
        }
        return dp[arr[0]][arr[1]][arr[2]] = ans;
    }
}
