...
void testcase(){
    ...
    ll allArr = 0;
    each(value,arr){
        allArr = (allArr is 0 ? value : lcm(allArr,value));
    }
    if(allArr isnt arr.back()){
        see(n);
    } else {
        map(ll,int)counts;
        each(value,arr){
        ...
        ll factor = 1;
        ll ans = 0;
        while(factor * factor <= arr.back()){
            if(mod(arr.back(),factor) is 0){
                ll first = factor,second = arr.back() / factor;
                if(counts.count(first) is false){
                    ll res = 0,l = 0;
                    each(element,counts){
                        if(mod(first,element.fi) is 0){
                            res += element.sc;
                            l = (l is 0 ? element.fi : lcm(l,element.fi));
                        }
                    }
                    if(l is first){
                        ans = max(ans,res);
                ...
                if(counts.count(second) is false){
                    ll res = 0,l = 0;
                    each(element,counts){
                        if(mod(second,element.fi) is 0){
                            res += element.sc;
                            l = l is 0 ? element.fi : lcm(element.fi,l);
                        }
                    }
                    if(l is second){
                        ans = max(ans,res);
