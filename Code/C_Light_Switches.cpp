...
void testcase(){
    ...
    readArray(arr);
    sorted(arr);
    pll range = mp(-inf,inf);
    inc(i,0,n){
        if(range.fi > range.sc){
            see(-1);
            return;
        }
        ll start = 0,end = 1e9,cnd = end;
        while(start <= end){
            ll mid = (start + end) >> 1ll;
            if(arr[i] + k + mid * 2ll * k > arr.back()){
                cnd = mid;
                end = mid - 1;
            } else {
                start = mid + 1;
        ...
        pll curr = mp(arr[i] + 2ll * k * cnd,arr[i] + 2ll * k * cnd + k - 1);
        range = mp(max(curr.fi,range.fi),min(range.sc,curr.sc));
        if(range.fi > range.sc){
            see(-1);
            return;
        }
