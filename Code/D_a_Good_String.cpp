...
const int inf = 2e9;
const ll linf = 8e18;
int solve(string &s,int start,int end,char curr);
void testcase();
int main(){
    ios;
    MANY;
...
int solve(string &s,int start,int end,char curr){
    if(start is end){
        return s[start] isnt curr;
    } else {
        int mid = (start + end) >> 1;
        int first = 0,second = 0;
        inc(i,start,end + 1){
            first += (i <= mid) * (s[i] is curr);
            second += (i > mid) * (s[i] is curr);
        }
        int ans1 = mid - start + 1 - first + solve(s,mid + 1,end,curr + 1);
        int ans2 = end - mid - second + solve(s,start,mid,curr + 1);
