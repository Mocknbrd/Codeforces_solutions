...
void testcase();
bool dfs(int diag,int row,int col,vvi &grid,int cnt);
int main(){
    ios;
    int t = 1;
    // cin >> t;
...
bool dfs(int diag,int row,int col,vvi &grid,int cnt){
    ...
    if(diag >= n){
        return true;
    } else if(cnt == grid[diag][diag]){
        grid[row][col] = grid[diag][diag];
        if(dfs(diag + 1,diag + 1,diag + 1,grid,1)){
            return true;
        } 
        return false;
    } else {
        ...
        each(move,moves){
            int nrow = row + move[0];
            int ncol = col + move[1];
            if(nrow >= 0 and ncol >= 0 and nrow < n and ncol < n and grid[nrow][ncol] == 0 and ncol < nrow and dfs(diag,nrow,ncol,grid,cnt + 1)){
                return true;
            }
        }
        return false;
