...
void testcase(){
    ...
    each(record,colors){
        ...
        vi steps = record.s;
        int maxSteps = 0;
        inc_la(i,0,steps.sz(),1){
            maxSteps = max(maxSteps,steps[i + 1] - steps[i]);
        }
        int cnd = 0;
        inc_la(i,0,steps.sz(),1){
            if(steps[i + 1] - steps[i] is maxSteps){
                int mid = (steps[i + 1] + steps[i]) / 2;
                maxSteps = -inf;
                cnd = max({cnd,mid - steps[i] - 1,steps[i + 1] - mid - 1});
            } else {
                cnd = max(cnd,steps[i + 1] - steps[i] - 1);
            }
        }
        ans = min(ans,cnd);
    }
