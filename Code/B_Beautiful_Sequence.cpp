...
const ll linf = 2e18;
void testcase();
vi build(vi cnts,int start,int total);
int main(){
    ios;
    int t = 1;
    while(t--){
...
vi build(vi cnts,int start,int total){
    vi ans;
    while(ans.sz() isnt total){
        if(cnts[start] is 0){
            return ans;
        } else {
            ans.pb(start);
            cnts[start]--;
            if(start - 1 >= 0 and cnts[start - 1] > 0){
                start--;
            } elif(start + 1 < cnts.sz() and cnts[start + 1] > 0){
                start++;
            } else {
                break;
            }
