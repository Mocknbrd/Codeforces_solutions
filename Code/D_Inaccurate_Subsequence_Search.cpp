...
void testcase(){
    ...
    ll curr = 0;
    inc(i,0,m){
        if(bvalues.count(a[i])){
            avalues[a[i]]++;
            curr += (avalues[a[i]] <= bvalues[a[i]]);
        }
    }
    ans += (curr >= k);
    // debug(0 << " " << k - 1 << " " << curr);
    slid_win(left,right,m,n){
        // debug(left << " " << right << " " << curr);
        if(bvalues.count(a[left - 1])){
            avalues[a[left - 1]]--;
            curr -= (avalues[a[left - 1]] < bvalues[a[left - 1]]);
        }
        if(bvalues.count(a[right])){
            avalues[a[right]]++;
            curr += (avalues[a[right]] <= bvalues[a[right]]);
        }
        ans += (curr >= k);
    }
    see(ans);
