...
const ll linf = 2e18;
void testcase();
ll findAns(vll &height,ll target);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    readArray(height);
    sorted(height);
    ll maxi = *max_element(all(height));
    see(min(findAns(height,maxi),findAns(height,maxi + 1)));
    return;
}
ll findAns(vll &height,ll target){
    ll ans = linf,start = 0,end = linf;
    while(start <= end){
        ll mid = (start + end) >> 1;
        ll even = mid >> 1,odd = mid - even;
        ll ops = 0;
        inc(i,0,height.sz()){
            ll value = height[i];
            ll evenops = (target - value) >> 1;
            value += 2 * min(evenops,even);
            even -= min(evenops,even);
            ops += target - value;
        }
        if(ops <= odd){
            ans = mid;
