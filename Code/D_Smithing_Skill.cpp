...
void testcase(){
    ...
    inc(i,0,n){
        pq.push(mp(a[i] - b[i],mp(a[i],b[i])));
    }
    ll mini = *min_element(all(a));
    ll ans = 0;
    while(pq.empty() is false and ingots.empty() is false and mini <= ingots.top()){
        while(pq.empty() is false and pq.top().sc.fi > ingots.top()){
            pq.pop();
        }
        while(pq.empty() is false and ingots.empty() is false and ingots.top() >= pq.top().sc.fi){
            ll ingot = ingots.top(); ingots.pop();
            ll count = 1 + (ingot - pq.top().sc.fi) / pq.top().fi;
            ans += 2ll * count;
            ingot -= pq.top().fi * count;
            if(ingot >= mini){
                ingots.push(ingot);
            }
        }
