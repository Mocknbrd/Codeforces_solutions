...
const ll linf = 2e18;
void testcase();
int findSize(vvi &tree,int vertex,int parent,int size,int &ans);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    int ans = 1,start = 1,end = n;
    while(start <= end){
        int size = (start + end) >> 1;
        int cnd = 0;
        findSize(tree,1,0,size,cnd);
        if(cnd >= k + 1){
            ans = size;
            start = size + 1;
        } else {
...
int findSize(vvi &tree,int vertex,int parent,int size,int &ans){
    ...
    each(child,tree[vertex]){
        if(child isnt parent){
            subtree += findSize(tree,child,vertex,size,ans);
        }
    }
    if(subtree >= size){
        subtree = 0;
