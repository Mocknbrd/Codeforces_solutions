...
void testcase(){
    ...
    rsort(neg);
    while(neg.sz() > 1){
        int first = neg.bk().sc; neg.rb();
        int second = neg.bk().sc; neg.rb();
        see(1 << " " << first << " " << second);
        pos.pb(second);
    }
    if(neg.sz()){
    ...
    while(zeros.sz() > 1){
        int first = zeros.bk(); zeros.rb();
        int second = zeros.bk(); zeros.rb();
        see(1 << " " << first << " " << second);
        zeros.pb(second);
    }
    if(zeros.sz() and pos.empty() is false){
        see(2 << " " << zeros.bk());
    }
    while(pos.sz() > 1){
        int first = pos.bk(); pos.rb();
        int second = pos.bk(); pos.rb();
        see(1 << " " << first << " " << second);
        pos.pb(second);
    }
    return;
