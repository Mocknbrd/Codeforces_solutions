...
void testcase(){
    ...
    if(k >= 3){
        ...
        if(k is 2) {
            inc(i,0,n){
                inc(j,0,n){
                    ll curr = abs(arr[j] - arr[i]);
                    auto upper = upper_bound(all(arr),curr);
                    if(upper isnt arr.end()){
                        ll value = *upper;
                        ans = min(ans,abs(curr - value));
                    }
                    if(upper isnt arr.begin()){
                        ll value = *--upper;
                        ans = min(ans,abs(curr - value));
                    }
                }
