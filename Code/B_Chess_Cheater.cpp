...
#define pair(tmp1,tmp2) pair<tmp1,tmp2>
...
void testcase(){
    ...
    each(gap,gaps){
        pq.push({gap.s - gap.f + 1,{gap.f,gap.s}});
    }
    while(pq.empty() is false){
        pair(int,pii)curr = pq.top(); pq.pop();
        inc(i,curr.s.f,curr.s.s + 1){
            if(k > 0){
                res[i] = 'W';
                k--;
            }
