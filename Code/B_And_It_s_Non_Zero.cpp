...
int main(){
    ...
    int t = 1;
    cin >> t;
    prefix = vvll(2e5 + 1,vll(21,0));
    inc(value,1,prefix.sz()){
        inc(pos,0,21){
            prefix[value][pos] = prefix[value - 1][pos] + ((value >> pos) & 1);
        }
    }
    while(t--){
        testcase();
...
void testcase(){
    ...
    ll ans = -inf;
    inc(pos,0,21){
        ans = max(ans,prefix[right][pos] - prefix[left - 1][pos]);
    }
    see(right - left + 1 - ans);
    return;
}