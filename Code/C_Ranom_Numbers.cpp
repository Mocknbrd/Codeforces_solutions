...
const ll linf = 2e18;
void testcase();
int solve(string &s,vt(vvi)&dp,int index,bool changed,int maxi);
int getNum(char ch);
int main(){
    ios;
    int t = 1;
...
void testcase(){
    string s;
    cin >> s;
    vt(vvi)dp(s.sz(),vvi(2,vi(5,-inf)));
    see(solve(s,dp,s.sz() - 1,false,0));
    return;
}
int solve(string &s,vt(vvi)&dp,int index,bool changed,int maxi){
    if(index is -1){
        return 0;
    } elif(dp[index][changed][maxi] isnt -inf){
        return dp[index][changed][maxi];
    } else {
        int ans1 = -inf,ans2 = -inf;
        ans1 = getNum(s[index]) * (1 - 2 * (s[index] - 'A' < maxi)) + solve(s,dp,index - 1,changed,max(maxi,s[index] - 'A'));
        if(changed is false){
            inc(next,0,5){
                ans2 = max(ans2,getNum('A' + next) * (1 - 2 * (next < maxi)) + solve(s,dp,index - 1,true,max(next,maxi)));
            }
        }
        return dp[index][changed][maxi] = max(ans1,ans2);
    }
}
int getNum(char ch){
    return power(10,ch - 'A');
}
