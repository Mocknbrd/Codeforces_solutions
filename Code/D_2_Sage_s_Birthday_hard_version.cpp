...
void testcase(){
    ...
    while(start <= end){
        ...
        vi cnd;
        while(small.empty() is false and large.empty() is false){
            cnd.pb(large.back()); large.rb();
            cnd.pb(small.back()); small.rb();
        }
        while(small.empty() is false){
            cnd.pb(small.back());
            small.rb();
        }
        while(large.empty() is false){
            cnd.pb(large.back());
            large.rb();
        }
        int count = 0;
        inc_la(i,1,n,1){
            count += cnd[i] < cnd[i - 1] and  cnd[i] < cnd[i + 1];
        }
        if(count >= mid){
            ans = mid;
            res = cnd;
