...
void testcase(){
    ...
    inc(pos,0,31){
        ll curr = 0,x = 0;
        vll sum(2,0),counts(2,0);
        counts[0] = 1;
        inc(i,0,n){
            x ^= ((arr[i] >> pos) & 1);
            ll right = mod(counts[x ^ 1] * (i + 1),md);
            curr = mod(curr + right - sum[x ^ 1],md);
            counts[x] = mod(counts[x] + 1,md);
            sum[x] = mod(sum[x] + i + 1,md);
        }
        curr = mod(curr * (1ll << pos),md);
        ans = mod(ans + curr,md);
    }
    see(ans);
    return;
