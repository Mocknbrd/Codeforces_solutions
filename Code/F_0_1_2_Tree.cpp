...
const ll linf = 2e18;
void testcase();
int getVertex(int &a,int &b,int &c){
    if(a isnt 0){
        a--;
        return 2;
    } elif(b isnt 0){
...
void testcase(){
    ...
    queue<pii>q;
    int ans = 0;
    q.push(mp(getVertex(a,b,c),0));
    while(q.empty() is false){
        pii curr = q.front(); q.pop();
        int id = curr.fi,height = curr.sc;
        ans = max(ans,height);
        if(id is 2){
            int left = getVertex(a,b,c),right = getVertex(a,b,c);
            if(right is -1){
                see(-1);
                return;
            }
            q.push(mp(left,height + 1));
            q.push(mp(right,height + 1));
        } elif(id is 1){
            int child = getVertex(a,b,c);
            if(child is -1){
                see(-1);
                return;
            }
            q.push(mp(child,height + 1));
        }
    }
    if(a isnt 0 or b isnt 0 or c isnt 0){
        see(-1);
    } else {
        see(ans);
    }
