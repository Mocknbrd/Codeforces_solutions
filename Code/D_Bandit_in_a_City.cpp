...
const ll linf = 2e18;
void testcase();
void dfs(vvll &tree,vll &arr,vll &sum,vll &count,int vertex);
int main(){
    ios;
    int t = 1;
    while(t--){
...
void testcase(){
    ...
    vvll tree(n + 1);
    inc(i,0,parent.sz()){
        tree[parent[i]].pb(i + 2);
    }
    vll arr(n + 1,0);
    readArray(arr,1);
    vll sum(n + 1,0),count(n + 1,0);
    dfs(tree,arr,sum,count,1);
    ll ans = -linf;
    inc(i,1,sum.sz()){
        ans = max(ans,ceil(sum[i],count[i]));
    }
    see(ans);
    return;
}
void dfs(vvll &tree,vll &arr,vll &sum,vll &count,int vertex){
    if(tree[vertex].empty() is true){
        sum[vertex] = arr[vertex];
        count[vertex] = 1;
    } else {
        sum[vertex] = arr[vertex];
        each(child,tree[vertex]){
            dfs(tree,arr,sum,count,child);
