...
void testcase(){
    ...
    char trump;
    cin >> trump;
    vt(string) trumpSuite;
    map(char,vt(string))clubs;
    vt(pair(string,string))ans;
    inc(i,0,2 * n){
        string card;
        cin >> card;
        char key = card.back();
        if(clubs[key].empty() is false and key isnt trump){
            string f = min(card,clubs[key].back());
            string s = max(card,clubs[key].back());
            ans.pb(mp(f,s));
            clubs[key].rb();
        } else {
            if(key is trump){
    ...
    each(entry,clubs){
        while(entry.sc.empty() is false){
            if(trumpSuite.empty() is true){
                see("IMPOSSIBLE");
                return;
            } 
            ans.pb(mp(entry.sc.back(),trumpSuite.back()));
            entry.sc.rb();
            trumpSuite.rb();
        }
    }
    if(mod(cast(trumpSuite.sz(),int),2) is 1){
        see("IMPOSSIBLE");
        return;
    }
    while(trumpSuite.empty() is false){
        string first = trumpSuite.back(); trumpSuite.rb();
        string second = trumpSuite.back(); trumpSuite.rb();
        ans.pb(mp(min(first,second),max(first,second)));
    }
    each(cnd,ans){
        see(cnd.fi << " " << cnd.sc);
    }
