...
void testcase(){
    ...
    inc(first,1,n){
        inc(second,first + 1,n){
            int third = (first ^ second);
            if(third > n or first + second <= third or first + third <= second or second + third <= first){
                continue;
            } else {
                vi cnd;
                cnd.pb(first); cnd.pb(second); cnd.pb(third);
                sorted(cnd);
                if(seen[cnd[0]][cnd[1]] is false){
                    seen[cnd[0]][cnd[1]] = true;
                }
            }
        }
    }
