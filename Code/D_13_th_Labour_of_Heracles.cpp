...
void testcase(){
    ...
    max_heap(pair(ll,pll))pq;
    each(entry,tree){
        if(entry.s.sz() is 1){
            pq.push({weights[entry.f] + weights[*entry.s.begin()],{entry.f,*entry.s.begin()}});
        }
    }
    inc(i,1,weights.sz()){
        pair(ll,pll) curr = pq.top(); pq.pop();
        pll edge = curr.s;
        ll add = weights[edge.s];
        ans[i] = ans[i - 1] + add;
        tree[edge.f].erase(edge.s);
        tree[edge.s].erase(edge.f);
        if(tree[edge.s].sz() is 1){
            pq.push({weights[edge.s] + weights[*tree[edge.s].begin()],{edge.s,*tree[edge.s].begin()}});
        }
    }
    inc(i,0,n - 1){
        cout << ans[i] << " ";
