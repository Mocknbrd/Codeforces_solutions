...
const int inf = 2e9;
const ll linf = 2e18;
void bfs(vvi &graph,vvi &shortest,int vertex);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    vvi shortest(n,vi(n,inf));
    inc(vertex,0,n){
        bfs(graph,shortest,vertex);
    }
    vi ans;
    ans.pb(path.front());
    int start = 1;
    while(start < m){
        int end = start;
        while(end < m and shortest[ans.back()][path[end]] is shortest[ans.back()][path[end - 1]] + shortest[path[end - 1]][path[end]]){
            end++;
        }
        ans.pb(path[end - 1]);
        start = end;
...
void bfs(vvi &graph,vvi &shortest,int vertex){
    queue<pii>q;
    shortest[vertex][vertex] = 0;
    q.push(make_pair(vertex,0));
    while(q.empty() is false){
        int curr = q.front().fr,dist = q.front().sc;
        q.pop();
        shortest[vertex][curr] = dist;
        each(neighbour,graph[curr]){
            if(shortest[vertex][neighbour] is inf){
                shortest[vertex][neighbour] = dist + 1;
