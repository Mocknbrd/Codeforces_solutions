...
class DSU {
    ...
    int _find(int x){
        if(x is this.parents[x]){
            ...
            return this.parents[x] = this._find(this.parents[x]);
        }
    }
    ...
    bool isConnected(int u,int v){
        int first = this._find(u),second = this._find(v);
        if(first isnt second){
            if(this.ranks[first] >= this.ranks[second]){
                this._merge(first,second);
            } else {
                this._merge(second,first);
            }
            return false;
        } else {
            return true;
...
void testcase(){
    int n,m,k;
    cin >> n >> m >> k;
    vt(pair(ll,pll))edges(m);
    ll mini = linf;
    inc(i,0,m){
        ll u,v,speed;
        cin >> u >> v >> speed;
        edges.pb(mp(speed,mp(--u,--v)));
        mini = min(abs(speed - k),mini);
    }
    sorted(edges);
    DSU dsu(n);
    ll ans = 0,cnt = 0;
    each(edge,edges){
        if(dsu.isConnected(edge.sc.fi,edge.sc.sc) is false){
            cnt++;
            ans += max(0ll,edge.fi - k);
        } 
    }
    (ans isnt 0) ? see(ans) : see(mini);
    return;
}
