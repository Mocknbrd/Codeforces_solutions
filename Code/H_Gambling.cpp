...
void testcase(){
    ...
    vi arr(n);
    readArray(arr);
    vi compressed = coordinateCompress(arr);
    map(int,int)revmapping;
    inc(i,0,n){
        revmapping[compressed[i]] = arr[i];
    }
    map(int,vi)pos;
    inc(i,0,n){
        pos[compressed[i]].pb(i);
    }
    int ans = compressed[0],left = 0,right = 0,mxprofit = 1;
    each(entry,pos){
        int value = entry.fr;
        int begin = entry.sc.front(),profit = 1;
        inc(i,1,entry.sc.sz()){
            profit++;
            profit -= entry.sc[i] - entry.sc[i - 1] - 1;
            if(profit <= 0){
                begin = entry.sc[i];
                profit = 1;
            }
    ...
    see(revmapping[ans] << " " << left + 1 << " " << right + 1);
    return;
}
