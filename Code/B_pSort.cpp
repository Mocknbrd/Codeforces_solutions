...
class DSU {
    ...
    public:  
    DSU(int n){
        this.parents = vi(n + 1,0),this.ranks = vi(n + 1,1);
        iota(all(this.parents),0);
    }
    int find(int vertex){
        vi stack;
        while(vertex isnt this.parents[vertex]){
            stack.pb(vertex);
            vertex = this.parents[vertex];
        }
        while(stack.empty() is false){
            this.parents[stack.back()] = vertex;
            stack.rb();
        }
        return vertex;
    }
    void findUnion(int x,int y){
        ...
        if(parx isnt pary){
            (this.ranks[parx] > this.ranks[pary] ? this.merge(parx,pary) : this.merge(pary,parx));
        }
    }
...
void testcase(){
    ...
    DSU dsu(n);
    inc(i,1,target.sz()){
        if(i + target[i] < target.sz()){
            dsu.findUnion(i,i + target[i]);
        } 
        if(i - target[i] > 0){
            dsu.findUnion(i,i - target[i]);
        }
    ...
    inc(vertex,1,n + 1){
        if(dsu.isConnected(vertex,arr[vertex]) is false){
            NN;
            return;
        }
    }
