...
const ll linf = 2e18;
void testcase();
bool is_bus(vi &degree);
bool is_start(vi &degree);
bool is_network(vi &degree);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    if(is_bus(degree) is true){
        see("bus topology");
    } elif(is_start(degree) is true){
        see("star topology");
    } elif(is_network(degree) is true){
        see("ring topology");
    } else {
        see("unknown topology");
    }
    return;
}
bool is_bus(vi &degree){
    ...
    inc(vertex,1,degree.sz()){
        ones += degree[vertex] is 1;
        twos += degree[vertex] is 2;
    }
    return ones is 2 and twos is n - 2;
}
bool is_start(vi &degree){
    int n = degree.sz() - 1;
    int ones = 0,maxi = 0;
    inc(vertex,1,degree.sz()){
        ones += degree[vertex] is 1;
        maxi += degree[vertex] is n - 1;
    }
    return ones is n - 1 and maxi is 1;
}
bool is_network(vi &degree){
    int n = degree.sz() - 1;
    return cnt(degree,2) is n;
}
