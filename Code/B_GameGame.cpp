...
const ll linf = 2e18;
void testcase();
void solve(vi &ones,vi &zeros,int curr);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    int n;
    cin >> n;
    vi ones(31,0),zeros(31,0);
    inc(i,0,n){
        int value;
        cin >> value;
        inc(pos,0,31){
            ones[pos] += ((value >> pos) & 1) is 1;
            zeros[pos] += ((value >> pos) & 1) is 0;
        }
    }
    solve(ones,zeros,30);
    return;
...
void solve(vi &ones,vi &zeros,int curr){
    if(curr is -1){
        see("DRAW");
    } elif(mod(ones[curr],2) is 0){
        solve(ones,zeros,curr - 1);
    } else {
        if(mod(ones[curr],4) is 3 and mod(zeros[curr],2) is 0){
            see("LOSE");
        } else {
            see("WIN");
        }
