...
class Node {
    ...
    ll value;
    Node *left,*right;
    Node(int start,int end,ll value){
        self.start = start;
        self.end = end;
        self.value = value;
        self.left = nullptr;
...
class SegTree {
    private:
    Node *root;
    Node* __build(vll &arr,int start,int end){
        if(start is end){
            ...
            Node *node = new Node(start,end,left->value + right->value);
            node->left = left;
            node->right = right;
            return node;
        }
    }
    ll __query(Node *node,int start,int end){
    ...
    void __update(Node *node,ll oldValue,ll newValue,int pos){
        if(!node or pos > node->end or pos < node->start){
            return;
        } elif(pos >= node->start and pos <= node->end){
            node->value += newValue - oldValue;
        }
        self.__update(node->left,oldValue,newValue,pos);
        self.__update(node->right,oldValue,newValue,pos);
    }
    public:
    SegTree(vll &arr){
    ...
    void update(ll oldValue,ll newValue,int pos){
        self.__update(self.root,oldValue,newValue,pos);
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    int ans = 0;
    {
        max_heap(pll)pq;
        dec(i,m - 1,0){
            while(tree.query(0,i) < tree.query(0,m - 1) and pq.empty() is false){
                pll curr = pq.top(); pq.pop();
                tree.update(curr.fr,-curr.fr,curr.sc);
                ans++;
            }
