...
void testcase(){
    ...
    ll ans = min(solve(arr,even),solve(arr,odd));
    ans = (ans is linf ? -1 : ans);
    see(ans);
    return;
}
ll solve(vi src,vi &target){
    ...
    ll ans = 0;
    while(start < src.sz()){
        if(src[start] isnt target[start]){
            if(target[start] is 1){
                auto it = one.upper_bound(start);
                if(it is one.end()){
                    return linf;
                } else {
                    ans += abs(start - *it);
                    swap(src[start],src[*it]);
                    zero.ins(*it);
                    one.erase(it);
                    zero.erase(start);
                }
            } else {
                auto it = zero.upper_bound(start);
                if(it is zero.end()){
                    return linf;
                } else {
                    ans += abs(start - *it);
