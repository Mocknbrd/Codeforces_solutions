...
const ll linf = 2e18;
void testcase();
bool dfs(string &b,string &curr,vi &cnts,bool smaller);
int main(){
    ios;
    int t = 1;
    while(t--){
...
void testcase(){
    ...
    if(a.sz() < b.sz()){
        ...
        string curr = "";
        dfs(b,curr,cnts,false);
        see(curr);
    }
    return;
}
bool dfs(string &b,string &curr,vi &cnts,bool smaller){
    if(*max_element(all(cnts)) is 0){
        return (stoll(curr) <= stoll(b));
    } else {
        int begin = (smaller is true) ? 9 : b[curr.sz()] - '0';
        dec(digit,begin,0){
            if(cnts[digit] > 0){
                curr.pb('0' + digit);
                cnts[digit]--;
                if(dfs(b,curr,cnts,smaller or (curr.bk() < '0' + begin)) is true){
                    return true;
                }
                cnts[digit]++;
                curr.rb();
