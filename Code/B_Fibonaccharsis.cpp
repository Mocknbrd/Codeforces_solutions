...
void testcase(){
    ...
    if(k <= 2){
        ...
        k -= 2;
        pll coeff1 = {1,0},coeff2 = {0,1};
        while(k--){
            coeff = {coeff1.f + coeff2.f,coeff1.s + coeff2.s};
            if(coeff.f > n and coeff2.s > n){
                see(0);
                return;
            }
            coeff1 = coeff2;
    ...
    ll ans = 0;
    inc_type(ll,value,0,n + 1){
        ll first = coeff.f * value;
        ll second = n - first;
        if((second % coeff.s is 0) and second / coeff.s >= value){
            ans++;
        }
    }
    see(ans);
