...
void testcase(){
    int n;
    cin >> n;
    vvb reachable(n + 1,vb(n + 1,false));
    vvi dag(n + 1);
    vi degree(n + 1,0);
    set(pii)edges;
    inc(i,0,n * (n - 1) / 2 - 1){
    ...
    inc(vertex,1,n + 1){
        each(child,dag[vertex]){
            reachable[vertex][child] = true;
        }
    }
    inc(intermediate,1,n + 1){
        inc(starting,1,n + 1){
            inc(target,1,n + 1){
                if(reachable[starting][intermediate] and reachable[intermediate][target]){
                    reachable[starting][target] = true;
                }
            }
        }
    }
    inc(vertex,1,n + 1){
        inc(target,1,n + 1){
            if(vertex isnt target and reachable[vertex][target] is true and edges.count(mp(target,vertex)) is 0){
                see(target << " " << vertex);
                return;
            }
        }
    ...
    inc(starting,1,n + 1){
        inc(target,1,n + 1){
            if(starting isnt target and edges.count(mp(starting,target)) is 0 and edges.count(mp(target,starting)) is 0){
                see(starting << " " << target);
                return;
            }
        }
