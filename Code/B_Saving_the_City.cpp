...
class DSU{
    ...
    int __find(int x){
        if(this->parents[x] is x){
            ...
            return this->parents[x] = this->__find(this->parents[x]);
        }
    }
    public:
    DSU(int n){
    ...
    bool findUnion(int x,int y){
        int parx = this->__find(x);
        int pary = this->__find(y);
        if(parx isnt pary){
            if(this->ranks[parx] >= this->ranks[pary]){
                this->__merge(parx,pary);
            } else {
                this->__merge(pary,parx);
            }
            return true;
        }
        return false;
...
void testcase(){
    ...
    int start = 0;
    while(start < arr.sz()){
        while(arr[start] is '0'){
            start++;
        }
        if(start < arr.sz()){
            int end = start;
            while(end < arr.sz() and arr[end] is '1'){
                end++;
            }
            ones.pb({start,end - 1});
            start = end;
    ...
    vi zeros(arr.sz() + 1,0);
    inc(i,1,zeros.sz()){
        zeros[i] = zeros[i - 1] + (arr[i - 1] is '0');
    }
    int ans = ones.sz() * a;
    if(ones.sz() > 1){
        int ans1 = 0;
        DSU dsu(ones.sz());
        int size = ones.sz();
        inc(currindex,0,ones.sz()){
            int nextindex = mod(currindex + 1,size);
            pii curr = ones[currindex],next = ones[nextindex];
            if(nextindex > currindex){
                int connectionCost = b * (zeros[next.f + 1] - zeros[curr.s]);
                if(connectionCost <= a){
                    ans1 += dsu.findUnion(currindex,nextindex) * connectionCost;
                }
            } else {
                int connectionCost = b * (zeros.back() - zeros[next.s]) + b * zeros[curr.f + 1];
                if(connectionCost <= a){
                    ans1 += dsu.findUnion(currindex,nextindex) * connectionCost;
                }
            }
        }
        ans1 += dsu.findComponents() * a;
        ans = min(ans,ans1);
    }
    see(ans);
    return;
