...
void testcase(){
    ...
    cin >> n >> m >> k;
    vpll arr(k);
    min_heap(pair(pll,int))pq;
    inc(i,0,k){
        cin >> arr[i].fi >> arr[i].sc;
        pq.push(mp(mp(arr[i].sc,-arr[i].fi),i));
    }
    vi indices;
    indices.pb(pq.top().sc);
    ll row = abs(pq.top().fi.sc),col = pq.top().fi.fi;
    ll res = (col - 1) * row;
    pq.pop();
    while(pq.empty() is false){
        while(pq.empty() is false and abs(pq.top().fi.sc) <= row){
            pq.pop();
        }
        if(pq.empty() is false){
            indices.pb(pq.top().sc);
            ll nrow = abs(pq.top().fi.sc);
            ll ncol = abs(pq.top().fi.fi);
            pq.pop();
            res += (ncol - 1) * (nrow - row);
            col = ncol;
            row = nrow;
