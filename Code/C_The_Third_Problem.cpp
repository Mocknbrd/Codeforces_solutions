...
void testcase(){
    ...
    ll ans = 1;
    if(n isnt 1) {
        ll left = min(pos[0],pos[1]),right = max(pos[0],pos[1]),md = 1e9 + 7;
        inc_type(ll,value,2,n){
            if(inBetween(left,pos[value],right) is true){
                ans = mod(ans * (right - left + 1 - value),md);
            }
            left = min(left,pos[value]);
            right = max(right,pos[value]);
        }
    }
    see(ans);
    return;
