...
const ll linf = 2e18;
void testcase();
int solve(vvi &dp,vi &leftmost,vi &rightmost,int curr,int left,int &cols);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    readArray(floors);
    rev(floors);
    vi leftmost(n,0),rightmost(n,0);
    inc(i,0,n){
        int first = -1,last = -1;
        inc(j,0,floors[i].sz()){
            if(floors[i][j] is '1'){
                last = j;
                first = (first is -1 ? j : first);
            }
        }
        if(first isnt -1){
            rightmost[i] = last;
            leftmost[i] = m + 1 - first;
        }
    }
    vvi dp(n,vi(2,-1));
    see(solve(dp,leftmost,rightmost,0,1,m));
    return;
}
int solve(vvi &dp,vi &leftmost,vi &rightmost,int curr,int left,int &cols){
    if(curr >= leftmost.sz()){
        return 0;
    } elif(dp[curr][left] isnt -1){
        return dp[curr][left];
    } else {
        if(left is 1){
            int ans1 = rightmost[curr];
            int ans2 = cols + 1;
            int nxt = -1;
            inc(i,curr + 1,leftmost.sz()){
                if(leftmost[i] isnt 0){
                    nxt = i;
                    break;
                }
            }
            if(nxt isnt -1){
                ans1 += rightmost[curr] + nxt - curr + solve(dp,leftmost,rightmost,nxt,left,cols);
                ans2 += nxt - curr + solve(dp,leftmost,rightmost,nxt,left ^ 1,cols);
            }
            return dp[curr][left] = min(ans1,ans2);
        } else {
            int ans1 = leftmost[curr];
            int ans2 = cols + 1;
            int nxt = -1;
            inc(i,curr + 1,leftmost.sz()){
                if(leftmost[i] isnt 0){
            ...
            if(nxt isnt -1){
                ans1 += leftmost[curr] + nxt - curr + solve(dp,leftmost,rightmost,nxt,left,cols);
                ans2 += nxt - curr + solve(dp,leftmost,rightmost,nxt,left ^ 1,cols);
            }
            return dp[curr][left] = min(ans1,ans2);
        }
