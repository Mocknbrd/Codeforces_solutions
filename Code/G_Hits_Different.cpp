...
int main(){
    ...
    vvll values = dp;
    inc(i,1,size){
        dp[i][0] = values[i][0] * values[i][0] + dp[i - 1][0];
    }
    vll squareSums(size,0);
    inc(i,1,size){
        inc(j,1,size){
            if(dp[i][j] is 0){
                break;
            }
            dp[i][j] = values[i][j] * values[i][j] + squareSums[j] + dp[i - 1][j - 1];
            squareSums[j] += values[i][j] * values[i][j];
        }
    }
    int t = 1;
    cin >> t;
    while(t--){
        ...
        cin >> n;
        pii position = pos[n];
        see(dp[position.fi][position.sc]);
    }
    return 0;
}
