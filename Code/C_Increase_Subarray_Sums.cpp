...
void testcase();
void findMaxSumSegment(vll &arr,int length,map<int,ll>&lengthSumMap);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    vll arr(n);
    readArray(arr);
    map<int,ll>lengthSumMap;
    inc(length,1,n+1){
        findMaxSumSegment(arr,length,lengthSumMap);
    }
    inc(k,0,n+1){
        ll ans = -linf;
        inc(length,1,n+1){
            ll sum = lengthSumMap[length];
            ll add = (k <= length) ? k * x : length * x;
            ll total = sum + add;
            ans = max(ans,total);
        }
        cout << max(llzero,ans) << " ";
...
void findMaxSumSegment(vll &arr,int length,map<int,ll>&lengthSumMap){
    ...
    lengthSumMap[length] = sum;
    slid_win(start,end,length,arr.sz()){
        sum -= arr[start - 1];
        sum += arr[end];
        lengthSumMap[length] = max(lengthSumMap[length],sum);
    }
}