...
void testcase(){
    int n,m;
    cin >> n >> m;
    vi curr(n),target(n),painters(m);
    readArray(curr);
    readArray(target);
    readArray(painters);
    map<int,vi>diff,same;
    inc(i,0,n){
        if(curr[i] is target[i]){
            same[curr[i]].pb(i);
        } else {
            diff[target[i]].pb(i);
        }
    }
    if(diff.count(painters.back()) is false and same.count(painters.back()) is false){
        NN;
    } else {
        vi ans(m,-1);
        int sink = -1;
        if(diff.count(painters.back()) is true){
            sink = diff[painters.back()].front();
        } else {
            sink = same[painters.back()].front();
        }
        inc(i,0,m - 1){
            int color = painters[i];
            if(diff.count(color) is true){
                curr[diff[color].back()] = color;
                ans[i] = diff[color].back() + 1;
                diff[color].rb();
                if(diff[color].empty() is true){
                    diff.erase(color);
                }
            } elif(same.count(color) is true){
                ans[i] = same[color].back() + 1;
                same[color].rb();
                if(same[color].empty() is true){
                    same.erase(color);
                }
            } else {
                ans[i] = sink + 1;
