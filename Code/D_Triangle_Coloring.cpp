...
void testcase(){
    ...
    rng(i,0,n,3){
        set(ll)values;
        values.ins(weights[i]);
        values.ins(weights[i + 1]);
        values.ins(weights[i + 2]);
        if(values.sz() is 1){
            ans = mod(ans * 3,md);
        } elif(values.sz() is 2){
            int smaller = 0;
            inc(j,i,i + 3){
                smaller += (weights[j] is *values.begin());
            }
            if(smaller is 2){
                ans = mod(ans * 2,md);
            }
    ...
    inc_type(ll,value,1,n / 3 + 1){
        if(value <= n / 6){
            denominator = mod(denominator * value,md);
        }
        numerator = mod(numerator * value,md);
    }
    denominator = power(denominator,2ll,md);
    ll nCr = mod(numerator * moduloInv(denominator,md),md);
    ans = mod(ans * nCr,md);
    see(ans);
    return;
}
