...
void testcase(){
    ...
    vi arr(n);
    readArray(arr);
    vi coord = coordinateCompress(arr);
    map(int,int)mapping;
    inc(i,0,n){
        mapping[arr[i]] = coord[i];
    }
    vi first(n,-1),last(n,-1);
    inc(i,0,n){
        first[coord[i]] = (first[coord[i]] is -1 ? i : first[coord[i]]);
        last[coord[i]] = i;
    }
    while(k--){
        int a,b;
        cin >> a >> b;
        if(mapping.count(a) is false or mapping.count(b) is false or first[mapping[a]] > last[mapping[b]]){
            NN;
        } else {
            YY;
        }
