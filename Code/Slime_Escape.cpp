...
void testcase(){
    ...
    rev(right);
    rev(left);
    vll minleft,minright,leftval,rightval;
    each(range,left){
        ll value = -linf,curr = 0;
        dec(i,range.sc,range.fi){
            curr += arr[i];
    ...
    ll curr = arr[k - 1];
    while(left.empty() is false and right.empty() is false){
        if(minleft.bk() > curr and minright.bk() > curr){
            NN;
            return;
        } elif(minleft.bk() <= curr){
            curr += leftval.bk();
            leftval.rb();
            left.rb();
            minleft.rb();
