...
void testcase(){
    ...
    if(k is n - 1){
        ...
        ans.pb(mp(1,3));
        ans.pb(mp(n - 1,n - 2));
        taken[1] = true,taken[3] = true;
    } else {
        ans.pb(mp(k,n - 1));
    }
    taken[ans.back().fi] = true,taken[ans.back().sc] = true;
    dec(value,n - 2,0){
        if(taken[value] is false){
            int msb = findMsb(value);
            int target = 0;
            dec(pos,msb,0){
                int bit = (value >> pos) & 1;
                target ^= ((bit ^ 1) << pos);
            }
            if(taken[target] is false){
                ans.pb(mp(value,target));
                taken[target] = true;
                taken[value] = true;
            } elif(taken[0] is false) {
                ans.pb(mp(value,0));
                taken[0] = true;
                taken[value] = true;
            } else {
