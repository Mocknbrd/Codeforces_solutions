...
int main(){
    ...
    Input(int,t);
    map<char,pii>moves;
    moves['R']={1,0},moves['L']={-1,0},moves['U']={0,1},moves['D']={0,-1};
    while(t--){
        Input(string,s);
        pii origin={0,0};
        vpii states;
        ...
        each(dir,s){
            pii move=moves[dir];
            curr.first+=move.first;
            curr.second+=move.second;
            states.push_back(curr);
        }
        pii ans=origin;
        each(state,states){
            if(state==origin){
                ...
                each(dir,s){
                    pii move=moves[dir];
                    pii next={curr.first+move.first,curr.second+move.second};
                    if(next!=state){
                        curr=next;
                    }
                }
