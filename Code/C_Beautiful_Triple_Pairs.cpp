...
const ll linf = 2e18;
void testcase();
vi generateKey(vi &cnds,int pos);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    vi arr(n);
    readArray(arr);
    map(vi,map(int,int))mapping;
    inc_la(i,0,n,2){
        vi cnds;
        cnds.pb(arr[i]),cnds.pb(arr[i + 1]),cnds.pb(arr[i + 2]);
        inc(j,0,cnds.sz()){
            mapping[generateKey(cnds,j)][cnds[j]]++;
        }
    }
    ll ans = 0;
    each(element,mapping){
        ll sum = 0;
        each(value,element.sc){
            sum += value.sc;
        }
        each(value,element.sc){
            ans += value.sc * (sum - value.sc);
        }
    }
    see(ans / 2);
    return;
