...
void testcase(){
    ...
    map(ll,ll)mask;
    inc(i,0,n){
        if(mask.count(arr[i]) is false){
            mask[arr[i]] = makeMask(arr[i]);
        }
        arr[i] = mask[arr[i]];
    }
    set(ll)values;
...
ll makeMask(ll value){
    ...
    ll ans = 1;
    each(entry,primes){
        ans *= power(entry.fr,mod(entry.sc,2ll));
    }
    return ans;
}
