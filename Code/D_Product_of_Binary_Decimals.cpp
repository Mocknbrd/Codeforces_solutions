...
bool isBin(int n){
    ...
    inc(i,0,check.sz()){
        if(check[i] isnt '0' and check[i] isnt '1'){
            return false;
        }
    }
    return true;
...
int main(){
    ...
    inc(i,0,numbers.sz()){
        inc(j,0,numbers.sz()){
            if(cnds.count(numbers[i] * numbers[j]) is false and numbers[i] * numbers[j] <= 1e5){
                q.push(numbers[i] * numbers[j]);
            } 
            if(numbers[i] * numbers[j] <= 1e5){
                cnds.ins(numbers[i] * numbers[j]);
            }
        }
    }
    while(q.empty() is false){
        ll curr = q.front(); q.pop();
        if(cnds.count(curr) is false){
            cnds.ins(curr);
        }
        each(value,cnds){
            if(value * curr <= 1e5 and cnds.count(value * curr) is false){
                q.push(curr * value);
            }
        }
    }
