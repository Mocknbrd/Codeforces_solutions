...
const int inf = 2e9;
const ll linf = 2e18;
int dfs(vvi &graph,vb &seen,int vertex,int prev);
int bfs(vvi &graph,int start,int end);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    vb seen(graph.sz(),false);
    int cycle = dfs(graph,seen,v,0);
    int first = bfs(graph,v,cycle),second = bfs(graph,m,cycle);
    if(second > first){
        YY;
    } else {
        NN;
...
int dfs(vvi &graph,vb &seen,int vertex,int prev){
    ...
    each(neighbour,graph[vertex]){
        if(neighbour isnt prev){
            if(seen[neighbour] is true){
                return neighbour;
            } else {
                int ans = dfs(graph,seen,neighbour,vertex);
                if(ans isnt -1){
                    return ans;
                }
            }
...
int bfs(vvi &graph,int start,int end){
    ...
    while(q.empty() is false){
        ...
        if(curr.fi is end){
            ...
            each(neighbour,graph[curr.fi]){
                if(seen[neighbour] is false){
                    seen[neighbour] = true;
                    if(neighbour is end){
                        return curr.sc + 1;
                    }
                    q.push(mp(neighbour,curr.sc + 1));
                }
            }
        }
    }
