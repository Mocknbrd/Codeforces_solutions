...
void testcase(){
    ...
    if(n is 1){
        ...
        q.push(make_pair(make_pair(*marked.begin(),-1),0));
        int ans = 0;
        while(q.empty() is false){
            pair(pii,int)curr = q.front(); q.pop();
            int vertex = curr.fr.fr,parent = curr.fr.sc,dist = curr.sc;
            if(marked.count(vertex) is true){
                ans = max(ans,dist);
            }
            each(neighbour,tree[vertex]){
                if(neighbour isnt parent){
                    q.push(make_pair(make_pair(neighbour,vertex),dist + 1));
                }
            }
        }
        see(ceil(ans,2));
