...
void testcase(){
    ...
    vpll ranges;
    inc(i,0,n){
        ranges.pb(make_pair(time[i] - health[i] + 1,time[i]));
    }
    sorted(ranges);
    ll ans = 0;
    int start = 0;
    while(start < n){
        pll curr = ranges[start];
        int end = start;
        while(end < n and ranges[end].fr <= curr.sc){
            curr = make_pair(min(curr.fr,ranges[end].fr),max(curr.sc,ranges[end].sc));
            end++;
        }
        ll length = curr.sc - curr.fr + 1;
        ans += length * (length + 1) / 2ll;
        start = end;
    }
    see(ans);
    return;
