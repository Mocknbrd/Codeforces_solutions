...
const int inf = 2e9;
const ll linf = 2e18;
void divideAndConqueor(vll &arr,vll &prefix,set(ll)&possible,int start,int end);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    set(ll)possible;
    divideAndConqueor(arr,prefix,possible,0,n - 1);
    while(q--){
        ll sum;
        cin >> sum;
        possible.count(sum) isnt 0 ? yy : nn;
    }
    return;
}
void divideAndConqueor(vll &arr,vll &prefix,set(ll)&possible,int start,int end){
    possible.ins(prefix[end + 1] - prefix[start]);
    if(arr[start] is arr[end]){
        return;
    } else {
        ll mid = (arr[start] + arr[end]) >> 1ll;
        int ind = upper_bound(arr.begin() + start,arr.begin() + end,mid) - arr.begin();
        divideAndConqueor(arr,prefix,possible,start,ind - 1);
        divideAndConqueor(arr,prefix,possible,ind,end);
    }
}
