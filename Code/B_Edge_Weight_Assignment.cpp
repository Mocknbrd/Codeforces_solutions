...
const int inf = 2e9;
const ll linf = 2e18;
void dfs(vvi &tree,vi &distances,int vertex,int parent,int curr);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    vi distances(n + 1);
    dfs(tree,distances,1,0,0);
    set(int)parity;
    inc(vertex,1,tree.sz()){
        if(tree[vertex].sz() is 1){
            parity.ins(mod(distances[vertex],2));
        }
    }
    int mini = (parity.sz() is 2) ? 3 : 1;
    int maxi = n - 1,leaves = 0,neighbour = 0;
    inc(vertex,1,tree.sz()){
        leaves += tree[vertex].sz() is 1;
        each(child,tree[vertex]){
            if(tree[child].sz() is 1){
                neighbour++;
                break;
            }
        }
    }
    maxi += neighbour - leaves;
    see(mini << " " << maxi);
    return;
}
void dfs(vvi &tree,vi &distances,int vertex,int parent,int curr){
    distances[vertex] = curr++;
    each(child,tree[vertex]){
        if(child isnt parent){
            dfs(tree,distances,child,vertex,curr);
        }
    }
    return;
}
