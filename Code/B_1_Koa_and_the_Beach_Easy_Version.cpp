...
const int inf = 2e9;
const ll linf = 2e18;
bool dfs(vi &arr,vi &tide,pii state,int k,int l,vvi &dp);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    vvi dp(arr.sz(),vi(tide.sz(),-1));
    if(dfs(arr,tide,mp(0,0),k,l,dp) is true){
        yy;
    } else {
        nn;
    }
...
bool dfs(vi &arr,vi &tide,pii state,int k,int l,vvi &dp){
    if(state.fi is arr.sz()){
        return true;
    } elif(dp[state.fi][state.sc] isnt -1){
        return dp[state.fi][state.sc];
    } elif(arr[state.fi] + tide[state.sc] > l){
        return dp[state.fi][state.sc] = false;
    } else {
        bool ans1 = dfs(arr,tide,mp(state.fi,mod(state.sc + 1,2 * k)),k,l,dp);
        bool ans2 = dfs(arr,tide,mp(state.fi + 1,mod(state.sc + 1,2 * k)),k,l,dp);
        return dp[state.fi][state.sc] = (ans1 or ans2);
    }
}
