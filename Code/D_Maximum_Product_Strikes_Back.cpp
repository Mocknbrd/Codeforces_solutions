...
const int inf = 2e9;
const ll linf = 2e18;
ll findRightSameParity(vll &prefix,int left,int right,int parity);
ll bs(vll &arr,int start,int end,ll value);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    twos.pb(0);
    each(value,arr){
        twos.pb(twos.back() + (abs(value) is 2));
        negative.pb(negative.back() + (value < 0));
    }
    int front = n,back = 0,count = 0;
    int start = 0;
    while(start < n){
        int end = start;
        ...
        inc(i,start,end){
            int right = findRightSameParity(negative,i,end,mod(negative[i],2ll));
            int tmpCnt = twos[right] - twos[i];
            if(tmpCnt > count){
                front = i;
                back = n - right;
                count = tmpCnt;
...
ll findRightSameParity(vll &prefix,int curr,int right,int parity){
    ll start = 0,end = inf,ans = curr;
    while(start <= end){
        ll mid = (start + end) >> 1ll;
        ll value = 2 * mid + parity;
        int ind = bs(prefix,curr,right,value);
        if(ind is -1){
            start = mid + 1;
        } elif(prefix[ind] < value){
            end = mid - 1;
        } else {
            ans = ind;
            start = mid + 1;
