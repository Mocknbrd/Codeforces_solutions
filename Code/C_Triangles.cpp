...
void testcase(){
    ...
    vec(string)arr(n);
    readArray(arr);
    vi maxrow(10,0),minrow(10,inf),maxcol(10,0),mincol(10,inf),ans(10,0);
    inc(i,0,n){
        inc(j,0,n){
            int d = arr[i][j] - '0';
    ...
    inc(i,0,n){
        inc(j,0,n){
            int d = arr[i][j] - '0';
            {
                int base = max(i,n - 1 - i);
                int height = max(maxcol[d] - j,j - mincol[d]);
                ans[d] = max(ans[d],base * height);
            }
            {
                int base = max(j,n - 1 - j);
                int height = max(maxrow[d] - i,i - minrow[d]);
                ans[d] = max(ans[d],base * height);
            }
        }
    }
