...
class SegTree {
    private:
    Node *root;
    Node *build(vll &arr,int start,int end){
        Node *node;
        if(start is end){
...
void testcase(){
    ...
    inc(i,0,n){
        ...
        if(r > l){
            leftRange.pb(mp(curr.fr(),curr.bk()));
            curr.clear();
        }
    }
    curr.clear();
    dec(i,n - 1,0){
        ...
        if(l > r){
            rightRange.pb(mp(curr.fr(),curr.bk()));
            curr.clear();
        }
    }
    sorted(rightRange);
    vll leftPrefix(n,0),rightPrefix(n,0);
    inc(index,0,leftRange.sz()){
        inc(i,leftRange[index].fi,leftRange[index].sc){
            leftPrefix[i] = 1;
        }
        if(index + 1 < leftRange.sz()){
            leftPrefix[leftRange[index].sc] = abs(pos[leftRange[index].sc] - pos[leftRange[index + 1].fi]);
        }
    }
    dec(index,rightRange.sz() - 1,0){
        pll range = rightRange[index];
        dec(i,range.fi,range.sc + 1){
            rightPrefix[i] = 1;
        }
        if(index - 1 >= 0){
            rightPrefix[range.sc] = abs(pos[range.sc] - pos[rightRange[index - 1].fi]);
        }
    }
    SegTree leftTree(leftPrefix),rightTree(rightPrefix);
    int m;
    cin >> m;
    while(m--){
        int s,e;
        ...
        s--,e--;
        if(s < e){
            see(leftTree.find(s,n - 1) - leftTree.find(e,n - 1));
        } else {
            see(rightTree.find(0,s) - rightTree.find(0,e));
        }
    }
    return;
}
