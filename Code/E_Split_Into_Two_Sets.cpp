...
const ll linf = 2e18;
void testcase();
bool bfs(vvi &graph,vb &seen,int start);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    inc(vertex,1,graph.sz()){
        if(seen[vertex] is false){
            if(bfs(graph,seen,vertex) is false){
                NN;
                return;
            }
        }
...
bool bfs(vvi &graph,vb &seen,int start){
    ...
    while(q.empty() is false){
        ...
        color[curr.fi] = curr.sc;
        seen[curr.fi] = true;
        each(neighbour,graph[curr.fi]){
            if(color[neighbour] is -1){
                color[neighbour] = (curr.sc ^ 1);
                seen[neighbour] = true;
                q.push(mp(neighbour,color[neighbour]));
            } else {
                if(color[neighbour] isnt (curr.sc ^ 1)){
                    return false;
                }
            }
        }
