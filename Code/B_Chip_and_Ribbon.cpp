...
class Node {
    ...
    ll mini;
    Node *left,*right;
    Node(int start,int end,ll mini){
        this.start = start;
        this.end = end;
        this.mini = mini;
        this.left = nullptr;
...
void testcase();
ll findAns(vll &arr,SegmentTree &tree,int start,int end,map(ll,vi)&pos,ll previous);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    see(findAns(arr,tree,0,n - 1,pos,0) - 1);
    return;
}
ll findAns(vll &arr,SegmentTree &tree,int start,int end,map(ll,vi)&pos,ll previous){
    if(start > end){
        ...
        return arr[start] - previous;
    } else {
        ll mini = tree.query(start,end);
        ll ans = 0;
        int prev = start;
        int s = lower_bound(all(pos[mini]),start) - pos[mini].bg();
        inc(i,s,pos[mini].sz()){
            if(pos[mini][i] <= end){
                int value = pos[mini][i];
                ans += findAns(arr,tree,prev,value - 1,pos,mini);
                prev = value + 1;
            } else {
                break;
            }
        }
        ans += findAns(arr,tree,prev,end,pos,mini);
        ans += mini - previous;
        return ans;
    }
}
