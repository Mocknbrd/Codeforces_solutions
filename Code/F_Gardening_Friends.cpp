...
const ll linf = 2e18;
void testcase();
ll findHeight(vvll &tree,vll &height,int vertex,int parent);
void findDiameter(vvll &tree,vll &diameter,vll &height,int vertex,int parent);
void findDepth(vvll &tree,vll &depth,int vertex,int parent,int curr);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    vll height(n + 1,0),depth = height,diameter = height;
    findHeight(tree,height,1,0);
    findDepth(tree,depth,1,0,0);
    findDiameter(tree,diameter,height,1,0);
    return;
}
ll findHeight(vvll &tree,vll &height,int vertex,int parent){
    each(child,tree[vertex]){
        if(child isnt parent){
            height[vertex] = max(height[vertex],1 + findHeight(tree,height,child,vertex));
        }
    }
...
void findDepth(vvll &tree,vll &depth,int vertex,int parent,int curr){
    ...
    each(child,tree[vertex]){
        if(child isnt parent){
            findDepth(tree,depth,child,vertex,curr + 1);
        }
    }
}
void findDiameter(vvll &tree,vll &diameter,vll &height,int vertex,int parent){
    ...
    each(child,tree[vertex]){
        if(child isnt parent){
            findDiameter(tree,diameter,height,child,vertex);
            if(pq.sz() < 2){
                pq.push(mp(height[child],child));
            } elif(pq.top().fi < height[child]){
                pq.pop();
                pq.push(mp(height[child],child));
            }
        }
    ...
    if(pq.sz() is 2){
        ll second = pq.top().sc; pq.pop();
        ll first = pq.top().sc; pq.pop();
        diameter[vertex] = height[first] + height[second] + 2;
    } else {
        diameter[vertex] = height[vertex];
    }
}
