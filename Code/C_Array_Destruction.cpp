...
const ll linf = 2e18;
void testcase();
vpii construct(map(int,int)values,int x);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    inc(i,0,arr.sz() - 1){
        int x = arr.back() + arr[i];
        vpii cnd = construct(values,x);
        if(cnd.sz() is n){
            YY;
            see(x);
            each(step,cnd){
...
vpii construct(map(int,int)values,int x){
    ...
    while(values.empty() is false){
        int first = (*values.rbegin()).fr;
        int second = x - first;
        if((first is second and values[first] < 2) or (values.count(second) is false)){
            return ans;
        } else {
            x = max(first,second);
            ans.pb(make_pair(first,second));
            if(--values[first] is 0){
                values.erase(first);
            }
            if(--values[second] is 0){
                values.erase(second);
            }
        }
    }
