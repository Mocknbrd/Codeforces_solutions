...
class DSU {
    ...
    vi ranks,parents;
    int extra;
    void __merge(int larger,int smaller){
        self.parents[smaller] = larger;
        self.ranks[larger] += self.ranks[smaller];
    }
    int __find(int x){
        if(self.parents[x] is x){
            return x;
        } else {
    ...
    DSU(int n){
        self.extra = 0;
        self.ranks = vi(n + 1,1);
        self.parents = vi(n + 1,0);
        inc(i,1,self.parents.sz()){
            self.parents[i] = i;
        }
    }
    ...
    int query(){
        vi cnds;
        inc(i,1,self.parents.sz()){
            if(i is self.parents[i]){
                cnds.pb(self.ranks[i]);
            }
        }
        rsort(cnds);
        int ans = 0;
        inc(i,0,self.extra + 1){
            ans += cnds[i];
        }
        return ans - 1;
    }
