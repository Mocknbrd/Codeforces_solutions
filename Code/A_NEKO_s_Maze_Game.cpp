    ...
    while(q--){
        ...
        if(state[r][c] is 0){
            ...
            if(r - 1 >= 0){
                ...
                if(c - 1 >= 0 and state[r - 1][c - 1] is 1){
                    diagonal[mp(r - 1,c - 1)]++;
                    diagonal[mp(r,c)]++;
                }
                if(c + 1 < n and state[r - 1][c + 1] is 1){
                    diagonal[mp(r - 1,c + 1)]++;
                    diagonal[mp(r,c)]++;
                }
            }
            ...
            state[r][c] = 0;
            if(--straight[mp(r + 1,c)] <= 0){
                straight.erase(mp(r + 1,c));
            }
            if(--straight[mp(r - 1,c)] <= 0){
                straight.erase(mp(r - 1,c));
            }
            if(--diagonal[mp(r + 1,c - 1)] <= 0){
                diagonal.erase(mp(r + 1,c - 1));
            }
            if(--diagonal[mp(r + 1,c + 1)] <= 0){
                diagonal.erase(mp(r + 1,c + 1));
            }
            ...
            straight.erase(mp(r,c));
            diagonal.erase(mp(r,c));
        }
        straight.empty() is true and diagonal.empty() is true ? yy : nn;
    }
    return;
}
