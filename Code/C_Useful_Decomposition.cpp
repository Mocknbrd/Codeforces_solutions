...
const ll linf = 2e18;
void testcase();
bool dfs(vvi &tree,int vertex,int parent);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    inc(i,0,n - 1){
        ...
        tree[u].pb(v);
        tree[v].pb(u);
        if(tree[u].sz() > tree[center].sz()){
            center = u;
        }
        if(tree[v].sz() > tree[center].sz()){
            center = v;
        }
    }
    each(child,tree[center]){
        if(dfs(tree,child,center) is false){
            nn;
            return;
        }
    }
    ...
    vpii paths;
    inc(vertex,1,n + 1){
        if(vertex isnt center and tree[vertex].sz() is 1){
            paths.pb(mp(center,vertex));
        }
    }
    see(paths.sz());
    each(path,paths){
...
bool dfs(vvi &tree,int vertex,int parent){
    if(tree[vertex].sz() is 1){
        return true;
    } elif(tree[vertex].sz() > 2){
        return false;
    } else {
        each(child,tree[vertex]){
            if(child isnt parent and dfs(tree,child,vertex) is false){
                return false;
            }
        }
        return true;
