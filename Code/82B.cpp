...
int main(){
    ...
    inc(int,i,0,n*(n-1)/2){
        ...
        unions.push_back(element);
    }
    if(unions.size()==1){
        cout<<1<<" "<<unions.front().front()<<endl;
        cout<<unions.front().size()-1<<" ";
        printArray(unions.front(),1);
    } else {
        map<int,vi>elements;
        map<vi,vi>pos;
        inc(int,i,0,unions.size()){
            each(element,unions[i]){
                elements[element].push_back(i);
            }
        }
        each(element,elements){
            pos[element.second].push_back(element.first);
        }
        each(element,pos){
