...
void testcase(){
    ...
    inc(i,0,n){
        ...
        end = max(end,max(arr[i].fi,arr[i].sc));
    }
    int start = 0,ans = end;
    while(start <= end){
        int cnd = (start + end) >> 1;
        ...
        pii interval = mp(0,0);
        inc(i,0,n){
            pii range = mp(interval.fi - cnd,interval.sc + cnd);
            interval = mp(max(arr[i].fi,range.fi),min(arr[i].sc,range.sc));
            if(interval.sc < interval.fi){
                isAns = false;
                break;
            }
        }
