...
void testcase(){
    ...
    readArray(arr);
    vll even(n),odd(n);
    even.back() = (mod(arr.back(),2ll) is 0);
    odd.back() = (mod(arr.back(),2ll) is 1);
    dec(i,n - 2,0){
        even[i] = even[i + 1] + (mod(arr[i],2ll) is 0);
        odd[i] = odd[i + 1] + (mod(arr[i],2ll) is 1);
    }
    ll ans = 0;
    each(value,arr){
        ans += ((value / 2ll) + 1ll);
