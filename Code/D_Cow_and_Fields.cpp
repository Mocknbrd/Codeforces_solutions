...
class SegmentTree {
    private:  
    Node *root;
    Node* build(vt(pair(int,pii))&arr,int start,int end){
        Node *node;
        if(start is end){
            node = new Node(start,end,arr[start].sc.sc);
        } else {
            int mid = (start + end) >> 1;
    ...
    public:  
    SegmentTree(vt(pair(int,pii))&arr){
...
void testcase(){
    ...
    vi start = bfs(graph,0);
    vi end = bfs(graph,n - 1);
    vt(pair(int,pii))data;
    each(vertex,special){
        data.pb(mp(start[vertex] - end[vertex],mp(start[vertex],end[vertex])));
    }
    sorted(data);
    SegmentTree tree(data);
    int ans = -inf;
    inc(i,0,data.sz()){
        ans = max(ans,data[i].sc.fi + tree.query(i + 1,k - 1));
    }
    see(min(start.bk(),ans + 1));
    return;
}
vi bfs(vvi &graph,int vertex){
    vi ans(graph.sz(),inf);
    ...
    while(q.empty() is false){
        ...
        int node = curr.fi,distance = curr.sc;
        each(neighbour,graph[node]){
            if(ans[neighbour] is inf){
                ans[neighbour] = distance + 1;
                q.push(mp(neighbour,distance + 1));
            }
        }
    }
    return ans;
