...
void testcase(){
    ...
    vi comp = coordinateCompress(maxi);
    each(value,comp){
        value++;
    }
    map(int,int)mapping;
    ...
    inc(i,0,maxi.sz()){
        mapping[maxi[i]] = comp[i];
        suffix.update(comp[i]);
    }
    int ans = 0;
    inc(i,0,n){
        suffix.update(comp[i],-1);
        int cnds = mod(suffix.query(comp[i]) + prefix.query(comp[i] - 1),md);
        ans = mod(ans + mod(maxi[i] * power(2,cnds,md),md),md);
        prefix.update(comp[i]);
    }
    see(ans);
    return;
}
