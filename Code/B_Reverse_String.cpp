...
const int inf = 2e9;
const ll linf = 2e18;
bool solve(vt(vvi)&dp,string &s,string &t,int i,int j,int dir);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    string s,t;
    cin >> s >> t;
    vt(vvi)dp(s.sz(),vvi(t.sz(),vi(2,-1)));
    inc(i,0,s.sz()){
        if(s[i] is t[0] and solve(dp,s,t,i,0,0) is true){
            YY;
            return;
        }
    }
...
bool solve(vt(vvi)&dp,string &s,string &t,int i,int j,int dir){
    if(j + 1 is t.sz()){
        return dp[i][j][dir] = true;
    } elif(dp[i][j][dir] isnt -1){
        return dp[i][j][dir];
    } else {
        if(dir is 0){
            bool ans1 = false,ans2 = false;
            if(i isnt 0 and s[i - 1] is t[j + 1]){
                ans1 = solve(dp,s,t,i - 1,j + 1,1);
            }
            if(i + 1 isnt s.sz() and s[i + 1] is t[j + 1]){
                ans2 = solve(dp,s,t,i + 1,j + 1,dir);
            }
            return dp[i][j][dir] = (ans1 or ans2);
        } else {
            if(i is 0 or s[i - 1] isnt t[j + 1]){
                return dp[i][j][dir] = false;
            } else {
                return dp[i][j][dir] = solve(dp,s,t,i - 1,j + 1,dir);
            }
        }
    }
}
