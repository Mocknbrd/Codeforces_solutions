...
class DSU {
    ...
    int findComponents(){
        int ans = 0;
        inc(vertex,1,this.parents.sz()){
            ans += (this.find(vertex) is vertex);
        }
        return ans;
    }
    int findBamboos(vi &degree){
        set(int)status;
        inc(vertex,1,degree.sz()){
            if(degree[vertex] isnt 2){
                status.ins(this.find(vertex));
            }
        }
        return status.sz();
    }
...
void testcase(){
    ...
    inc(u,1,n + 1){
        int v;
        cin >> v;
        bool add = dsu.findUnion(u,v);
        degree[u] += add;
        degree[v] += add;
    }
    writeArray(degree);
    int maxi = dsu.findComponents(),mini = min(maxi,maxi - dsu.findBamboos(degree) + 1);
    see(mini << " " << maxi);
    return;
}
