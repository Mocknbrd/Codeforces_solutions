...
void testcase(){
    ...
    vll lengths;
    sorted(right);
    each(rightBound,right){
        ll leftBound = *--left.upper_bound(rightBound);
        lengths.pb(rightBound - leftBound);
        left.erase(leftBound);
    }
    sorted(weights);
    rsort(lengths);
    while(lengths.empty() is false){
        ans += lengths.back() * weights.back();
        lengths.rb();
        weights.rb();
    }
    see(ans);
