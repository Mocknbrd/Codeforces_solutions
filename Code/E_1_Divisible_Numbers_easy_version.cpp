...
const ll linf = 2e18;
void testcase();
pll verify(ll x,ll y,ll a,ll b,ll c,ll d);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    while(prime * prime <= target){
        if(mod(target,prime) is 0){
            pll cnd = verify(prime,target / prime,a,b,c,d);
            if(cnd.fi > 0 and cnd.sc > 0){
                see(cnd.fi << " " << cnd.sc);
                return;
            }
            cnd = verify(target / prime,prime,a,b,c,d);
            if(cnd.fi > 0 and cnd.sc > 0){
                see(cnd.fi << " " << cnd.sc);
                return;
            }
...
pll verify(ll x,ll y,ll a,ll b,ll c,ll d){
    if(inBetween(b,y,d,false) is false){
        ll start = 0,end = 1e5,ans = 0;
        while(start <= end){
            ll mid = (start + end) >> 1;
            if(inBetween(b,mid * y,d,false)){
                ans = mid;
                break;
            } elif(mid * y <= b){
                start = mid + 1;
    ...
    if(inBetween(a,x,c,false) is false){
        ll start = 0,end = 1e5,ans = 0;
        while(start <= end){
            ll mid = (start + end) >> 1;
            if(inBetween(a,mid * x,c,false)){
                ans = mid;
                break;
            } elif(mid * x <= a){
                start = mid + 1;
