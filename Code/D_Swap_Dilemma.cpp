...
class FenwickTree {
    ...
    public:  
    FenwickTree(int n){
        this.counts = vi(n + 1,0);
    }
    void update(int value){
        value++;
        while(value < this.counts.sz()){
            this.counts[value]++;
...
void testcase(){
    ...
    readArray(a);
    readArray(b);
    mset(int)f(all(a)),s(all(b));
    if(f isnt s){
        NN;
    } else {
        coordinateCompressInplace(a);
        coordinateCompressInplace(b);
        FenwickTree aTree(n),bTree(n);
        int first = 0,second = 0;
        inc(i,0,n){
            first += i - aTree.query(a[i]);
            second += i - bTree.query(b[i]);
            aTree.update(a[i]);
            bTree.update(b[i]);
        }
        mod(first,2) is mod(second,2) ? YY : NN;
    }
    return;
}
