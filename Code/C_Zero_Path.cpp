...
void testcase(){
    ...
    if(mod(n + m,2) is 0){
        ...
        vvi maxi = arr,mini = arr;
        dec(col,m - 2,0){
            maxi[n - 1][col] += maxi[n - 1][col + 1];
            mini[n - 1][col] += mini[n - 1][col + 1];
        }
        dec(row,n - 2,0){
            maxi[row][m - 1] += maxi[row + 1][m - 1];
            mini[row][m - 1] += mini[row + 1][m - 1];
        }
        dec(row,n - 2,0){
            dec(col,m - 2,0){
                maxi[row][col] += max(maxi[row + 1][col],maxi[row][col + 1]);
                mini[row][col] += min(mini[row + 1][col],mini[row][col + 1]);
            }
        }
        if(maxi[0][0] < 0 or mini[0][0] > 0){
            NN;
        } else {
            YY;
        }
