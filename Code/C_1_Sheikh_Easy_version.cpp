...
void testcase(){
    ...
    int _,__;
    cin >> _ >> __;
    vll prefix(n + 1,0),xors(n + 1,0);
    inc(i,1,prefix.sz()){
        prefix[i] = prefix[i - 1] + input[i - 1];
        xors[i] = xors[i - 1] ^ input[i - 1];
    }
    ll target = prefix.bk() - xors.bk();
    int left = 1,right = n;
    inc(i,0,n){
        int start = i,end = n - 1,res = inf;
        while(start <= end){
            int mid = (start + end) >> 1;
            ll curr = prefix[mid + 1] - prefix[i] - (xors[mid + 1] ^ xors[i]);
            if(curr is target){
                res = mid;
                end = mid - 1;
            } else {
