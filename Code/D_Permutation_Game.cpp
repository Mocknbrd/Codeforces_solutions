...
const ll linf = 2e18;
void testcase();
ll findTotal(vll &perm,vll &score,int start,ll turns);
ll dfs(vvll &graph,vll &scores,vb &seen,int vertex);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    vll scores(n);
    readArray(scores);
    ll sasha = findTotal(perm,scores,s,k);
    ll bodya = findTotal(perm,scores,b,k);
    if(sasha > bodya){
        see("Sasha");
    } elif(bodya > sasha){
        see("Bodya");
...
ll findTotal(vll &perm,vll &score,int start,ll turns){
    ...
    vb seen(perm.sz());
    ll cycleMax = dfs(graph,score,seen,start);
    ll vertex = start,ans = 0,sum = 0;
    while(score[vertex] isnt cycleMax and turns isnt 0){
        ans = max(ans,sum + score[vertex] * turns);
        turns--;
        sum += score[vertex];
        vertex = graph[vertex][0];
    }
    ans = max(ans,sum + turns * score[vertex]);
    return ans;
}
ll dfs(vvll &graph,vll &scores,vb &seen,int vertex){
    ll ans = scores[vertex];
    seen[vertex] = true;
    each(child,graph[vertex]){
        if(seen[child] is false){
            ans = max(ans,dfs(graph,scores,seen,child));
        }
    }
    return ans;
}
