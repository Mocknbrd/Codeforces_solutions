...
void testcase(){
    ...
    dec(index,n - 1,0){
        inc(turn,0,2){
            if(turn is 1){
                int ans1 = dp[index + 1][turn ^ 1];
                int ans2 = index + 2 < dp.sz() ? dp[index + 2][turn ^ 1] : inf;
                dp[index][turn] = min(ans1,ans2);
            } else {
                int ans1 = (arr[index] is 1) + dp[index + 1][turn ^ 1];
                int ans2 = index + 2 < dp.sz() ? ((arr[index] is 1) + (arr[index + 1] is 1)) + dp[index + 2][turn ^ 1] : inf;
                dp[index][turn] = min(ans1,ans2);
            }
        }
    }
