...
void testcase(){
    int p,t;
    cin >> p >> t;
    vll programming(p + t + 1),testing(p + t + 1);
    readArray(programming);
    readArray(testing);
    set(int)programmers,testers;
    ll program = 0,test = 0;
    inc(i,0,programming.sz()){
        if(programmers.sz() is p + 1){
            testers.ins(i);
            test += testing[i];
            continue;
        }
        if(testers.sz() is t + 1){
            programmers.ins(i);
            program += programming[i];
            continue;
        }
        if(programming[i] < testing[i]){
            testers.ins(i);
            test += testing[i];
        } else {
            programmers.ins(i);
    ...
    inc(i,0,programming.sz()){
        if(programmers.count(i) is true){
            if(programmers.sz() is p + 1){
                cout << program - programming[i] + test << " ";
            } else {
                cout << program - programming[i] + programming[*--testers.end()] + test - testing[*--testers.end()] << " ";
            }
        } else {
            if(testers.sz() is t + 1){
                cout << program + test - testing[i] << " ";
            } else {
                cout << program - programming[*--programmers.end()] + test - testing[i] + testing[*--programmers.end()] << " ";
            }
        }
    }
    br();
