...
void testcase(){
    int n;
    string isclown,isacro;
    cin >> n >> isclown >> isacro;
    vi clown,acrobat,both,none;
    inc(i,0,n){
        if(isclown[i] is '1'){
            if(isacro[i] is '1'){
                both.pb(i + 1);
    ...
    inc(a,0,clown.sz() + 1){
        inc(c,0,both.sz() + 1){
            int b = acrobat.sz() + both.sz() - a - 2 * c;
            int d = n / 2 - a - b - c;
            if(inBetween(0,b,cast(acrobat.sz(),int)) and inBetween(0,d,cast(none.sz(),int))){
                inc(i,0,a){
                    cout << clown[i] << " ";
                }
                inc(i,0,b){
