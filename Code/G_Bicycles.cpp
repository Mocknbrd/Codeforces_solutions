...
void testcase(){
    ...
    vll slow(n);
    readArray(slow);
    vvll dist(n,vll(*max_element(all(slow)) + 1,linf));
    dist[0][slow[0]] = 0;
    min_heap(pair(pll,pll))pq;
    each(edge,graph[0]){
        pq.push(mp(mp(slow[0] * edge.sc,slow[0]),mp(0,edge.fi)));
    }
    while(pq.empty() is false){
        pair(pll,pll)curr = pq.top(); pq.pop();
        ll weight = curr.fi.fi,currSlow = curr.fi.sc,from = curr.sc.fi,to = curr.sc.sc;
        ll nxtSlow = min(slow[to],currSlow);
        if(dist[from][currSlow] + weight < dist[to][nxtSlow]){
            dist[to][nxtSlow] = dist[from][currSlow] + weight;
            each(edge,graph[to]){
                if(dist[to][nxtSlow] + nxtSlow * edge.sc < dist[edge.fi][min(nxtSlow,slow[edge.fi])]){
                    pq.push(mp(mp(nxtSlow * edge.sc,nxtSlow),mp(to,edge.fi)));
                }
            }
        }
    }
    see(*min_element(all(dist.bk())));
    return;
}
