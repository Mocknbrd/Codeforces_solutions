...
void testcase(){
    ...
    vll arr(n);
    readArray(arr);
    ll ans = n,start = 1,end = n;
    while(start <= end){
        int window = (start + end) >> 1;
        bool isAns = true;
        inc(pos,0,20){
            ll target = 0;
            inc(i,0,window){
            ...
            bool bit = (target isnt 0);
            ll curr = target;
            slid_win(left,right,window,n){
                curr -= ((arr[left - 1] >> pos) & 1);
                curr += ((arr[right] >> pos) & 1);
                bool currBit = (curr isnt 0);
                if(currBit isnt bit){
                    isAns = false;
                    break;
                }
