...
void markCycle(vi &a,vi &b,vi &c,map<int,int>&apos,map<int,int>&bpos,set<int>&seen,int start);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    inc(i,0,n){
        if(c[i] != -1){
            markCycle(a,b,c,apos,bpos,seen,i);
        }
    }
    ll numCycles = 0;
    inc(i,0,n){
...
void markCycle(vi &a,vi &b,vi &c,map<int,int>&apos,map<int,int>&bpos,set<int>&seen,int start){
    seen.ins(start);
    int targetIndex = (b[start] == c[start] ? bpos[a[start]] : apos[b[start]]);
    if(!seen.count(targetIndex)){
        markCycle(a,b,c,apos,bpos,seen,targetIndex);
    }
}