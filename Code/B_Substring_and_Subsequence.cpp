...
void testcase(){
    ...
    inc(i,0,b.sz()){
        ...
        while(first < a.sz() and second < b.sz()){
            while(first < a.sz() and a[first] isnt b[second]){
                first++;
            }
            if(first < a.sz()){
                second++;
    ...
    int cnd = findMaxIndex(lengths);
    int cpy = cnd;
    vi indices;
    inc(i,0,a.sz()){
        if(cnd < b.sz() and a[i] is b[cnd]){
            cnd++;
            indices.pb(i);
        }
    }
    string ans = b.substr(0,cpy) + a + b.substr(cpy + lengths[cpy]);
    see(ans.sz());
    return;
}
