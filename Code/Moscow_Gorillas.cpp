...
void testcase(){
    ...
    inc(i,1,n){
        ...
        ll lcnt = max(0ll,min(al,bl) - max(amn,bmn) + 1);
        ll rnct = max(0ll,min(amx,bmx) - max(ar,br) + 1);
        ans += lcnt * rnct;
    }
    ll left = min(apos[1],bpos[1]) - 1,right = n - max(apos[1],bpos[1]),mid = abs(apos[1] - bpos[1]) - 1;
    ans += left * (left + 1) / 2ll + right * (right + 1) / 2ll + mid * (mid + 1) / 2ll;
    ans++;
    see(ans);
    return;
}
