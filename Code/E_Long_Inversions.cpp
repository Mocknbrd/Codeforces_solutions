...
void testcase(){
    ...
    inc(window,1,n + 1){
        ...
        int cnt = 0;
        inc(i,0,n){
            while(q.empty() is false and q.front() + window - 1 < i){
                q.pop();
            }
            bool isEven = (q.sz() % 2 is 0);
            if(i + window <= n){
                if((s[i] is '0' and isEven is true) or (s[i] is '1' and isEven is false)){
                    q.push(i);
                } 
                cnt++;
            } else {
                if((s[i] is '1' and isEven is true) or (s[i] is '0' and isEven is false)){
                    cnt++;
                }
            }
        }
