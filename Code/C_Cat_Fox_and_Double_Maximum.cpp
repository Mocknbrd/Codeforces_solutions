...
void testcase(){
    ...
    vi even(n,0),odd(n,0);
    {
        ...
        inc(i,1,n){
            if(mod(i,2) is 0){
                evenpos.pb(mp(arr[i],i));
            } else {
                oddpos.pb(mp(arr[i],i));
            }
        }
        rsort(evenpos);
        rsort(oddpos);
        each(element,oddpos){
            even[element.sc] = *values.begin();
            values.erase(values.begin());
        }
        each(element,evenpos){
            int left = arr[element.sc - 1] + even[element.sc - 1],right = arr[element.sc + 1] - even[element.sc + 1];
            int diff = max(0,max(left - arr[element.sc],right - arr[element.sc]));
            auto it = values.upper_bound(diff);
            if(it is values.end()){
                --it;
            }
            even[element.sc] = *it;
    ...
    }
    {
        ...
        rsort(oddpos);
        each(element,evenpos){
            odd[element.sc] = *values.begin();
            values.erase(values.begin());
        }
        each(element,oddpos){
            int left = arr[element.sc - 1] + odd[element.sc - 1],right = arr[element.sc + 1] + odd[element.sc + 1];
            int diff = max(0,max(left - arr[element.sc],right - arr[element.sc]));
            auto it = values.upper_bound(diff);
            if(it is values.end()){
                --it;
        ...
        odd.back() = *values.begin();
    }

    if(findPeaks(arr,even) > findPeaks(arr,odd)){
        writeArray(even);
    } else {
        writeArray(odd);
    }
