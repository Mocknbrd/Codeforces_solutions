...
void testcase(){
    ...
    vvi left(n,vi(m,0)),right = left,up = left,down = left;
    inc(i,0,n){
        left[i].fr() = (arr[i].fr() is star);
        right[i].bk() = (arr[i].bk() is star);
    }
    inc(j,0,m){
        up.fr()[j] = (arr.fr()[j] is star);
        down.bk()[j] = (arr.bk()[j] is star);
    }
    inc(i,0,n){
        inc(j,1,m){
            left[i][j] = (arr[i][j] is star) * (left[i][j - 1] + 1);
        }
        dec(j,m - 2,0){
            right[i][j] = (arr[i][j] is star) * (right[i][j + 1] + 1);
        }
    }
    inc(j,0,m){
        inc(i,1,n){
            up[i][j] = (arr[i][j] is star) * (up[i - 1][j] + 1);
        }
        dec(i,n - 2,0){
            down[i][j] = (arr[i][j] is star) * (down[i + 1][j] + 1);
        }
    }
    vvi ans;
    inc(i,0,n){
        inc(j,0,m){
            if(arr[i][j] is star){
                int length = min(min(up[i][j],down[i][j]),min(left[i][j],right[i][j]));
                if(length > 1){
                    inc(add,0,length){
                        dots.erase(mp(i + add,j));
                        dots.erase(mp(i,j + add));
                        dots.erase(mp(i - add,j));
                        dots.erase(mp(i,j - add));
                    }
                    vi cnd;
