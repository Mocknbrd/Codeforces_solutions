...
const ll linf = 8e18;
void testcase();
int findRight(vi &prefix,int count);
int main(){
    ios;
    SINGLE;
    return 0;
...
void testcase(){
    ...
    string s;
    cin >> n >> s;
    vvi prefix(26,vi(1,0));
    inc(character,0,26){
        each(curr,s){
            prefix[character].pb(prefix[character].back() + (curr - 'a' is character));
        }
    }
    int q;
    cin >> q;
    while(q--){
        ...
        int ans = 0;
        inc(character,0,26){
            ans = max(ans,findRight(prefix[character],counts[character]));
        }
        see(ans);
    }
    return;
...
int findRight(vi &prefix,int count){
    int start = 0,end = prefix.sz() - 1,ans = end;
    while(start <= end){
        int mid = (start + end) >> 1;
        if(prefix[mid] >= count){
            ans = mid;
