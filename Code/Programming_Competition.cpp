...
const ll linf = 2e18;
void testcase();
int computeSize(vvi &tree,vi &subtree,int vertex);
int dfs(vvi &tree,vi &subtree,int vertex,int paired,int seen);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    inc(vertex,1,n){
        int parent;
        cin >> parent;
        tree[parent - 1].pb(vertex);
    }
    vi subtree(n);
    computeSize(tree,subtree,0);
    see(dfs(tree,subtree,0,0,0) / 2);
    return;
}
int computeSize(vvi &tree,vi &subtree,int vertex){
    int ans = 1;
    each(child,tree[vertex]){
        ans += computeSize(tree,subtree,child);
    }
    return subtree[vertex] = ans;
}
int dfs(vvi &tree,vi &subtree,int vertex,int paired,int seen){
    int ans = (tree.sz() - subtree[vertex] - paired - seen > 0);
    paired += (tree.sz() - subtree[vertex] - paired - seen > 0);
    each(child,tree[vertex]){
        ans += dfs(tree,subtree,child,paired,seen + 1);
    }
    return ans;
}
