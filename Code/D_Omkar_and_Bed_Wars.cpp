...
void testcase(){
    ...
    while(start < n){
        int end = start;
        while(end < n and s[start] is s[end]){
            end++;
        }
        lengths.pb(end - start);
        start = end;
    }
    if(lengths.sz() is 1){
        see(ceil(lengths[0],3));
    } else {
        int ans = 0;
        inc(i,1,lengths.sz() - 1){
            ans += lengths[i] / 3;
        }
        if(s.front() is s.back()){
            ans += (lengths.front() + lengths.back()) / 3;
        } else {
            ans += lengths.front() / 3 + lengths.back() / 3;
        }
        see(ans);
    }
    return;
