...
void testcase(){
    ...
    int maxi = 0;
    inc(i,1,segments.sz()){
        cin >> segments[i].fi >> segments[i].sc;
        maxi = max(maxi,segments[i].sc);
    }
    vi counts(maxi + 2,0);
    vvi status(counts.sz());
    inc(i,1,segments.sz()){
        counts[segments[i].fi]++;
        counts[segments[i].sc + 1]--;
        status[segments[i].fi].pb(i);
        status[segments[i].sc + 1].pb(-i);
    }
    inc(i,1,counts.sz()){
        counts[i] += counts[i - 1];
    }
    int curr = 0;
    vi subtract(counts.sz(),0),ans(segments.sz(),0);
    set(pii)currSegs;
    inc(vertex,1,counts.sz()){
        curr += subtract[vertex];
        each(index,status[vertex]){
            if(index > 0){
                currSegs.ins(mp(segments[index].sc,index));
            } else {
                currSegs.erase(mp(segments[abs(index)].sc,abs(index)));
            }
        }
        while(counts[vertex] - curr > k and currSegs.empty() is false){
            pii currSeg = *--currSegs.end();
            pii segment = segments[currSeg.sc];
            subtract[segment.sc + 1]--;
            ans[currSeg.sc] = 1;
            curr++;
            currSegs.erase(currSeg);
        }
    }
    see(accumulate(all(ans),0));
    inc(i,0,ans.sz()){
        if(ans[i] is 1){
            cout << i << " ";
        }
