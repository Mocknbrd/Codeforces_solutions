...
void testcase(){
    ...
    vvi seating(n,vi(m,0));
    int row = 0,col = 0;
    each(entry,mp){
        while(col isnt 0 and entry.sc.empty() is false and row isnt n - 1){
            seating[row][col] = entry.sc.back();
            entry.sc.rb();
            if(++col is m){
                col = 0;
                row++;
        ...
        while(entry.sc.empty() is false){
            seating[row][col] = entry.sc.front();
            entry.sc.rf();
            if(++col is m){
                col = 0;
                row++;
    ...
    inc(i,0,n){
        inc(start,0,m){
            inc(end,start + 1,m){
                ans += seating[i][end] > seating[i][start];
            }
        }
    }
    see(ans);
