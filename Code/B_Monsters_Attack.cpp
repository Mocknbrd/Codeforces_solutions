...
const ll linf = 2e18;
void testcase();
bool findCoord(int original,int steps){
    if(original < 0){
        return original + steps >= 0;
    } else {
        return original - steps <= 0;
    }
};
...
void testcase(){
    ...
    inc(i,0,n){
        pq.push(mp(abs(coords[i]),mp(coords[i],arr[i])));
    }
    ll shots = 0;
    while(pq.empty() is false){
        pair(ll,pll)curr = pq.top(); pq.pop();
        ll coord = curr.sc.fi,health = curr.sc.sc;
        if(findCoord(coord,shots) is true){
            NN;
            return;
        } else {
            ll required = ceil(health,k);
            if(findCoord(coord,shots + required - 1) is true){
                NN;
                return;
            }
            shots += required;
            ll remShots = required * k - health;
            while(remShots > 0 and pq.empty() is false){
                pair(ll,pll)tmp = pq.top(); pq.pop();
                if(tmp.sc.sc <= remShots){
                    remShots -= tmp.sc.sc;
                } else {
                    tmp.sc.sc -= remShots;
                    pq.push(tmp);
