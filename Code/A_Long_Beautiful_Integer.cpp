...
const ll linf = 2e18;
void testcase();
bool dfs(string &s,string &ans,map(int,char)&digit,int k,int index,bool bigger);
int main(){
    ios;
    int t = 1;
    while(t--){
...
bool dfs(string &s,string &ans,map(int,char)&digit,int k,int index,bool bigger){
    if(index is s.sz()){
        return ans >= s;
    } elif(digit.count(mod(index,k)) is true){
        ans[index] = digit[mod(index,k)];
        if(dfs(s,ans,digit,k,index + 1,bigger) is true){
            return true;
        }
        ans[index] = '0';
        return false;
        ...
        if(bigger is true){
            digit[mod(index,k)] = '0';
            ans[index] = '0';
            if(dfs(s,ans,digit,k,index + 1,true) is true){
                return true;
            }
            digit.erase(mod(index,k));
        } else {
            inc(d,s[index] - '0',10){
                char curr = '0' + d;
                ans[index] = curr;
                digit[mod(index,k)] = curr;
                if(dfs(s,ans,digit,k,index + 1,curr > s[index]) is true){
                    return true;
                }
                ans[index] = '0';
                digit.erase(mod(index,k));
