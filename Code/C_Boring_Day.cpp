...
const ll linf = 2e18;
void testcase();
ll solve(vll &arr,vll &prefix,vll &dp,int ind,int l,int r);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
ll solve(vll &arr,vll &prefix,vll &dp,int ind,int l,int r){
    if(ind >= arr.sz()){
        ...
        ll ans1 = 0,ans2 = 0,ans3 = solve(arr,prefix,dp,ind + 1,l,r);
        {
            int start = ind,end = arr.sz() - 1,first = start;
            while(start <= end){
                int mid = (start + end) >> 1;
                if(prefix[mid + 1] - prefix[ind] >= l){
                    first = mid;
                    end = mid - 1;
                } else {
                    start = mid + 1;
            ...
            ans1 = inBetween((ll)l,prefix[first + 1] - prefix[ind],(ll)r) + solve(arr,prefix,dp,first + 1,l,r);
        }
        {
            int start = ind,end = arr.sz() - 1,second = end;
            while(start <= end){
                int mid = (start + end) >> 1;
                if(prefix[mid + 1] - prefix[ind] <= r){
                    second = mid;
                    start = mid + 1;
                } else {
                    end = mid - 1;
            ...
            ans2 = inBetween((ll)l,prefix[second + 1] - prefix[ind],(ll)r) + solve(arr,prefix,dp,second + 1,l,r);
        }
        return dp[ind] = max(ans1,max(ans2,ans3));
    }
}
