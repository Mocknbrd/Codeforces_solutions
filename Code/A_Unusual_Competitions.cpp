...
void testcase(){
    ...
    string arr;
    cin >> arr;
    vi open(arr.sz(),0),close(arr.sz(),0);
    open[0] = arr[0] is '(',close[0] = arr[0] is ')';
    inc(i,1,arr.sz()){
        open[i] = open[i - 1] + (arr[i] is '(');
        close[i] = close[i - 1] + (arr[i] is ')');
    }
    if(open.back() isnt close.back()){
        see(-1);
    } else {
        int ans = 0,start = 0;
        while(start < open.sz()){
            if(arr[start] is '(' or open[start] >= close[start]){
                start++;
            } else {
                int end = start;
                while(end < arr.sz() and open[end] < close[end]){
                    end++;
                }
                ans += end - start + 1;
                start = end + 1;
