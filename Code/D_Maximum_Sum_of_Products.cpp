...
const ll linf = 2e18;
void testcase();
ll solve(vll &a,vll &b,vvll &dp,int start,int end);
int main(){
    ios;
    testcase();
    return 0;
...
void testcase(){
    ...
    prefix.pb(0);
    inc(i,0,n){
        prefix.pb(prefix.back() + a[i] * b[i]);
    }
    inc(i,0,n){
        inc(j,i,n){
            if(dp[i][j] is -1){
                dp[i][j] = solve(a,b,dp,i,j);
            }
        }
    }
    ll ans = prefix.back();
    inc(i,0,n){
        inc(j,i,n){
            ans = max(ans,prefix[i] + dp[i][j] + prefix.back() - prefix[j + 1]);
        }
    }
    see(ans);
    return;
...
ll solve(vll &a,vll &b,vvll &dp,int start,int end){
    if(start > end){
        return 0;
    } elif(dp[start][end] isnt -1){
        return dp[start][end];
    } else {
        return dp[start][end] = (a[start] * b[end] + a[end] * b[start]) / (1ll + (start is end)) + solve(a,b,dp,start + 1,end - 1);
    }
}
