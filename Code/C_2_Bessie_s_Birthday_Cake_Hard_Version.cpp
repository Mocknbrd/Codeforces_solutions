...
const ll linf = 2e18;
void testcase();
int findAns(int sides){
    if(sides <= 2){
        return 0;
    } else {
        return sides / 2 + findAns(ceil(sides,2));
    }
}
int main(){
    ios;
...
void testcase(){
    ...
    sorted(vertex);
    inc_la(i,0,x,1){
        int src = vertex[i],tgt = vertex[i + 1];
        int curr = src + 2;
        while(curr < tgt){
            if(y > 0){
                vertex.pb(curr);
    ...
    sorted(vertex);
    int src = vertex.back(),tgt = vertex.front();
    int curr = src + 2;
    while(curr <= n){
        if(y > 0){
            vertex.pb(curr);
    ...
    sorted(vertex);
    x = vertex.sz();
    int ans = findAns(vertex.sz());
    inc(i,0,vertex.sz()){
        int nxt = mod(i + 1,x);
        ans += (mod(vertex[nxt] - vertex[i],n) is 2);
    }
    see(ans);
    return;
}
