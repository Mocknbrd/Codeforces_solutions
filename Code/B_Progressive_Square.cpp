...
void testcase(){
    ...
    matrix[0][0] = *values.begin();
    values.erase(matrix[0][0]);
    inc(i,0,n){
        inc(j,0,n){
            if(i + 1 < n){
                if(matrix[i + 1][j] is -1){
                    if(values.count(matrix[i][j] + c) is false){
                        NN;
                        return;
                    }
                    matrix[i + 1][j] = matrix[i][j] + c;
                    values.erase(values.find(matrix[i + 1][j]));
                } else {
                    if(matrix[i][j] + c isnt matrix[i + 1][j]){
                        NN;
                        return;
                    }
                }
            ...
            if(j + 1 < n){
                if(matrix[i][j + 1] is -1){
                    if(values.count(matrix[i][j] + d) is false){
                        NN;
                        return;
                    }
                    matrix[i][j + 1] = matrix[i][j] + d;
                    values.erase(values.find(matrix[i][j + 1]));
                } else {
                    if(matrix[i][j] + d isnt matrix[i][j + 1]){
                        NN;
                        return;
                    }
                }
