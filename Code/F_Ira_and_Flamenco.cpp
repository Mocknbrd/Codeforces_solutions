...
void testcase(){
    ...
    vll cnts(counts.sz(),0);
    vll values(counts.sz(),0);
    int start = 0;
    each(element,counts){
        cnts[start] = element.sc;
        values[start++] = element.fi;
    }

    if(values.sz() < m){
        see(0);
    } else {
        ll ans = 0,curr = 1,md = 1e9 + 7;
        inc(i,0,m){
            curr = modMult(curr,cnts[i],md);
        }
        if(values[m - 1] - values[0] < m){
            ans = mod(ans + curr,md);
        }
        slid_win(left,right,m,cnts.sz()){
            curr = modMult(curr,moduloInv4Prime(cnts[left - 1],md),md);
            curr = modMult(curr,cnts[right],md);
            if(values[right] - values[left] < m){
                ans = mod(ans + curr,md);
            }
        }
        see(ans);
