...
class DSU {
    ...
    map(pii,pii)parents;
    map(pii,int)ranks;
    void merge(pii large,pii small){
        this.parents[small] = large;
        this.ranks[large] += this.ranks[large];
    }
    pii find(pii x){
        vpii stack;
        while(this.parents[x] isnt x){
            stack.pb(x);
            x = this.parents[x];
        }
    ...
    DSU(int r,int c){
        inc(i,0,r){
            inc(j,0,c){
                parents[mp(i,j)] = mp(i,j);
                ranks[mp(i,j)] = 1;
            }
        }
    }
    void connect(pii x,pii y){
        pii parx = this.find(x);
        pii pary = this.find(y);
        if(parx isnt pary){
    ...
    map(pii,vpii)getMapping(int r,int c){
        map(pii,vpii)ans;
        inc(i,0,r){
            inc(j,0,c){
                ans[this.find(mp(i,j))].pb(mp(i,j));
            }
        }
        return ans;
    }
...
void testcase(){
    ...
    inc(i,0,n){
        inc(j,0,n){
            dsu.connect(mp(i,j),mp(j,n - i - 1));
        }
    }
    map(pii,vpii)mapping = dsu.getMapping(n,n);
    set(pii)seen;
    int ans = 0;
    inc(i,0,n){
        inc(j,0,n){
            if(seen.find(mp(i,j)) is seen.end()){
                seen.ins(mp(i,j));
                int one = 0,zero = 0;
                each(p,mapping[mp(i,j)]){
                    one += (arr[p.fi][p.sc] is '1');
                    zero += (arr[p.fi][p.sc] is '0');
                }
                ans += min(one,zero);
            }
        }
