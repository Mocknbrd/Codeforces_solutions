...
void testcase(){
    ...
    if(mod(ones,k) isnt 0 or mod(zeros,k) isnt 0){
        see(-1);
    } else {
        vpii one,zero;
        int start = 0;
        while(start < n){
            int end = start;
            while(end < n and s[end] is s[start]){
                end++;
            }
            if(s[start] is '1'){
                one.pb(mp(start,end - 1));
            } else {
                zero.pb(mp(start,end - 1));
            }
            start = end;
        }
        int ind = -1;
        if(ind is -1){
            each(range,one){
                if(range.sc - range.fi + 1 > k){
                    int more = range.sc - range.fi + 1 - k;
                    ind = range.fi + more - 1;
                    break;
                } elif(range.sc - range.fi + 1 < k){
                    ind = range.sc;
                    break;
                }
            }
        ...
        if(ind is -1){
            ...
            string begin = s.substr(0,ind + 1);
            reverse(all(begin));
            string ans = s.substr(ind + 1) + begin;
            inc(i,0,k){
                if(ans[i] isnt ans[0]){
                    see(-1);
                    return;
