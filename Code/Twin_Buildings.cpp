...
void testcase(){
    ...
    inc(i,0,n){
        ...
        pq.push(mp(length * width,mp(length,width)));
    }
    stringstream ss;
    ss.precision(1);
    if(n is 1){
        ld ans = pq.top().fi / 2.0;
        ss << fixed << ans;
    } else {
        pair(ll,pll)first = pq.top(); pq.pop();
        pair(ll,pll)second = pq.top();
        ld ans1 = first.fi / 2.0;
        ld ans2 = min(first.sc.fi,second.sc.fi) * min(first.sc.sc,second.sc.sc);
        ss << fixed << max(ans1,ans2);
    }
    see(ss.str());
    return;
}
