...
const int inf = 2e9;
const ll linf = 2e18;
vt(pair(ld,ld)) coords;
vt(pair(ld,pll))sides;
void testcase();
ld findDistance(pair(ld,ld)&first,pair(ld,ld)&second);
ld findArea(ld hinge,ld first,ld second,ld free);
ld heron(ld first,ld second,ld third);
int main(){
    ios;
    int t = 1;
    while(t--){
...
void testcase(){
    ...
    inc(i,0,n){
        inc(j,i + 1,n){
            inc(k,j + 1,n){
                inc(f,k + 1,n){
                    ans = max(ans,findArea(i,j,k,f));
                }
            }
        }
    }
    see(setprecision(27) << ans);
    return;
}
ld findDistance(pair(ld,ld)&first,pair(ld,ld)&second){
    return sqrt((first.fi - second.fi) * (first.fi - second.fi) + (first.sc - second.sc) * (first.sc - second.sc));
}
ld findArea(ld hinge,ld first,ld second,ld free){
    
}
ld heron(ld first,ld second,ld third){
    ld semi = (first + second + third) / 2.0;
    return sqrt(semi * (semi - first) * (semi - second) * (semi - third));
}
