...
void testcase(){
    ...
    set(int)seen;
    inc(i,0,n){
        if(seen.count(i) is false){
            int minDist = inf,cnd = i;
            inc(j,i + 1,n){
                if(seen.count(j) is false and dist(input[i],input[j]) < minDist){
                    minDist = dist(input[i],input[j]);
                    cnd = j;
                }
            }
            seen.ins(cnd);
