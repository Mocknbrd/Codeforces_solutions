...
inline bool compare(char first,char second){
    return first is '?' or second is '?' or first is second;
};
int main(){
    ios;
    int t = 1;
...
void testcase(){
    string s;
    cin >> s;
    vvi isEqual(s.sz() / 2 + 1,vi(s.sz(),0));
    int ans = 0;
    inc(length,1,s.sz() / 2 + 1){
        inc_la(i,0,s.sz(),length){
            isEqual[length][i] = compare(s[i],s[i + length]);
        }
        int sum = 0;
        inc(i,0,length){
            sum += isEqual[length][i];
        }
        slid_win(left,right,length,s.sz()){
            if(sum is length){
                ans = 2 * length;
                break;
            }
            sum += isEqual[length][right] - isEqual[length][left - 1];
            if(sum is length){
                ans = 2 * length;
                break;
            }
