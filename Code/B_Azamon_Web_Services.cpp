...
void testcase(){
    ...
    vpii stack;
    while(copy isnt first){
        int index = copy.sz() - 1;
        dec(it,copy.sz() - 1,0){
            if(copy[it] isnt first[it]){
                index = it;
                break;
            }
        }
        int target = pos[first[index]];
        swap(pos[copy[target]],pos[copy[index]]);
        swap(copy[index],copy[target]);
        stack.pb(make_pair(index,target));
    }
    if(stack.empty() is false){
        swap(copy[stack.back().f],copy[stack.back().s]);
    }
    if(copy < second){
        see(copy);
    } else {
