...
void testcase(){
    ...
    each(character,s){
        if(character is 'R' or character is 'L'){
    ...
    int x = 1,y = 1;
    {
        int executed = 0,start = 1,end = m;
        while(start <= end){
            int mid = (start + end) >> 1;
            int pos = mid,curr = 0;
            each(character,horizontal){
                pos += (character is 'R') - (character is 'L');
                if(pos <= 0 or pos > m){
                    break;
                }
                curr++;
    ...
    {
        int executed = 0,start = 1,end = n;
        while(start <= end){
            int mid = (start + end) >> 1;
            int pos = mid,curr = 0;
            each(character,vertical){
                pos += (character is 'D') - (character is 'U');
                if(pos <= 0 or pos > n){
                    break;
                }
                curr++;
