...
int main(){
    ...
    Input(string,correct);
    vector<bool>same(correct.size(),false),shift(correct.size(),false);
    int cnt=0;
    inc(int,i,0,correct.size()){
        same[i]=(correct[i]==incorrect[i]);
        shift[i]=(correct[i]==incorrect[i+1]);
        cnt+=(same[i] or shift[i]);
    }
    if(cnt!=correct.size()){
        puts(0);
    } else {
        int first=correct.size();
        inc(int,i,0,same.size()){
            if(!same[i]){
                first=i;
                break;
            }
        ...
        set<int>indices;
        indices.insert(first+1);
        while(first>0 and incorrect[first]==incorrect[first-1]){
