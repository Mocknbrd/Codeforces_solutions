...
void testcase();
bool checkMatrix(vvi &arr);
int countNeighbours(vvi &arr,int row,int col);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    if(!checkMatrix(arr)){
        ...
        inc(i,0,n){
            inc(j,0,m){
                arr[i][j] = countNeighbours(arr,i,j);
            }
        }
        writeMatrix(arr);
    }
...
bool checkMatrix(vvi &arr){
    inc(i,0,arr.sz()){
        inc(j,0,arr[i].sz()){
            if(countNeighbours(arr,i,j) < arr[i][j]){
                return false;
            }
        }
    }
...
int countNeighbours(vvi &arr,int row,int col){
    vvi moves = {{0,1},{0,-1},{1,0},{-1,0}};
    int ans = 0;
    each(move,moves){
        int nrow = row + move[0];
        int ncol = col + move[1];
        ans += (nrow >= 0 and ncol >= 0 and nrow < arr.sz() and ncol < arr[0].sz());
    }
    return ans;
}