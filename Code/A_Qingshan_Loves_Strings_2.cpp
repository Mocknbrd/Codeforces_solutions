...
void testcase(){
    ...
    vi ans;
    while(ans.sz() <= 300 and dq.empty() is false){
        char left = dq.front().sc,right = dq.back().sc;
        if(left isnt right){
            dq.rf();
            dq.rb();
        } else {
            int leftInd = dq.front().fi,rightInd = dq.back().fi;
            if(left is '0'){
                ans.pb(rightInd);
                dq.pb(mp(rightInd + 1,'0'));
                dq.pb(mp(rightInd + 2,'1'));
            } else {
                ans.pb(leftInd - 1);
                inc(i,0,dq.sz()){
                    dq[i].fi += 2;
                }
                dq.pf(mp(leftInd + 1,'1'));
                dq.pf(mp(leftInd,'0'));
            }
        }
    }
