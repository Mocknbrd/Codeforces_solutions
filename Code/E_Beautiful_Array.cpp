...
template<typename tmp> tmp moduloInv(tmp base,tmp md){
...
void testcase(){
    ...
    map(int,vll)indices;
    inc(i,0,n){
        indices[mod(arr[i],(ll)k)].pb(arr[i]);
    }
    int odd = 0;
    each(element,indices){
        odd += mod((ll)element.sc.sz(),2ll);
        sorted(element.sc);
    }
    if(odd > 1){
        see(-1);
        ...
        ll ans = 0;
        each(element,indices){
            if(mod((ll)element.sc.sz(),2ll) is 0){
                rng_la(int,i,0,element.sc.sz(),2,1){
                    ans += (element.sc[i + 1] - element.sc[i]) / k;
                }
            } elif(element.sc.sz() > 1){
                vll prefix(element.sc.sz(),0),suffix = prefix;
                prefix[1] = element.sc[1] - element.sc[0],suffix[element.sc.sz() - 2] = element.sc.back() - element.sc[element.sc.sz() - 2];
                inc(i,2,element.sc.sz()){
                    prefix[i] = element.sc[i] - element.sc[i - 1] + prefix[i - 2];
                }
                dec(i,element.sc.sz() - 3,0){
                    suffix[i] = element.sc[i + 1] - element.sc[i] + suffix[i + 2];
                }
                ll mini = min(suffix[1],prefix[prefix.sz() - 2]);
                inc(i,2,element.sc.sz() - 2){
                    if(mod(i,2) is 0){
                        mini = min(mini,prefix[i - 1] + suffix[i + 1]);
                    }
                }
                ans += mini / k;
            }
