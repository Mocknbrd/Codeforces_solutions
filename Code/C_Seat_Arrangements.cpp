...
void testcase(){
    ...
    inc(i,0,n){
        int start = 0;
        while(start < arr[i].sz()){
            while(start < arr[i].sz() and arr[i][start] is '*'){
                start++;
            }
            int end = start;
            while(end < arr[i].sz() and arr[i][end] is '.'){
                end++;
            }
    ...
    inc(j,0,m){
        int start = 0;
        while(start < arr.sz()){
            while(start < arr.sz() and arr[start][j] is '*'){
                start++;
            }
            int end = start;
            while(end < arr.sz() and arr[end][j] is '.'){
                end++;
            }
            int length = end - start;
            ans += max(0,length - k + 1);
