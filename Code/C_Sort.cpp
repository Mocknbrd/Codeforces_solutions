...
void testcase(){
    ...
    string a,b;
    cin >> n >> q >> a >> b;
    vvi aprefix(26,vi(1,0)),bprefix = aprefix;
    inc(curr,0,26){
        inc(i,0,n){
            aprefix[curr].pb(aprefix[curr].back() + (a[i] - 'a' is curr));
            bprefix[curr].pb(bprefix[curr].back() + (b[i] - 'a' is curr));
        }
    }
    while(q--){
        int l,r;
        cin >> l >> r;
        vi acounts(26,0),bcounts(26,0);
        inc(curr,0,26){
            acounts[curr] = aprefix[curr][r] - aprefix[curr][l - 1];
            bcounts[curr] = bprefix[curr][r] - bprefix[curr][l - 1];
        }
        int ans = 0;
        inc(i,0,26){
            ans += abs(acounts[i] - bcounts[i]);
        }
        see(ans / 2);
    }
    return;
