...
void testcase();
int findMaxDist(pii curr,vpii &corners);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    int n,m;
    cin >> n >> m;
    vpii corners = {{0,0},{0,m - 1},{n - 1,0},{n - 1,m - 1}};
    min_heap(int)pq;
    inc(i,0,n){
        inc(j,0,m){
            pq.push(findMaxDist({i,j},corners));
        }
    }
    inc(k,0,n * m){
        cout << pq.top() << " ";
...
int findMaxDist(pii curr,vpii &corners){
    int ans = -inf;
    each(corner,corners){
        int dist = abs(curr.f - corner.f) + abs(curr.s - corner.s);
        ans = max(ans,dist);
    }
    return ans;
}