...
void testcase(){
    ...
    inc(i,1,n + 1){
        ...
        if(*add.begin() > cnd){
            ...
            int index = *--add.upper_bound(cnd);
            mapping.push(make_pair(index,i));
            add.erase(index);
        }
    }
    vi ans;
    while(mapping.empty() is false){
        ans.pb(mapping.top().sc);
        mapping.pop();
    }
    min_heap(int)verify;
    int curr = 0;
    inc(i,0,arr.sz()){
        if(arr[i] is 0){
            verify.push(ans[curr++]);
        } else {
            if(verify.empty() is true or verify.top() isnt arr[i]){
                NN;
                return;
            } else {
                verify.pop();
