...
class SegmentTree {
    ...
    Node* build(vll &arr,int start,int end){
        Node *node;
        if(start is end){
            node = new Node(start,end,arr[end]);
        } else {
            int mid = (start + end) >> 1;
            Node *left = this.build(arr,start,mid);
            Node *right = this.build(arr,mid + 1,end);
            node = new Node(start,end,min(right->mini,left->mini));
            node->left = left;
            node->right = right;
        }
        return node;
    }
    ll findMin(Node *node,int start,int end){
        if(!node or start > node->end or end < node->start){
            return inf;
        } elif(node->start >= start and node->end <= end){
            return node->mini;
        } else {
            return min(this.findMin(node->left,start,end),this.findMin(node->right,start,end));
        }
    }
    ...
    ll findMin(int start,int end){
        return this.findMin(this.root,start,end);
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    vll prefix(n + 1,0);
    vll oneWithout(n + 1,0);
    inc(i,1,prefix.sz()){
        prefix[i] = prefix[i - 1] + arr[i - 1];
    }
    inc(i,1,prefix.sz()){
        oneWithout[i] = oneWithout[i - 1] + arr[i - 1] - 1;
    }
    SegmentTree tree(arr);
    while(q--){
        int start,end;
        cin >> start >> end;
        int mini = tree.findMin(--start,--end);
        int length = end - start + 1;
        int left = lower_bound(all(pos[mini]),start) - pos[mini].begin();
        int right = upper_bound(all(pos[mini]),end) - pos[mini].begin();
        ...
        if(length is 1){
            ...
            if(oneWithout[end + 1] - oneWithout[start] >= count){
                YY;
            } else {
                NN;
            }
