...
const ll linf = 2e18;
void testcase();
inline int digSum(string &s,int end,int start = 0);
int main(){
    ios;
    int t = 1;
    while(t--){
...
void testcase(){
    ...
    each(value,arr){
        dp[value.sz()][digSum(value,value.sz())]++;
    }

    ll ans = 0;
    
    each(value,arr){
        int start = mod(cast(value.sz(),int),2);

        rng(length,start,value.sz() + 1,2){
            int total = value.sz() + length;
            int target = digSum(value,total / 2),extra = digSum(value,value.sz(),total / 2);
            ans += (target - extra >= 0) * dp[length][target - extra];
        }
    ...
    each(value,arr){
        ...
        rng(length,start,value.sz(),2){
            int total = value.sz() + length;
            int extra = digSum(value,value.sz() - total / 2),target = digSum(value,value.sz(),value.sz() - total / 2);
            ans += (target - extra >= 0) * dp[length][target - extra];
        }
...
inline int digSum(string &s,int end,int start){
    int ans = 0;
