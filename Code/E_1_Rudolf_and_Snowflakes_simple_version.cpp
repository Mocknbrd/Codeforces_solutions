...
bool check(ll n){
    ...
    if(base is -1){
        ...
        if(mod(base - 1,2ll) is 0 and mod(base + 1,2ll) is 0){
            return true;
        } else {
            return false;
        }
...
void testcase(){
    ...
    if(n < 7){
        ...
        ll base = 2;
        while(base * base * base <= n){
            ll start = 3,end = roundl((log2(n) / log2(base))) + 1;
            while(start <= end){
                ll mid = (start + end) >> 1ll;
                ll left = power(base,mid) - 1;
                ll right = (base - 1ll) * n;
                if(left is right){
                    YY;
                    return;
