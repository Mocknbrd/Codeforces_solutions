...
const ll linf = 2e18;
void testcase();
int dfs(vvi &arr,vpii &dirs,int r,int c);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
int dfs(vvi &arr,vpii &dirs,int r,int c){
    if(r < 0 or r >= arr.sz() or c < 0 or c >= arr[0].sz() or arr[r][c] is 0){
        return 0;
    } else {
        int ans = arr[r][c];
        arr[r][c] = 0;
        each(dir,dirs){
            ans += dfs(arr,dirs,r + dir.fi,c + dir.sc);
        }
        return ans;
    }
}
