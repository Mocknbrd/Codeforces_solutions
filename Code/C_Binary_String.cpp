...
void testcase(){
    string s;
    cin >> s;
    vi ones(s.sz() + 1,0),zeros(s.sz() + 1,0);
    inc(i,0,s.sz()){
        ones[i + 1] = ones[i] + (s[i] is '1');
        zeros[i + 1] = zeros[i] + (s[i] is '0');
    }
    int ans = min(ones.back(),zeros.back());
    inc(i,0,s.sz()){
        int start = i,end = s.sz() - 1;
        while(start <= end){
            int mid = (start + end) >> 1;
            int zerosRemaining = zeros[mid + 1] - zeros[i],onesRemoved = ones[i] + ones.back() - ones[mid + 1];
            ans = min(ans,max(onesRemoved,zerosRemaining));
            if(onesRemoved >= zerosRemaining){
                start = mid + 1;
            } else {
                end = mid - 1;
            }
