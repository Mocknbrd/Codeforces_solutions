...
void testcase(){
    ...
    vll arr(n);
    readArray(arr);
    vvll minimax(n - 2,vll(2,0));
    inc(i,0,minimax.sz()){
        minimax[i][0] = (s <= arr[i + 1]) * s;
        minimax[i][1] = arr[i + 1] - minimax[i][0];
    }
    vvll dp(n - 2,vll(2,0));
    dp[0][0] = arr.front() * minimax[0][0];
    dp[0][1] = arr.front() * minimax[0][1];
    inc(i,1,dp.sz()){
        dp[i][0] = min(dp[i - 1][0] + minimax[i - 1][1] * minimax[i][0],dp[i - 1][1] + minimax[i - 1][0] * minimax[i][0]);
        dp[i][1] = min(dp[i - 1][0] + minimax[i - 1][1] * minimax[i][1],dp[i - 1][1] + minimax[i - 1][0] * minimax[i][1]);
    }
    ll ans = min(dp.back()[0] + minimax.back()[1] * arr.back(),dp.back()[1] + minimax.back()[0] * arr.back());
    see(ans);
    return;
}
