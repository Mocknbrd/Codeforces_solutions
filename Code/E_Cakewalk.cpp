...
void testcase(){
    ...
    while(q.empty() is false){
        ...
        int cnt = curr.sc;
        seen.ins(vertex);
        pii down = mp(vertex.fi,vertex.sc + 1),right = mp(vertex.fi + 1,vertex.sc);
        if(vertex.fi is h - 1 and vertex.sc is w - 1){
            see(cnt);
            return;
        } 
        if(down.sc < w and seen.count(down) is false and arr[down.fi][down.sc] is '*'){
            seen.ins(down);
            q.push(mp(down,cnt + 1));
        }
        if(right.fi < h and seen.count(right) is false and arr[right.fi][right.sc] is '*'){
            seen.ins(right);
            q.push(mp(right,cnt + 1));
        }
        if(down.sc < w and seen.count(down) is false){
            seen.ins(down);
            q.push(mp(down,cnt + (arr[down.fi][down.sc] is '*')));
        }
        if(right.fi < h and seen.count(right) is false){
            seen.ins(right);
            q.push(mp(right,cnt + (arr[right.fi][right.sc] is '*')));
        }
    }
    return;
}
