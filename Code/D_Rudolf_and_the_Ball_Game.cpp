...
const int inf = 2e9;
const ll linf = 2e18;
int find(int curr,int move,int n){
    curr += move;
    if(curr > n){
        curr -= n;
    } elif(curr <= 0){
...
void testcase(){
    ...
    while(q.empty() is false){
        ...
        state.ins(curr);
        int query = curr.sc,pos = curr.fi;
        switch(queries[query].sc){
            case '0': {
                int nxt = find(pos,queries[query].fi,n);
                pii next = mp(nxt,query + 1);
                if(state.count(next) is false){
                    state.ins(next);
                    q.push(next);
                }
            }; break;
            ...
            default: {
                ...
                nxt = find(pos,-queries[query].fi,n);
                next = mp(nxt,query + 1);
                if(state.count(next) is false){
                    state.ins(next);
                    q.push(next);
