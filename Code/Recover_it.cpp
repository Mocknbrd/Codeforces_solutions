...
ll findPrimeIndex(vll &primes,int ind);
ll findLargestDivisor(ll n);
void testcase();
int main(){
    ios;
    int t = 1;
...
void testcase(){
    ...
    vll arr(n * 2);
    readArray(arr);
    vll primes = eratosthenes(2750131ll);
    map(ll,ll)divisors;
    mset(ll)values;
    each(value,arr){
        values.ins(value);
        if(findPrimeIndex(primes,value) is -1){
            divisors[value] = findLargestDivisor(value);
        }
    }
    vll ans;
    while(values.empty() is false){
        ll curr = *--values.end();
        values.erase(--values.end());
        ll index = findPrimeIndex(primes,curr);
        if(index is -1){
            ans.pb(curr);
            values.erase(values.find(divisors[curr]));
        } else {
            ans.pb(index);
            values.erase(values.find(index));
        }
    }
    writeArray(ans);
    return;
...
vll eratosthenes(ll n){
    vll primes;
    set(ll)notPrimes;
    inc_type(ll,prime,2,n + 1){
        if(notPrimes.count(prime) is false){
            primes.pb(prime);
            rng_type(ll,next,prime * prime,n + 1,prime){
                notPrimes.ins(next);
            }
        }
    }
...
ll findPrimeIndex(vll &primes,int ind){
    int start = 0,end = primes.sz() - 1;
    while(start <= end){
        int cnd = (start + end) >> 1;
        if(primes[cnd] is ind){
            return cnd + 1;
        } elif(ind > primes[cnd]){
            start = cnd + 1;
        } else {
