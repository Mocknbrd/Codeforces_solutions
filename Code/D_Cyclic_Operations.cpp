...
const ll linf = 2e18;
void testcase();
int findLength(umap(int,int)&funcGraph,set(int)&seen,int vertex);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    if(k is 1){
        ...
        while(q.empty() is false){
            ...
            int neighbour = funcGraph[vertex];
            if(--degree[neighbour] is 0){
                q.push(neighbour);
            }
            funcGraph.erase(vertex);
        }
        set(int)seen;
        each(node,funcGraph){
            int vertex = node.fr;
            if(seen.count(vertex) is false and findLength(funcGraph,seen,vertex) isnt k){
                NN;
                return;
            }
        }
...
int findLength(umap(int,int)&funcGraph,set(int)&seen,int vertex){
    if(seen.count(vertex) is true){
        return 0;
    } else {
        seen.ins(vertex);
        return 1 + findLength(funcGraph,seen,funcGraph[vertex]);
    }
}
