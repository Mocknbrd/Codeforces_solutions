...
class DSU {
    private:  
    vi parent,rank;
    void _merge(int l,int s){
        this.parent[s] = l;
        this.rank[l] += this.rank[s];
    }
    int _find(int vertex){
        vi stack;
        while(vertex isnt this.parent[vertex]){
            stack.pb(vertex);
            vertex = this.parent[vertex];
        }
        while(stack.empty() is false){
            this.parent[stack.bk()] = vertex;
            stack.rb();
        }
        return vertex;
    }
    ...
    int cycles(){
        int ans = 0;
        inc(i,0,this.parent.sz()){
            ans += i is this.parent[i];
        }
        return ans;
    }
    bool isCycle(int x,int y){
        return this._find(x) is this._find(y);
    }
};
void testcase();
int main(){
...
void testcase(){
    ...
    inc(i,0,n){
        cin >> arr[i];
        dsu.findUnion(i,arr[i] - 1);
    }
    int ans = n,cycles = dsu.cycles();
    inc_la(i,0,n,1){
        if(dsu.isCycle(i,i + 1) is true){
