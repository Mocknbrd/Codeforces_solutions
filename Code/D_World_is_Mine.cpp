...
void testcase(){
    ...
    while(start < n){
        int end = start;
        while(end < n and arr[start] is arr[end]){
            end++;
        }
        mapping[end - start].ins(arr[start]);
        counts[arr[start]] = end - start;
        start = end;
    }
    writeArray(arr);
    start = 0;
    int ans = 0;
    while(start < n){
        if(counts.count(arr[start]) is true){
            ans++;
            int currCnt = counts[arr[start]];
            mapping[currCnt].erase(arr[start]);
            if(mapping[currCnt].empty() is true){
                mapping.erase(currCnt);
            }
            counts.erase(arr[start]);
            if(mapping.empty() is false){
                int value = *(mapping.begin()->sc).rbegin();
                mapping[counts[value]].erase(value);
                if(mapping[counts[value]].empty() is true){
                    mapping.erase(counts[value]);
                }
                debug(arr[start] << " " << value);
                counts[value]--;
                if(counts[value] is 0){
                    counts.erase(value);
                } else {
                    mapping[counts[value]].ins(value);
                }
            }
        } 
        start++;
