...
template<typename tmp>
vector<pair<int,tmp>>enumerated(tmp &arr){
...
void testcase(){
    ...
    if(firstOne is -1 or lastZero is n or firstOne > lastZero){
        see(arr);
    } else {
        string res = arr.substr(0,firstOne);
        res += arr.substr(lastZero);
        see(res);
    }
    return;
}