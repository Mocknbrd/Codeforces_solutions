...
void testcase(){
    ...
    while(start < arr.sz()){
        int end = start;
        while(end < arr.sz() and arr[start] is arr[end]){
            end++;
        }
        ranges.pb(mp(start + 1,end));
        start = end;
    }
    int q;
    cin >> q;
    while(q--){
        ...
        cin >> l >> r;
        int left = 0,right = 0;
        int start = 0,end = ranges.sz() - 1;
        while(start <= end){
            int mid = (start + end) >> 1;
            if(l >= ranges[mid].fi and l <= ranges[mid].sc){
                left = mid;
                end = mid - 1;
            } elif(l > ranges[mid].sc){
                start = mid + 1;
            } else {
                end = mid - 1;
            }
        }
        start = 0,end = ranges.sz() - 1;
        while(start <= end){
            int mid = (start + end) >> 1;
            if(r >= ranges[mid].fi and r <= ranges[mid].sc){
                right = mid;
                start = mid + 1;
            } elif(r < ranges[mid].fi){
                end = mid - 1;
            } else {
                start = mid + 1;
            }
        ...
        if(left is right){
            ...
            see(ranges[left].sc << " " << ranges[left + 1].fi);
        }
    }
    return;
}
