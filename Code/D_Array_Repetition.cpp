...
const int inf = 2e9;
const ll linf = 8e18;
ll checkPresence(vll &arr,ll value);
ll findFirstLarger(vll &arr,ll value);
void testcase();
int main(){
    ios;
    MANY;
...
void testcase(){
    ...
    while(n--){
        ...
        cin >> type >> value;
        if(type is 1){
            dp.pb(min((ll)2e18,dp.back() + 1));
            last.pb(value);
        } else {
            dp.pb((value + 1) > 2e18 / dp.back() ? (ll)2e18 : dp.back() * (value + 1));
            last.pb(last.back());
        }
    }
    vll ans;
    while(q--){
        ...
        cin >> ind;
        while(true){
            int presence = checkPresence(dp,ind);
            if(presence isnt -1){
                ans.pb(last[presence]);
                break;
            } else {
                int first = findFirstLarger(dp,ind);
                ind = mod(ind,dp[first - 1]);
                if(ind is 0){
                    ans.pb(last[first - 1]);
                    break;
                }
...
ll findFirstLarger(vll &arr,ll value){
    int start = 0,end = arr.sz() - 1,ans = end;
    while(start <= end){
        int mid = (start + end) >> 1;
        if(arr[mid] > value){
            ans = mid;
