...
class SegmentTree {
    ...
    Node* build(vi &arr,int start,int end){
        ...
        if(start is end){
            ...
            node = new Node(start,end,min(left->mini,right->mini));
            node->left = left;
            node->right = right;
        }
        return node;
...
void testcase(){
    ...
    inc(_,0,m){
        int a,b;
        cin >> a >> b;
        ref[min(a,b) - 1] = min(ref[min(a,b) - 1],max(a,b));
    }
    SegmentTree tree(ref);
    ll ans = 0;
    inc(i,0,n){
        int start = i,end = n - 1,ind = i;
        while(start <= end){
            int mid = (start + end) >> 1;
            if(tree.query(i,mid) > arr[mid]){
                ind = mid;
                start = mid + 1;
            } else {
                end = mid - 1;
