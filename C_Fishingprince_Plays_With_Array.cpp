...
void testcase();
vector<string>compress(vll &arr,ll m);
int main(){
    ios;
    int t = 1;
    cin >> t;
...
void testcase(){
    ...
    vll b(k);
    readArray(b);
    vector<string>aCompressed = compress(a,m);
    vector<string>bCompressed = compress(b,m);
    if(aCompressed.sz() != bCompressed.sz()){
        nn;
    } else {
        inc(i,0,aCompressed.sz()){
            if(aCompressed[i] != bCompressed[i]){
                nn;
                return;
            }
        }
...
vector<string> compress(vll &arr,ll m){
    ...
    inc(i,0,arr.sz()){
        ll value = arr[i],c = 0;
        while(value >= m and value % m == 0){
            value /= m;
            c++;
        }
        c = cast(pow(m,c),ll);
        if(values.sz() == 0 or value != values.back()){
            values.pb(value);
            cnts.pb(c);
        } else {
            cnts.back() += c;
    ...
    vector<string>compressedArr;
    inc(i,0,values.sz()){
        string element = to_string(values[i]) + "," + to_string(cnts[i]);
        compressedArr.pb(element);
    }
