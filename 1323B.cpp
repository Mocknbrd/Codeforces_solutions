...
vll findFactors(ll k);
void consecOnes(vll &arr,vll &ans);
int main(){
    ios;
    ll n,m,k;
    cin>>n>>m>>k;
    ...
    consecOnes(a,aones);
    consecOnes(b,bones);
    vll factors=findFactors(k);
    ll ans=0;
    map<ll,ll>cnta,cntb;
    each(factor,factors){
        each(aone,aones){
            if(aone>=factor){
                cnta[factor]+=aone-factor+1;
            }
        }
        each(bone,bones){
            if(bone>=factor){
                cntb[factor]+=bone-factor+1;
            }
        }
    }
    each(factor,factors){
        ans+=cnta[factor]*cntb[k/factor];
    }
    puts(ans);
    return 0;
}
...
void consecOnes(vll &arr,vll &ans){
    ...
    while(start<arr.size()){
        int end=start;
        while(end<arr.size() and arr[end]==1){
            end++;
        }
        if(start!=end){
            ans.pb(end-start);
