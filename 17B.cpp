...
int main(){
    ...
    inputArray(q);
    Input(int,m);
    PQ<pair<int,pii>,vector<pair<int,pii>>,greater<pair<int,pii>>>pq;
    increment(i,0,m){
        int a,b,c;
        cin>>a>>b>>c;
        pq.push({c,{a-1,b-1}});
    ...
    int edges=0,cost=0;
    set<int>suboordinates;
    while(edges!=n-1 and pq.size()){
        pair<int,pii>curr=pq.top(); pq.pop();
        int currCost=curr.first;
        int supervisor=curr.second.first;
        int suboordinate=curr.second.second;
        if(!suboordinates.count(suboordinate)){
            cost+=currCost;
            edges++;
            suboordinates.insert(suboordinate);
        }
    }
    if(edges==n-1){
        cout<<cost<<endl;
