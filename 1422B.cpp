...
void testCase(){
    ...
    inputMatrix(matrix);
    ll ans = 0;
    int upperow = 0,lowerow = n - 1;
    while(upperow <= lowerow){
        int start = 0, end = m - 1;
        while(start <= end){
            vll values = {matrix[upperow][start]};
            if(start != end){
                values.pb(matrix[upperow][end]);
            }
            if(upperow != lowerow){
                values.pb(matrix[lowerow][start]);
                if(start != end){
                    values.pb(matrix[lowerow][end]);
                }
            }
            ll minAdd = linf;
            each(cnd,values){
