...
int main(){
    ...
    while(t--){
        ...
        pii ans={-1,-1};
        while(start<end){
            int maxi=*elements.rbegin();
            int mini=*elements.begin();
            if(arr[start]!=maxi and arr[start]!=mini and arr[end]!=maxi and arr[end]!=mini){
                ans={start+1,end+1};
                break;
            } else {
                if(arr[start]==maxi or arr[start]==mini){
                    elements.erase(arr[start++]);
                }
                if(arr[end]==maxi or arr[end]==mini){
                    elements.erase(arr[end--]);
                }
            }
        }
        if(ans.f==-1){
