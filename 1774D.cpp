    ...
    while(t--){
        ...
        if(total%n==0){
            ...
            inc(int,col,0,m){
                vi moreOne,lessZero;
                inc(int,row,0,n){
                    if(oneCnt[row]>target and arr[row][col]){
                        moreOne.pb(row);
                    } else if(oneCnt[row]<target and !arr[row][col]){
                        lessZero.pb(row);
                    }
                }
                int first=0,second=0;
                while(first<moreOne.size() and second<lessZero.size()){
                    int large=moreOne[first++];
                    int smaller=lessZero[second++];
                    moves.pb({large+1,smaller+1,col+1});
                    oneCnt[large]--;
                    oneCnt[smaller]++;
                }
            }
