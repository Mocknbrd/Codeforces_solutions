...
int main(){
    ...
    while(t--){
        ...
        string b;
        int i=first.sz()-1,j=sum.sz()-1;
        while(i>=0 and j>=0){
            int digit=first[i]-'0';
            if(first[i]<=sum[j]){
                b.pb('0'+(sum[j--]-'0')-(first[i--]-'0'));
            } else {
                if(j==0){
                    break;
                }
                int number=stoi(sum.substr(j-1,2));
                if(number-digit>9 or number-digit<0){
                    break;
                } else {
                    b+=to_string(number-digit);
                    i--;
                    j-=2;
                }
            }
