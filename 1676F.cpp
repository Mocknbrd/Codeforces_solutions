...
void testCase(){
    ...
    vi values=vi(all(cnds));
    if(!values.sz()){
        see(-1);
    } else {
        int start=0,left=values[0],right=values[0];
        while(start<values.sz()){
            int end=start+1;
            while(end<values.sz() and values[end]-values[end-1]<=1){
                end++;
            }
            if(right-left<values[end-1]-values[start]){
                left=values[start];
                right=values[end-1];
            }
            start=end;
