    ...
    inc(int,x1,0,51){
        inc(int,x2,x1+1,51){
            inc(int,y1,0,51){
                ...
                pii point={-1,-1};
                each(element,pts){
                    if(((element.first==x1 or element.first==x2) and inBetween(y1,element.second,y2)) or ((element.second==y1 or element.second==y2) and inBetween(x1,element.first,x2))){
                        continue;
                    } else {
                        point=element;
                        left++;
