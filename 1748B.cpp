...
ll getCnd(string &number,int window);
bool verify(map<char,ll>&cnts);
void testcase();
int main(){
...
ll getCnd(string &number,int window){
    if(window > number.sz()){
        ...
        ans += verify(cnts);
        slid_win(start,end,window,number.sz()){
            if(--cnts[number[start - 1]] == 0){
                cnts.erase(number[start - 1]);
            }
            cnts[number[end]]++;
            ans += verify(cnts);
        }
