...
int main(){
    ...
    while(t--){
        ...
        vi ans(n+1,-1);
        for_each(keyValuePair,pos){
            int element=keyValuePair.first;
            int segment=max(keyValuePair.second[0]+1,n-keyValuePair.second.back());
            increment_look_ahead(i,0,keyValuePair.second.size(),1){
                segment=max(segment,keyValuePair.second[i+1]-keyValuePair.second[i]);
            }
            increment(i,segment,n+1){
                if(ans[i]==-1){
                    ans[i]=element;
                } else {
                    break;
