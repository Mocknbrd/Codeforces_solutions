...
void testCase(){
    ll n;
    cin >> n;
    ll limit = findLimit(n);
    ll number = n;
    vll ternary(limit + 1,0);
    ll cnt = 0;
    dec(base,limit,0){
        ternary[base] = number / cast(pow(3,base),ll);
        number %= cast(pow(3,base),ll);
        cnt += ternary[base] >= 2;
    }
    if(cnt == 0){
        see(n);
...
ll findLimit(ll n){
    ll ans = 0, start = 1,end = 50;
    while(start <= end){
        ll mid = (start + end) >> 1;
        ll power = cast(pow(3,mid),ll);
        if(power >= n){
            ans = mid;
            end = mid - 1;
        } else {
