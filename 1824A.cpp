...
#define window(left,right,size,end) for(int left = 1, right = size; right < end; left++,right++)
...
void testcase(){
    ...
    vi seats(n);
    readArray(seats);
    int ones = cnt(seats,-1);
    int twos = cnt(seats,-2);
    vb seated(m,false);
    each(value,seats){
        if(value > 0){
            seated[value - 1] = true;
    ...
    int start = 0;
    while(start < seated.sz() and ones > 0 and !seated[start]){
        seated[start++] = ones-- > 0;
    }
    while(start < seated.sz() and twos > 0){
        seated[start++] = twos-- > 0;
    }
    see(cnt(seated,true));
    return;
