...
int main(){
    ...
    ll ans=0;
    map<ll,ll>cnts;
    vll prefix(digits.size()+1,0);
    inc(int,i,1,prefix.size()){
        prefix[i]=prefix[i-1]+digits[i-1]-'0';
    }
    inc(int,start,1,prefix.size()){
        inc(int,end,start,prefix.size()){
            cnts[prefix[end]-prefix[start-1]]++;
        }
    }
    if(a==0){
        each(row,cnts){
            each(col,cnts){
                if(row.first>0 and col.first>0){
                    break;
                } else {
        ...
        inc(int,start,1,prefix.size()){
            inc(int,end,start,prefix.size()){
                ll sum=prefix[end]-prefix[start-1];
                if(sum!=0 and a%sum==0 and cnts.count(a/sum)){
                    ans+=cnts[a/sum];
                }
            }
        }
