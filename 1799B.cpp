...
template<typename tmp>
inline tmp ceil(tmp num,tmp den){
...
int main(){
    ...
    while(t--){
        ...
        vi arr(n);
        inputArray(arr);
        if(*min_element(all(arr))!=*max_element(all(arr)) and *min_element(all(arr))<2){
            puts(-1);
        } else {
            vpii moves;
            while(*max_element(all(arr))!=*min_element(all(arr))){
                int index=0;
                inc(i,0,n){
                ...
                inc(i,0,n){
                    if(arr[i]!=arr[index]){
                        arr[i]=ceil(arr[i],arr[index]);
                        moves.pb({i+1,index+1});
                    }
                }
            }
            puts(moves.sz());
