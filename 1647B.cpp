...
inline void inputMatrix(vector<vector<tmp>>&matrix,int r=0,int c=0){
    inc(int,i,r,matrix.size()){
        inc(int,j,c,matrix[i].size()){
            cin>>matrix[i][j];
        }
    }
}
...
int main(){
    ...
    while(t--){
        ...
        inputArray(arr);
        bool elegant=true;
        inc_look_ahead(int,i,0,n,1){
            inc_look_ahead(int,j,0,m,1){
                int ones=(arr[i][j]=='1')+(arr[i][j+1]=='1')+(arr[i+1][j]=='1')+(arr[i+1][j+1]=='1');
                if(ones==3){
                    elegant=false;
                    break;
                }
