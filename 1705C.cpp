...
void testcase(){
    ...
    while(q--){
        ...
        cin >> position;
        while(position > n){
            int start = lower_bound(all(length),position) - length.begin();
            position = position - length[start - 1] - 1 +left[start];
        }
        query.pb(str[position - 1]);
    }
    each(value,query){
        see(value);
    }
