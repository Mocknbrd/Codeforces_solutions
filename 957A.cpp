...
int main(){
    ...
    while(start<n){
        ...
        inc_look_ahead(int,i,start,end,1){
            if(s[i]==s[i+1]){
                ans=false;
                break;
            }
    ...
    if(!ans){
        ...
        int questions=0,matches=0;
        inc(int,i,0,n){
            questions+=(s[i]=='?');
        }
        vector<string>patterns={"C?Y","Y?C","C?M","M?C","Y?M","M?Y"};
        for_each(pattern,patterns){
            inc_look_ahead(int,i,0,s.size(),2){
                if(s[i]==pattern[0] and s[i+1]==pattern[1] and s[i+2]==pattern[2]){
                    matches++;
                }
            }
        }
        if(matches==questions){
            nn;
        } else {
            yy;
        }
