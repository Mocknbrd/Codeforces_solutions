...
int main(){
    ...
    while(t--){
        ...
        cin>>n>>m;
        int target=(m>>1)+(m%2);
        vector<vi>available(m+1),lengths(n+1);
        vi times(n+1,0);
        inc(int,i,1,m+1){
            Input(int,number);
            lengths[number].push_back(i);
            while(number--){
                Input(int,index);
                available[i].push_back(index);
            }
        }
        vi ans(m+1,0);
        inc(int,i,1,lengths.size()){
            each(day,lengths[i]){
                int minCnt=inf,cnd=-1;
                each(index,available[day]){
                    if(times[index]<minCnt){
                        minCnt=times[index];
                        cnd=index;
                    }
                }
