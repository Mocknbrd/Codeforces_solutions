...
void testCase(){
    ...
    string time;
    cin>>time;
    int hour=stoi(time.slice(0,1));
    int minutes=stoi(time.slice(3,4));
    vi cnd={0,1,2,5,8};
    map<int,int>reflect;
    each(c,cnd){
        if(c==2 or c==5){
    ...
    ll minDiff = mod(hour * m + minutes,h * m);
    string res="00:00";
    each(cnd1,cnd){
        each(cnd2,cnd){
            each(cnd3,cnd){
                each(cnd4,cnd){
                    int nhour=10*cnd1+cnd2;
                    int nmin=10*cnd3+cnd4;
                    int rhour=10*reflect[cnd4]+reflect[cnd3];
                    int rmin=10*reflect[cnd2]+reflect[cnd1];
                    if(rhour<h and rmin<m){
                        int diff = mod(nhour * m  + nmin - hour * m + minutes, h * m);
                        if(diff < minDiff){
                            minDiff = diff;
                            res=to_string(cnd1)+to_string(cnd2)+":"+to_string(cnd3)+to_string(cnd4);
                        }
                    }
                }
            }
