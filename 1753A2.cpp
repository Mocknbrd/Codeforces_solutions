...
void testCase(){
    ...
    vi arr(n+1,0);
    inputArray(arr,1);
    int ones=cnt(arr,1)+cnt(arr,-1);
    if(ones%2){
        puts(-1);
    } else {
        vpii moves;
        vb seen(n+1,false);
        int one=cnt(arr,1),minus=cnt(arr,-1);
        int start=2;
        if(minus>one){
            while(start<=n and minus>one){
                if(arr[start]==-1){
                    moves.pb({start-1,start});
                    minus--;
                    one++;
                    seen[start-1]=seen[start]=true;
                    start+=2;
                } else {
                    start++;
                }
            ...
            while(start<=n){
                if(!seen[start]){
                    moves.pb({start,start});
                }
                start++;
            }
        } else {
            while(start<=n and one>minus){
                if(arr[start]==1){
                    moves.pb({start-1,start});
                    one--;
                    minus++;
