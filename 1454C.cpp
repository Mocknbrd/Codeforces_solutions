...
int main(){
    ...
    cin>>t;
    while(t--){
        map<int,int>lastpos,ans;
        int n;
        cin>>n;
        vi arr(n);
        inputArray(arr);
        inc(i,0,n){
            if(!lastpos.count(arr[i])){
                lastpos[arr[i]]=-1;
            } 
            ans[arr[i]]+=(abs(lastpos[arr[i]]-i)>1);
            lastpos[arr[i]]=i;
        }
        int minOps=inf;
        each(element,lastpos){
            ans[element.f]+=(element.s!=n-1);
            minOps=min(minOps,ans[element.f]);
        }
        puts(minOps);
    }
    return 0;
